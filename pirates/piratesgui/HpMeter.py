...
class HpMeter(DirectFrame):
    ...
    def __init__(self, name = '', width = 0.4, height = 0.025, fadeOut = 0, parent = None, originAtMidPt = False):
        ...
        if originAtMidPt:
            ...
            meterPos = (0, 0, 0)
        self.meter = DirectWaitBar(parent = self, relief = DGG.RAISED, borderWidth = (0.004, 0.004), range = self.max, value = self.value, frameColor = (0, 0, 0, 1), barColor = (0.1, 0.1, 0.7, 1), pos = meterPos, frameSize = (0, width, 0, height))
        self.valueLabel = DirectLabel(parent = self.meter, relief = None, text = '', text_scale = PiratesGuiGlobals.TextScaleTiny, text_align = TextNode.ACenter, text_fg = PiratesGuiGlobals.TextFG2, text_shadow = PiratesGuiGlobals.TextShadow, pos = (0.2, 0, 0.005), textMayChange = 1)
