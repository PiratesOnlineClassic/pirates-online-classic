...
class VitaeMeter(DirectFrame):
    ...
    def __init__(self, parent, **kw):
        ...
        toplevel_gui = loader.loadModelCopy('models/gui/toplevel_gui')
        ...
        detailLabel = DirectLabel(relief = None, state = DGG.DISABLED, text = PLocalizer.VitaeDesc, text_align = TextNode.ALeft, text_scale = PiratesGuiGlobals.TextScaleExtraLarge, text_fg = PiratesGuiGlobals.TextFG1, text_wordwrap = 15, text_shadow = (0, 0, 0, 1), pos = (0.0, 0, -0.025), textMayChange = 0, sortOrder = 91)
        height = -(detailLabel.getHeight() + 0.01)
        width = max(0.25, detailLabel.getWidth() + 0.04)
        self.helpBox = BorderFrame(parent = self, state = DGG.DISABLED, suffix = '_f', frameSize = (-0.04, width, height, 0.05), pos = (0.05, 0, -0.05), sortOrder = 90)
