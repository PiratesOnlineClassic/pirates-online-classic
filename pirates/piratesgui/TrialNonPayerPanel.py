...
class TrialNonPayerPanel(DirectFrame):
    ...
    def __init__(self, w = 9.0, h = 6.0, trial = False, lock = True):
        ...
        if not self.trial:
            ...
            self.imageOne = DirectFrame(parent = self, relief = None, image = geom, image_scale = (1, 1, 0.95), scale = 0.4, pos = (4.45, 0, 3.1))
            ...
            self.titleText = DirectLabel(parent = self.imageOne, relief = None, text = PLocalizer.VR_FeaturePopTitle, text_align = TextNode.ACenter, text_scale = 0.2, text_fg = PiratesGuiGlobals.TextFG16, text_font = PiratesGlobals.getPirateFont(), text_shadow = PiratesGuiGlobals.TextShadow, pos = (0, 0, 0.2))
            ...
            self.upgradeButton = DirectButton(parent = self.imageOne, relief = None, geom = (norm_geom, down_geom, over_geom), pos = (0, 0, 0.05), scale = 1, command = self.upgradeNow, text = PLocalizer.VR_FirstAddUpgrade, text_fg = PiratesGuiGlobals.TextFG1, text_font = PiratesGlobals.getInterfaceFont(), text_shadow = PiratesGuiGlobals.TextShadow, text_scale = 0.09, text_pos = (0, -0.64))
            norm_geom.getChild(0).stash()
            over_geom.getChild(0).stash()
            down_geom.getChild(0).stash()
            self.passButton = DirectButton(parent = self.imageOne, relief = None, geom = (norm_geom, down_geom, over_geom), scale = (0.8, 1, 0.1), text = PLocalizer.VR_FirstAddBasic, text_fg = PiratesGuiGlobals.TextFG15, text_font = PiratesGlobals.getInterfaceFont(), text_scale = (0.125, 0.8), text_pos = (0, -0.45), pos = (0, 0, -0.75), command = self.dismissNow)
            basegeom = gui2.find('**/close_button')
            norm_geom = basegeom.find('**/normal')
            over_geom = basegeom.find('**/over')
            down_geom = basegeom.find('**/down')
            self.dismissButton = DirectButton(parent = self, relief = None, geom = (norm_geom, down_geom, over_geom), scale = 0.4, pos = (4.45, 0, 3.1), command = self.dismissNow)
        else:
            gui2 = loader.loadModel('models/textureCards/basic_unlimited')
            self.imageOne = DirectFrame(parent = self, relief = None, image = gui2.find('**/but_message_panel_border'), image_scale = (1, 1, 0.95), scale = 0.8, pos = (4.5, 0, 3.0))
            self.imageTwo = DirectFrame(parent = self.imageOne, relief = None, image = gui2.find('**/but_compass'), scale = 1.0, pos = (-0.49, 0, 0.22))
            self.titleText = DirectLabel(parent = self.imageOne, relief = None, text = PLocalizer.FirstAddTitle, text_align = TextNode.ALeft, text_scale = 0.1, text_wordwrap = 8, text_fg = PiratesGuiGlobals.TextFG1, text_font = PiratesGlobals.getPirateFont(), text_shadow = PiratesGuiGlobals.TextShadow, pos = (-0.26, 0, 0.29))
