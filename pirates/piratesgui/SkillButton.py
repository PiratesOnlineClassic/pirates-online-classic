...
class SkillButton(DirectFrame):
    ...
    def updateSkillId(self, skillId):
        ...
        self.skillButton = DirectButton(parent = self, relief = None, pos = (0, 0, 0), text = ('', '', self.name), text_align = TextNode.ACenter, text_shadow = Vec4(0, 0, 0, 1), text_scale = 0.04, text_fg = Vec4(1, 1, 1, 1), text_pos = (0.0, 0.09), image = image, image_scale = 0.15, image_color = image_color, geom = geom, geom_scale = geom_scale, geom_color = geomColor, command = self.callback, sortOrder = 50, extraArgs = [
            skillId])
        self.skillButton.bind(DGG.ENTER, self.showDetails)
        self.skillButton.bind(DGG.EXIT, self.hideDetails)
    ...
    def createHelpFrame(self, args = None):
        ...
        stats = tuple(stats)
        if self.skillRank:
            rankText = DirectFrame(parent = self, relief = None, text = PLocalizer.makeHeadingString(PLocalizer.Rank + ' %s' % self.skillRank, 2), text_align = TextNode.ARight, text_scale = PiratesGuiGlobals.TextScaleSmall, text_fg = PiratesGuiGlobals.TextFG2, text_wordwrap = 15, text_shadow = (0, 0, 0, 1), pos = (0.45, 0, 0), textMayChange = 1, sortOrder = 92, state = DGG.DISABLED)
