...
TextDict = {
    PiratesGlobals.RewardBlackPearl: [
        PLocalizer.RewardBlackPearlComplete, PLocalizer.RewardBlackPearlReward,
        PLocalizer.RewardBlackPearlDescription, 'sail_recharge', False
    ],
    PiratesGlobals.RewardVoodooDoll: [
        PLocalizer.RewardVoodooDollComplete, PLocalizer.RewardVoodooDollReward,
        PLocalizer.RewardVoodooDollDescription, 'doll', True
    ],
    PiratesGlobals.RewardDagger: [
        PLocalizer.RewardDaggerComplete, PLocalizer.RewardDaggerReward,
        PLocalizer.RewardDaggerDescription, 'dagger', True
    ],
    PiratesGlobals.RewardVoodooStaff: [
        PLocalizer.RewardVoodooStaffComplete,
        PLocalizer.RewardVoodooStaffReward,
        PLocalizer.RewardVoodooStaffDescription, 'staff', True
    ],
    PiratesGlobals.RewardGrenade: [
        PLocalizer.RewardGrenadeComplete, PLocalizer.RewardGrenadeReward,
...
class RewardPanel(PDialog):
    ...
    def __init__(self,
                 parent=None,
                 type=PiratesGlobals.RewardBlackPearl,
                 doneCallback=None,
                 **kw):
        ...
        if isWeapon:
            ...
            background.setTransparency(1)  #background.setTransparency(0.5)
        else:
    ...
    def showWeaponNotoriety(self):
        ...
        if inv:
            for repCat in ReputationGlobals.getReputationCategories():
                totalReputation += localAvatar.getInventory().getReputation(
                    repCat)

        level, leftoverValue = ReputationGlobals.getLevelFromTotalReputation(
            InventoryType.OverallRep, totalReputation)
