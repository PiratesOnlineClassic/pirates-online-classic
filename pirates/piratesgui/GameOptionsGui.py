...
class GameOptionsGui(DirectFrame):
    ...
    def __init__(self, gameOptions, title, x, y, width, height, options = None, file_path = None, pipe = None, access = 0, chooser = 0, keyMappings = None):
        ...
        DirectFrame.__init__(self, relief = None, image = loader.loadModel('models/misc/fade'), image_scale = (5, 2, 2), image_color = (0, 0, 0, 0.8), image_pos = (0.5, 0, 0.8), state = DGG.NORMAL, pos = (x, 0.0, y), sortOrder = 20)
    ...
    def setupUpperFrame(self):
        ...
        DirectLabel(parent = self.upperFrame, relief = None, text = PLocalizer.GameOptionsTitle, text_align = TextNode.ACenter, text_scale = PiratesGuiGlobals.TextScaleTitleSmall, text_pos = (0.51, 0.46), text_fg = PiratesGuiGlobals.TextFG2, text_shadow = PiratesGuiGlobals.TextShadow, text_font = PiratesGlobals.getInterfaceOutlineFont(), textMayChange = 1)
        x += -0.08
        y = 1.3
        self.videoButton = GuiButton(parent = self, text = PLocalizer.GameOptionsVideo, pos = (x, 0, y), command = self.updateUI, extraArgs = [
            'video'])
        self.videoButton.setBin('gui-fixed', 20)
        y -= 0.08
        self.audioButton = GuiButton(parent = self, text = PLocalizer.GameOptionsAudio, pos = (x, 0, y), command = self.updateUI, extraArgs = [
            'audio'])
    ...
    def setupVideoFrame(self):
        ...
        self.videoRadios = self.createRadioButtonGroup(self.videoFrame, x, y, sx, oy, self.videoVar, 0.15, [
            ...
            PLocalizer.GameOptionsCustom], 1.4, self.videoRadioButtonCB)
    ...
    def setupLowerFrame(self):
        ...
        gui_main.removeNode()
        self.customFrame = DirectFrame(parent = self.lowerFrame, relief = None, image = bottomImage, image_scale = 0.3, frameSize = (0, self.width, 0, PiratesGuiGlobals.TextScaleLarge * 22), pos = (0, 0, 0))
        self.customFrame.setBin('gui-fixed', 15)
        x = -0.08
        y = 0.8
        self.graphicsButton = GuiButton(parent = self.lowerFrame, text = PLocalizer.GameOptionsGraphics, pos = (x, 0, y), command = self.updateUI, extraArgs = [
            'graphics'])
        self.graphicsButton.setBin('gui-fixed', 20)
        y -= 0.08
        self.displayButton = GuiButton(parent = self.lowerFrame, text = PLocalizer.GameOptionDisplay, pos = (x, 0, y), command = self.updateUI, extraArgs = [
            'display'])
    ...
    def setupGraphicsFrame(self):
        ...
        self.characterDetailRadios = self.createRadioButtonGroup(parent, rx, y, sx, 0.03, self.characterDetailVar, 0.15, [
            PLocalizer.GameOptionsLow,
            PLocalizer.GameOptionsMedium,
            PLocalizer.GameOptionsHigh], 0.8, self.characterDetailRadiosCB)
        y += oy
        ...
        self.terrainDetailRadios = self.createRadioButtonGroup(parent, rx, y, sx, 0.03, self.terrainDetailVar, 0.15, [
            PLocalizer.GameOptionsLow,
            PLocalizer.GameOptionsMedium,
            PLocalizer.GameOptionsHigh], 0.8, self.terrainDetailRadiosCB)
        y += oy
        ...
        self.reflectionRadios = self.createRadioButtonGroup(parent, rx, y, sx, 0.03, self.reflectionsVar, 0.15, [
            ...
            PLocalizer.GameOptionsOn], 0.8, self.reflectionRadiosCB)
        y += oy
        ...
        self.specialEffectsRadios = self.createRadioButtonGroup(parent, rx, y, sx, 0.03, self.specialEffectsVar, 0.15, [
            PLocalizer.GameOptionsLow,
            PLocalizer.GameOptionsMedium,
            PLocalizer.GameOptionsHigh], 0.8, self.specialEffectsRadiosCB)
        y += oy
        ...
        if self.gameOptions is not None and self.gameOptions.options.texture_scale_mode == False:
            ...
            self.textureDetailRadios = self.createRadioButtonGroup(parent, rx, y, sx, 0.03, self.textureDetailVar, 0.15, [
                ...
                PLocalizer.GameOptionsMedium,
                PLocalizer.GameOptionsHigh,
                PLocalizer.GameOptionsMaximum], 0.8, self.textureDetailRadiosCB)
        else:
            self.textureDetailRadios = self.createRadioButtonGroup(parent, rx, y, sx, 0.03, self.textureDetailVar, 0.15, [
                PLocalizer.GameOptionsLow,
                PLocalizer.GameOptionsMedium,
    ...
    def create_slider(self, update_function, default_value, x, y, resolution, label, parent):
        ...
        slider = DirectSlider(parent = parent, relief = None, command = update_slider, image = charGui.find('**/chargui_slider_small'), image_scale = (2.15, 2.15, 1.5), thumb_relief = None, thumb_image = (charGui.find('**/chargui_slider_node'), charGui.find('**/chargui_slider_node_down'), charGui.find('**/chargui_slider_node_over')), pos = (x, 0.0, y + 0.01), text_align = TextNode.ACenter, text_scale = (0.1, 0.1), text_pos = (0.0, 0.1), text_fg = PiratesGuiGlobals.TextFG1, scale = 0.28, pageSize = resolution, text = None, value = default_value)
        charGui.removeNode()
        slider.label = label
        slider['extraArgs'] = [
            slider,
    ...
    def antiliasingCheckCB(self, val):
        ...
        if not config.GetBool('want-anti-aliasing-opt', False):
            return
