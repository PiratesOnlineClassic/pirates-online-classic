...
class GameOptionsGui(DirectFrame):
    ...
    textureScaleOptionList = [
        texture_scale_low, texture_scale_medium, texture_scale_high,
        texture_scale_maximum
    ]
    textureOptionList = [
        texture_low, texture_medium, texture_high, texture_maximum
    ]
    ...
    def setupUpperFrame(self):
        ...
        gui_main.removeNode()
        x = 0.3
        self.upperFrame = DirectFrame(
            parent=self,
            relief=None,
            image=topImage,
            image_scale=0.3,
            pos=(x, 0,
                 self.height - 0.26 - PiratesGuiGlobals.TextScaleLarge * 7))
        self.upperFrame.setBin('gui-fixed', 15)
        DirectLabel(
            parent=self.upperFrame,
            relief=None,
            text=PLocalizer.GameOptionsTitle,
        ...
        x += -0.08
        y = 1.3
        self.videoButton = GuiButton(
            parent=self,
            text=PLocalizer.GameOptionsVideo,
            pos=(x, 0, y),
            command=self.updateUI,
            extraArgs=['video'])
        self.videoButton.setBin('gui-fixed', 20)
        y -= 0.08
        self.audioButton = GuiButton(
            parent=self,
        ...
        self.defaultButton = GuiButton(
            parent=parent,
            text=text,
            pos=(x, 0, y),
            command=self.defaultButtonCB)
        x += ox
        text = PLocalizer.GameOptionsRestore
        self.restoreButton = GuiButton(
            parent=parent,
            text=text,
            pos=(x, 0, y),
            command=self.restoreButtonCB)
        x += ox
        ...
        if self.gameOptions:
            ...
            command = None
        button = GuiButton(
            parent=parent, text=text, pos=(x, 0, y), command=command)
        x += ox
    ...
    def setupVideoFrame(self):
        ...
        sx = 0.24
        oy = 0.04
        self.videoRadios = self.createRadioButtonGroup(
            self.videoFrame, x, y, sx, oy, self.videoVar, 0.15, [
                PLocalizer.GameOptionsLow, PLocalizer.GameOptionsMedium,
                PLocalizer.GameOptionsHigh, PLocalizer.GameOptionsCustom
            ], 1.4, self.videoRadioButtonCB)
        return
    ...
    def setupAudioFrame(self):
        ...
        x += 0.335
        self.soundEffectCheck = CheckButton(
            parent=parent,
            relief=None,
            scale=0.4,
            pos=(x, 0, y + 0.015),
        ...
        if self.gameOptions:
            self.sound_volume_slider = self.create_slider(
                sound_volume_update_function,
                self.gameOptions.options.sound_volume, x, y, resolution, text,
                parent)
        else:
            self.sound_volume_slider = self.create_slider(
                sound_volume_update_function, default_value, x, y, resolution,
                text, parent)
        x = 0.1
        y -= 0.1
        ...
        if self.gameOptions:
            self.music_volume_slider = self.create_slider(
                music_volume_update_function,
                self.gameOptions.options.music_volume, x, y, resolution, text,
                parent)
        else:
            self.music_volume_slider = self.create_slider(
                music_volume_update_function, default_value, x, y, resolution,
                text, parent)
        return

    def setupLowerFrame(self):
        self.lowerFrame = DirectFrame(
            parent=self,
            relief=None,
            pos=(0.3, 0,
                 self.height - 0.91 - PiratesGuiGlobals.TextScaleLarge * 17))
        gui_main = loader.loadModel('models/gui/gui_main')
        bottomImage = gui_main.find('**/game_options_panel/bottom')
        bottomImage.setPos(0.52, 0, 0.9)
        gui_main.removeNode()
        self.customFrame = DirectFrame(
            parent=self.lowerFrame,
            relief=None,
            image=bottomImage,
            image_scale=0.3,
    ...
    def setupGraphicsFrame(self):
        ...
        self.characterDetailRadios = self.createRadioButtonGroup(
            parent, rx, y, sx, 0.03, self.characterDetailVar, 0.15, [
                PLocalizer.GameOptionsLow, PLocalizer.GameOptionsMedium,
                PLocalizer.GameOptionsHigh
            ], 0.8, self.characterDetailRadiosCB)
        y += oy
        text = PLocalizer.GameOptionsTerrainDetailLevel
        self.create_label(x, y, text, parent, sl)
        self.terrainDetailVar = [0]
        self.terrainDetailRadios = self.createRadioButtonGroup(
            parent, rx, y, sx, 0.03, self.terrainDetailVar, 0.15, [
                PLocalizer.GameOptionsLow, PLocalizer.GameOptionsMedium,
                PLocalizer.GameOptionsHigh
            ], 0.8, self.terrainDetailRadiosCB)
        y += oy
        text = PLocalizer.GameOptionsReflections
        self.create_label(x, y, text, parent, sl)
        self.reflectionsVar = [0]
        self.reflectionRadios = self.createRadioButtonGroup(
            parent, rx, y, sx, 0.03, self.reflectionsVar, 0.15, [
                PLocalizer.GameOptionsOff, PLocalizer.GameOptionsSkyOnly,
                PLocalizer.GameOptionsOn
            ], 0.8, self.reflectionRadiosCB)
        y += oy
        text = PLocalizer.GameOptionsSpecialEffectsLevel
        self.create_label(x, y, text, parent, sl)
        self.specialEffectsVar = [0]
        self.specialEffectsRadios = self.createRadioButtonGroup(
            parent, rx, y, sx, 0.03, self.specialEffectsVar, 0.15, [
                PLocalizer.GameOptionsLow, PLocalizer.GameOptionsMedium,
                PLocalizer.GameOptionsHigh
            ], 0.8, self.specialEffectsRadiosCB)
        y += oy
        text = PLocalizer.GameOptionsTextureDetailLevel
        self.create_label(x, y, text, parent, sl)
        self.textureDetailVar = [0]
        if self.gameOptions and self.gameOptions.options.texture_scale_mode == False:
            sx = 0.13
            self.textureDetailRadios = self.createRadioButtonGroup(
                parent, rx, y, sx, 0.03, self.textureDetailVar, 0.15, [
                    PLocalizer.GameOptionsLow, PLocalizer.GameOptionsMedium,
                    PLocalizer.GameOptionsHigh, PLocalizer.GameOptionsMaximum
                ], 0.8, self.textureDetailRadiosCB)
        else:
            self.textureDetailRadios = self.createRadioButtonGroup(
                parent, rx, y, sx, 0.03, self.textureDetailVar, 0.15, [
                    PLocalizer.GameOptionsLow, PLocalizer.GameOptionsMedium,
        ...
        self.create_label(x, y, text, parent, sl)
        self.compressedTextureCheck = CheckButton(
            parent=parent,
            relief=None,
            scale=sc,
            pos=(x + 0.42, 0, y + 0.015),
        ...
        if self.gameOptions and self.gameOptions.shader_support:
            ...
            self.create_label(x, y, text, parent, sl)
            self.shaderLevelCheck = CheckButton(
                parent=parent,
                relief=None,
                scale=sc,
                pos=(x + 0.315, 0, y + 0.015),
        ...
        self.create_label(x, y, text, parent, sl)
        self.renderedShadowsCheck = CheckButton(
            parent=parent,
            relief=None,
            scale=sc,
            pos=(x + 0.38, 0, y + 0.015),
            command=self.renderedShadowsCheckCB)
        y += oy
        text = PLocalizer.GameOptionsAggressiveMemory
        self.create_label(x, y, text, parent, sl)
        self.aggressiveMemoryCheck = CheckButton(
            parent=parent,
            relief=None,
            scale=sc,
            pos=(x + 0.62, 0, y + 0.015),
            command=self.aggressiveMemoryCheckCB)
    ...
    def setupDisplayFrame(self):
        ...
        self.create_label(x, y, text, parent, sl)
        self.fullScreenCheck = CheckButton(
            parent=parent,
            relief=None,
            scale=sc,
            pos=(x + 0.23, 0, y + 0.015),
            command=self.fullScreenCheckCB)
        ...
        for windowed_resolution in resolution_table:
            self.windowed_resolutions.append(windowed_resolution[0].__repr__() +
                                             'x' +
                                             windowed_resolution[1].__repr__())

        self.windowedResolutionMenu = OptionMenu(
            parent=parent,
            scale=0.05,
            pos=(x + 0.64, 0, y),
            items=self.windowed_resolutions,
            command=self.windowedResolutionMenuCB)
        y += oy
        ...
        for fullscreen_resolution in resolution_table:
            fullscreen_resolutions.append(fullscreen_resolution[0].__repr__() +
                                          'x' +
                                          fullscreen_resolution[1].__repr__())

        self.fullscreenResolutionMenu = OptionMenu(
            parent=parent,
            scale=0.05,
            pos=(x + 0.64, 0, y),
            items=fullscreen_resolutions,
            command=self.fullscreenResolutionMenuCB)
        y += oy
        text = PLocalizer.GameOptionsInvertMouseLook
        self.create_label(x, y, text, parent, sl)
        self.invertMouseCheck = CheckButton(
            parent=parent,
            relief=None,
            scale=sc,
            pos=(x + 0.36, 0, y + 0.015),
        ...
        resolution = 0.01
        if self.gameOptions:
            self.gui_scale_slider = self.create_slider(
                gui_scale_update_function, self.gameOptions.options.gui_scale,
                x + ox, y, resolution, text, parent)
        else:
            self.gui_scale_slider = self.create_slider(
                gui_scale_update_function, 1.0, x + ox, y, resolution, text,
                parent)
        y += oy
        text = PLocalizer.GameOptionsHardwareGamma
        self.create_label(x, y, text, parent, sl)
        self.hardwareGammaCheck = CheckButton(
            parent=parent,
            relief=None,
            scale=sc,
            pos=(x + 0.36, 0, y + 0.015),
        ...
        if base.config.GetBool('enable-frame-rate-counter', False):
            ...
            self.frameRateCheck = CheckButton(
                parent=parent,
                relief=None,
                scale=sc,
                pos=(x + 0.37, 0, y + -0.085),
                command=self.frameRateCheckCB)
            ...
            if base.win and base.win.getGsg():
                if self.gameOptions and self.gameOptions.options.gamma_enable:
                    base.win.getGsg().setGamma(
                        self.gameOptions.options.optionsGammaToGamma(
                            self.gameOptions.options.gamma))

        resolution = 0.02
        if self.gameOptions:
            self.gamma_slider = self.create_slider(
                gamma_update_function, self.gameOptions.options.gamma, x + ox,
                y, resolution, text, parent)
        else:
            self.gamma_slider = self.create_slider(
                gamma_update_function, 1.0, x + ox, y, resolution, text, parent)
        if self.gameOptions.enable_hdr:
            y += oy
            text = PLocalizer.GameOptionsHdr
            self.create_label(x, y, text, parent, sl)
            self.hdrCheck = CheckButton(
                parent=parent,
                relief=None,
                scale=sc,
                pos=(x + 0.8, 0, y + 0.015),
            ...
            if self.gameOptions:
                self.hdr_factor_slider = self.create_slider(
                    hdr_update_function, self.gameOptions.options.hdr_factor,
                    x + ox, y, resolution, text, parent)
            else:
                self.hdr_factor_slider = self.create_slider(
                    hdr_update_function, 1.0, x + ox, y, resolution, text,
                    parent)
            text = PLocalizer.GameOptionsRestartRequired
            y += oy
            self.create_label(x, y, text, parent, 0.9, color=(0.7, 0.7, 0.7, 1))
        if base.config.GetBool('want-cpu-frequency-warning', 0):
            ...
            self.create_label(x, y, text, parent, sl)
            self.cpuFrequencyWarningCheck = CheckButton(
                parent=parent,
                relief=None,
                scale=sc,
                pos=(x + 0.45, 0, y + 0.015),
                command=self.cpuFrequencyWarningCheckCB)
        else:
    ...
    def createRadioButtonGroup(self,
        ...
        i = 0
        for label in labels:
            self.create_label(x + i * sx, y, label, parent, textScale,
                              TextNode.ACenter)
            i += 1
    ...
    def create_label(self,
        ...
        label = DirectLabel(
            ...
            text_scale=PiratesGuiGlobals.TextScaleLarge * scale,
            text_pos=(x, y),
            text_fg=color,
            text_shadow=PiratesGuiGlobals.TextShadow,
            text_font=PiratesGlobals.getInterfaceOutlineFont(),
    ...
    def initResolutionSettings(self):
        ...
        if base.inAdFrame:
            ...
            if len(self.windowed_resolutions) > 2:
                self.windowedResolutionMenu[
                    'items'] = self.windowed_resolutions[:2]
            ...
            self.windowedResolutionMenu.setByValue(
                '%dx%d' % (self.gameOptions.options.window_width,
                           self.gameOptions.options.window_height), False)
            self.fullscreenResolutionMenu.setByValue(
                '%dx%d' % (self.gameOptions.options.fullscreen_width,
                           self.gameOptions.options.fullscreen_height), False)
    ...
    def set_options(self, change_display):
        if self.gameOptions is None:
            return
        if change_display:
            self.gameOptions.set_display(self.gameOptions.options, base.pipe,
                                         self.gameOptions.options.getWidth(),
        ...
        self.specialEffectsRadios[
            self.gameOptions.options.special_effects].check()
        if self.gameOptions.options.texture_scale_mode:
            if self.gameOptions.options.texture_scale in self.textureScaleOptionList:
                self.textureDetailRadios[self.textureScaleOptionList.index(
                    self.gameOptions.options.texture_scale)].check()
        else:
            if self.gameOptions.options.texture in self.textureOptionList:
                self.textureDetailRadios[self.textureOptionList.index(
                    self.gameOptions.options.texture)].check()
        self.compressedTextureCheck.setQuiet(
            self.gameOptions.options.textureCompression)
        self.characterDetailRadios[self.gameOptions.options.reflection].check()
        self.terrainDetailRadios[
            self.gameOptions.options.terrain_detail_level].check()
        ...
        self.sound_volume_slider[
            'value'] = self.gameOptions.options.sound_volume
        self.musicCheck['value'] = self.gameOptions.options.music
        self.music_volume_slider[
            'value'] = self.gameOptions.options.music_volume
        ...
        if self.cpuFrequencyWarningCheck:
            self.cpuFrequencyWarningCheck[
                'value'] = self.gameOptions.options.cpu_frequency_warning
    ...
    def fullScreenCheckCB(self, val):
        ...
        if self.gameOptions.velvet:
            ...
            if val:
                self.gameOptions.set_display(
                    self.gameOptions.options, base.pipe,
                    self.gameOptions.options.fullscreen_width,
                    self.gameOptions.options.fullscreen_height)
            else:
                self.gameOptions.set_display(
                    self.gameOptions.options, base.pipe,
                    self.gameOptions.options.window_width,
    ...
    def windowedResolutionMenuCB(self, val, index):
        ...
        if base.inAdFrame:
            ...
            self.gameOptions.set_display(
                self.gameOptions.options, base.pipe,
                base.windowed_resolution_table[index][0],
                base.windowed_resolution_table[index][1])
            if base.hasEmbedded:
            ...
            if not self.gameOptions.options.fullscreen_runtime:
                self.gameOptions.set_display(
                    ...
                    base.windowed_resolution_table[index][0],
                    base.windowed_resolution_table[index][1])
            self.gameOptions.options.window_width = base.windowed_resolution_table[
                index][0]
            self.gameOptions.options.window_height = base.windowed_resolution_table[
                index][1]
        return
    ...
    def fullscreenResolutionMenuCB(self, val, index):
        ...
        if base.inAdFrame:
            ...
            if self.gameOptions.options.fullscreen_runtime:
                self.gameOptions.set_display(
                    self.gameOptions.options, base.pipe,
                    base.fullscreen_resolution_table[index][0],
                    base.fullscreen_resolution_table[index][1])
            self.gameOptions.options.fullscreen_width = base.fullscreen_resolution_table[
                index][0]
            self.gameOptions.options.fullscreen_height = base.fullscreen_resolution_table[
                index][1]
        return
    ...
    def reflectionRadiosCB(self, val):
        ...
        funcList = [
            Water.all_reflections_off, Water.all_reflecttions_show_through_only,
    ...
    def textureDetailRadiosCB(self, val):
        ...
        if self.gameOptions.options.texture_scale_mode:
            if self.gameOptions.options.texture_scale != self.textureScaleOptionList[val[0]]:
                self.gameOptions.display_restart_dialog()
            self.gameOptions.options.texture_scale = self.textureScaleOptionList[
                val[0]]
        else:
    ...
    def defaultButtonCB(self):
        ...
        self.defaultDialog = PDialog.PDialog(
            text=PLocalizer.GameOptionsDefaultConfirm,
            style=OTPDialog.YesNo,
            giveMouse=False,
            command=self.defaultDialogCB)
    ...
    def restoreButtonCB(self):
        ...
        self.restoreDialog = PDialog.PDialog(
            text=PLocalizer.GameOptionsRestoreConfirm,
            style=OTPDialog.YesNo,
            giveMouse=False,
            command=self.restoreDialogCB)
    ...
    def restoreDialogCB(self, val):
        ...
        if val == 1:
            ...
            self.videoRadios[
                self.gameOptions.options.simple_display_option].check()
        return
    ...
    def videoRadioButtonCB(self, var):
        ...
        def handleAggressiveMemoryCheck():
            ...
            physical_memory = self.gameOptions.options.getPhysicalMemory(
                base.pipe)
