...
class PiratesTimer(OTPTimer.OTPTimer):
    ...
    def __init__(self, showMinutes = 0, mode = None, titleText = '', titleFg = None, infoText = '', cancelText = '', cancelCallback = None, alarmTime = 0):
        ...
        if self.alarmTime > 0:
            self.alarmSfx = loader.loadSfx('audio/sfx_gui_click_20.wav')
            self.outOfTimeSfx = loader.loadSfx('audio/sfx_gui_negative.mp3')
    ...
    def loadDials(self):
        model = loader.loadModelOnce('models/gui/gui_timer')
        model.setScale(0.2)
        model.flattenLight()
        ...
        self.fgLabel = DirectLabel(parent = self, state = DGG.DISABLED, relief = None, image = self.ClockImage, image_pos = (-.01, 0, 0.035), image_scale = (1.1, 1, 0.8), text_scale = 0.07, text_align = TextNode.ACenter, text_font = PiratesGlobals.getPirateOutlineFont())
    ...
    def createTimerText(self, titleText, titleFg, infoText):
        self.titleText = DirectFrame(parent = self, state = DGG.DISABLED, relief = None, text = titleText, text_align = TextNode.ACenter, text_scale = 0.05, text_fg = titleFg, text_shadow = PiratesGuiGlobals.TextShadow, text_font = PiratesGlobals.getPirateOutlineFont(), textMayChange = 1, text_wordwrap = 6, pos = (0, 0, 0.7))
    ...
    def startAlarm(self):
        if self.end == False:
            ...
            scale = origScale * 1.2
            t = 0.5
            self.alarmInterval = Sequence(Parallel(SoundInterval(self.alarmSfx), LerpScaleInterval(self, t, scale, origScale, blendType = 'noBlend')),
                                          Parallel(SoundInterval(self.alarmSfx), LerpScaleInterval(self, t, origScale, scale, blendType = 'noBlend')))
