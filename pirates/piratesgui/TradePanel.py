...
class TradeOfferFrame(DirectFrame):
    ...
    def __init__(self, avName, w, h, isLocal, tradePanel):
        ...
        self.itemsLabel = DirectLabel(
            ...
            textMayChange=1)
        if self.isLocal:
            self.goldButton = DirectButton(
                parent=self,
                relief=DGG.RAISED,
                borderWidth=PiratesGuiGlobals.BorderWidthSmall,
                frameSize=(0, w - 0.04, 0, 0.04),
                pos=(0.02, 0, h - 0.1),
                text='Add 1 Gold',
                text_scale=PiratesGuiGlobals.TextScaleSmall,
                text_align=TextNode.ALeft,
                ...
                command=self.tradePanel.addGold)
        self.readyButton = DirectButton(
            parent=self,
            borderWidth=PiratesGuiGlobals.BorderWidthSmall,
            frameSize=(0, w - 0.04, 0, 0.04),
            pos=(0.02, 0, 0.02),
...
class TradePanel(GuiPanel.GuiPanel):
    ...
    def __init__(self, trade):
        ...
        self.localOffer = TradeOfferFrame(localName, 0.4 - 0.02, 0.95 - 0.02, 1,
                                          self)
        self.localOffer.setPos(0.41, 0, 0.01)
        otherName = 'Other'
        self.otherOffer = TradeOfferFrame(otherName, 0.4 - 0.02, 0.95 - 0.02, 0,
                                          self)
        ...
        if base.cr.avatarFriendsManager.checkIgnored(
                self.trade.firstAvatarId
        ) or base.cr.avatarFriendsManager.checkIgnored(
                self.trade.secondAvatarId):
            self.closePanel()
            return
...
# okay decompiling .\pirates\piratesgui\TradePanel.pyc
