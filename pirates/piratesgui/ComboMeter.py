...
class ComboMeter(DirectObject.DirectObject):
    ...
    def newHit(self, value, numAttackers, totalDamage):
        ...
        if self.combo < value:
            ...
            if self.animIval:
                ...
                self.animIval = None
            scaleIval = self.comboCounter.scaleInterval(
                0.2, 1.0, startScale=2.0, blendType='easeIn')
        ...
        if abs(self.totalDamage) < abs(totalDamage):
            ...
            self.subText['text'] = str(abs(
                self.totalDamage)) + ' ' + PLocalizer.Damage
        taskMgr.remove(self.__getResetComboMeter())
        taskMgr.doMethodLater(self.COMBO_METER_RESET, self.resetMeter,
                              self.__getResetComboMeter())
        return
    ...
    def newBackstab(self):
        ...
        if self.backstabAnimIval:
            ...
            self.backstabAnimIval = None
        scaleIval = self.backstabText.scaleInterval(
            0.2, 1.0, startScale=2.0, blendType='easeIn')
        self.backstabAnimIval = Parallel(scaleIval)
        self.backstabAnimIval.start()
        taskMgr.remove(self.__getResetBackstab())
        taskMgr.doMethodLater(self.COMBO_METER_RESET, self.resetBackstab,
                              self.__getResetBackstab())
        return
    ...
    def __fadeInMeter(self):
        ...
        if self.faderIn:
            return
        self.faderIn = LerpFunctionInterval(
    ...
    def __fadeOutMeter(self):
        ...
        self.faderOut = Sequence(fadeOut, Func(self.text.hide),
                                 Func(restoreColor))
        self.faderOut.start()
        return
    ...
    def __fadeOutBackstab(self):
        ...
        self.backstabText.setAlphaScale(1.0)
        fadeOut = LerpFunctionInterval(
            self.backstabText.setAlphaScale, fromData=1, toData=0, duration=1.0)
        self.backstabFaderOut = Sequence(fadeOut, Func(self.backstabText.hide))
        self.backstabFaderOut.start()
        return


# okay decompiling .\pirates\piratesgui\ComboMeter.pyc
