...
class ShipItemGUI(InventoryItemGui.InventoryItemGui):
    ...
    def createGui(self):
        ...
        self.miscText = None
        self.picture = DirectFrame(parent = self, relief = None, state = DGG.DISABLED, image = myTex, image_scale = 0.06, pos = (0.085, 0, 0.075))
            ...
            if not Freebooter.getPaidStatus(base.localAvatar.getDoId()):
                ...
                if item != ItemId.INTERCEPTOR_L1 and item != ItemId.MERCHANT_L1:
                    if not self.miscText:
                        self.miscText = DirectLabel(parent = self, relief = None, text = '', text_scale = PiratesGuiGlobals.TextScaleSmall, text_align = TextNode.ALeft, text_fg = PiratesGuiGlobals.TextFG2, text_shadow = PiratesGuiGlobals.TextShadow, text_wordwrap = 11, text_pos = (0.05, 0, 0), pos = (0.16, 0, 0.025))
                    ...
                    appendMe = DirectFrame(parent = self, relief = None, pos = (self.width - 0.405, 0, -0.015), state = DGG.DISABLED, geom = subCard.find('**/subscribers_lock'), geom_scale = 0.12, geom_pos = (0.06, 0, 0.06))
                    subCard.removeNode()
