...
class ShipItemGUI(InventoryItemGui.InventoryItemGui):
    ...
    def __init__(self, data, trade=0, buy=0, sell=0, use=0, **kw):
        ...
        InventoryItemGui.InventoryItemGui.__init__(self, data, trade, buy, sell,
                                                   use, **kw)
    ...
    def createGui(self):
        ...
        if item in [
            ...
            if item in [
                ...
                if item in [
                        ItemId.WARSHIP_L1, ItemId.WARSHIP_L2, ItemId.WARSHIP_L3
                ]:
            ...
            if not Freebooter.getPaidStatus(base.localAvatar.getDoId()):
                ...
                if item != ItemId.INTERCEPTOR_L1 and item != ItemId.MERCHANT_L1:
                    ...
                    appendMe = DirectFrame(
                        parent=self,
                        relief=None,
                        pos=(self.width - 0.405, 0, -0.015),
                        state=DGG.DISABLED,
                        geom=subCard.find('**/subscribers_lock'),
                        geom_scale=0.12,
                        geom_pos=(0.06, 0, 0.06))
                    subCard.removeNode()
            elif minLvl > ReputationGlobals.getLevelFromTotalReputation(
                    repId, repAmt)[0]:
                if not self.miscText:
                    self.miscText = DirectLabel(
                        parent=self,
