...
class ShipStatFrame(BorderFrame):
    ...
    def __init__(self,
        ...
        optiondefs = (('state', DGG.DISABLED, None),
                      ('frameSize', (0.05, 0.64, 0.04, 0.39), None))
    ...
    def scheduleShow(self, time=1):
        if self.showTask:
            ...
            self.showTask = None
        if time:
            self.showTask = taskMgr.doMethodLater(
                time, self.show, 'ShipStatFrame-show', extraArgs=[])
        else:
            self.show()
        return
...
# okay decompiling .\pirates\piratesgui\ShipStatFrame.pyc
