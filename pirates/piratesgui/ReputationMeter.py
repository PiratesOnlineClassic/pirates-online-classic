...
class ReputationMeter(DirectFrame):
    ...
    def __init__(self, category, width = 0.4):
        ...
        self.meterFrame = DirectFrame(parent = self, relief = None, image = gui.find('**/ship_battle_speed_bar*'), image_scale = (width * 0.75, 1.0, 0.6), image_pos = (0, 0, 0), pos = (0, 0, -0.01))
        self.meter = DirectWaitBar(parent = self.meterFrame, relief = DGG.FLAT, state = DGG.DISABLED, range = self.max, value = self.value, frameSize = (-width * 0.5, width * 0.5, -0.008, 0.008), frameColor = (55 / 255.0, 65 / 255.0, 55 / 255.0, 1), barColor = (150 / 255.0, 185 / 255.0, 150 / 255.0, 1), pos = (0, 0, 0.01))
        self.valueLabel = DirectLabel(parent = self.meter, relief = None, text = '', text_scale = PiratesGuiGlobals.TextScaleMed, text_align = TextNode.ACenter, text_fg = PiratesGuiGlobals.TextFG2, text_shadow = PiratesGuiGlobals.TextShadow, text_font = PiratesGlobals.getInterfaceFont(), text_pos = (0, 0.032), pos = (0, 0, -0.01), textMayChange = 1)
