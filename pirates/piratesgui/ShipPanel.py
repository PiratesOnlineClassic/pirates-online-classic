...
class ShipPanel(DirectFrame):
    ...
    def __init__(self, shipPage, shipId, **kwargs):
        ...
        cannonLabel = DirectLabel(
            ...
            text_fg=(1, 1, 1, 1),
            text_shadow=(0, 0, 0, 1))
        self.broadsideLeftLimit = DirectLabel(
            parent=self,
            relief=None,
            state=DGG.DISABLED,
            geom=broadsideIcon,
            geom_scale=(-0.45, 0.45, 0.45),
            text='',
            text_scale=0.045,
            text_align=TextNode.ACenter,
            text_pos=textPos,
            ...
            text_font=PiratesGlobals.getInterfaceOutlineFont(),
            pos=(0.74, 0, 0.31))
        self.broadsideRightLimit = DirectLabel(
            parent=self,
            relief=None,
            state=DGG.DISABLED,
            geom=broadsideIcon,
    ...
    def setShipClass(self, shipClass):
        self.classLabel['text'] = PLocalizer.makeHeadingString(
            PLocalizer.ShipClassNames.get(shipClass), 1)
    ...
    def setShipCrew(self, crewArray, maxCrewCount):
        ...
        if not self.crewDots:
            ...
            for x in range(maxCrewCount):
                crewDot = DirectButton(
                    parent=self,
                    relief=None,
                    geom=(crewDotGeom[1], crewDotGeom[1], crewDotGeom[1],
                          crewDotGeom[0]),
                    pos=(0.54 + 0.04 * (x % columns), 0,
                         0.418 - 0.04 * (x / columns)),
                    scale=0.5)
