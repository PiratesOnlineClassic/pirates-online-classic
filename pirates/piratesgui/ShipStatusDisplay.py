...
class ShipStatusDisplay(GuiTray.GuiTray):
    ...
    def __init__(self, parent, shipId, **kw):
        optiondefs = (('relief', None, None), ('pos', (-0.03, 0, -0.55),
                                               None), ('shipId', shipId, None),
                      ('shipName', ('', 0),
                       self.applyShipName), ('shipClass', '',
                                             self.applyShipClass),
                      ('shipHp', (0, 0), self.applyShipHp), ('shipSp', (0, 0),
                                                             self.applyShipSp),
                      ('shipCargo', (0, 0), self.applyShipCargo), ('oldCargo',
                                                                   0, None),
                      ('shipCrew', (0, 0),
                       self.applyShipCrew), ('oldCrew', 0, None), ('ownShip', 0,
                                                                   None))
    ...
    def loadGUI(self):
        ...
        self.hpMeter = DirectWaitBar(
            ...
            text_shadow=(0, 0, 0, 1),
            text_font=PiratesGlobals.getInterfaceFont())
        self.hpMeterChange = DirectFrame(
            parent=self.hpFrame,
            relief=DGG.FLAT,
            borderWidth=(0.004, 0.004),
        ...
        self.hpMeterDownIval = Sequence(
            Func(self.hpMeterChange.show), Wait(0.1),
            LerpColorInterval(
                self.hpMeterChange,
                0.5,
            ...
            LerpColorInterval(
                self.hpMeterChange,
                0.25,
                color=VBase4(0.0, 0.0, 0.0, 1.0),
                blendType='easeOut'), Func(self.hpMeterChange.hide))
        self.hpMeterUpGreenIval = Sequence(
            Func(self.hpMeterChange.show), Wait(0.1),
            LerpColorInterval(
                self.hpMeterChange, 0.75, color=VBase4(0.1, 0.7, 0.1, 1.0)),
            Func(self.hpMeterChange.hide))
        self.hpMeterUpRedIval = Sequence(
            Func(self.hpMeterChange.show), Wait(0.1),
            LerpColorInterval(
                self.hpMeterChange, 0.75, color=VBase4(1.0, 0.0, 0.0, 1.0)),
            Func(self.hpMeterChange.hide))
        self.hpMeterUpYellowIval = Sequence(
            Func(self.hpMeterChange.show), Wait(0.1),
            LerpColorInterval(
                self.hpMeterChange, 0.75, color=VBase4(1.0, 1.0, 0.1, 1.0)),
            Func(self.hpMeterChange.hide))
        ...
        self.cargoMeter = GuiButton.GuiButton(
            parent=self,
            frameSize=(-0.0453125, 0.0453125, -0.0453125, 0.0453125),
            pos=(0.32, 0, 0.21),
            helpText=PLocalizer.CargoIconHelp,
            helpPos=(0.054, 0, -0.1),
            command=self.toggleCargo,
            image=(base1, base3, base2),
            image_scale=0.1,
            relief=None)
        icons = loader.loadModel('models/gui/toplevel_gui')
        tex = icons.find('**/topgui_icon_ship_chest01')
        self.cargoLabel = DirectLabel(
            parent=self.cargoMeter,
            relief=None,
            state=DGG.DISABLED,
            image=tex,
            image_scale=0.27,
            image_color=(1, 1, 1, 0.8),
            text='0/0',
            text_scale=0.045,
            text_align=TextNode.ACenter,
            text_pos=(0.0045, -0.025),
            text_fg=PiratesGuiGlobals.TextFG1,
            text_shadow=(0, 0, 0, 1),
            text_font=PiratesGlobals.getInterfaceFont())
        self.crewMeter = GuiButton.GuiButton(
            parent=self,
            relief=None,
            state=DGG.DISABLED,
            frameSize=(-0.0453125, 0.0453125, -0.0453125, 0.0453125),
            pos=(0.45, 0, 0.21),
            helpText=PLocalizer.CrewIconHelp,
            helpPos=(-0.05, 0, -0.1),
            image=base1,
            image_scale=0.1)
        icons = loader.loadModel('models/textureCards/icons')
        tex = icons.find('**/icon_stickman')
        self.crewLabel = DirectLabel(
            parent=self.crewMeter,
            relief=None,
            state=DGG.DISABLED,
            image=tex,
            image_scale=0.08,
        ...
        self.statusEffectsPanel = StatusEffectsPanel.StatusEffectsPanel(
            parent=self, pos=(0.3, 0, 0.3))
    ...
    def setupPermissionUI(self):
        if not self.permissionButton:
            ...
            self.permissionButton = GuiButton.GuiButton(
                ...
                image=(base1, base3, base2),
                image_scale=0.1,
                command=self.showPermissionPanel)
    ...
    def createBoardingPermissionPanel(self):
        if not self.permissionPanel:
            self.permissionPanel = BoardingPermissionPanel(
                self, ownShip=self['ownShip'], command=self.hidePermissionPanel)
                ...
                if team == PiratesGlobals.UNDEAD_TEAM:
                    ...
                    if team == PiratesGlobals.FRENCH_UNDEAD_TEAM:
                        ...
                        if team == PiratesGlobals.SPANISH_UNDEAD_TEAM:
                            self.nameBox[
                                'text_fg'] = PiratesGlobals.SPANISH_NAMETAG
    ...
    def setShipHp(self, hp, maxHp):
        ...
        if prevValue > hp:
            ...
            if prevValue < hp:
                ...
                self.hpMeterChange['frameSize'] = (0.0, frameRight, -0.011,
                                                   0.008)
                if hpFraction >= 0.5:
    ...
    def setShipCargo(self, cargo, maxCargo):
        ...
        if self['oldCargo'] != cargo:
            ...
            scaleAnim = self.cargoMeter.scaleInterval(
                0.5, Point3(1), startScale=Point3(1.5), blendType='easeIn')
            scaleAnim.start()
    ...
    def setShipCrew(self, crew, maxCrew):
        ...
        if self['oldCrew'] != crew:
            ...
            scaleAnim = self.crewMeter.scaleInterval(
                0.5, VBase3(1), startScale=VBase3(1.5), blendType='easeIn')
            scaleAnim.start()
    ...
    def updateStatusEffects(self, effects):
        ...
        for effectKeyId in effectIdList:
            ...
            if effectKeyId not in self.skillEffects.keys():
                self.statusEffectsPanel.addStatusEffect(effectId, maxDur, ts,
                                                        attackerId)
            else:
                self.statusEffectsPanel.updateStatusEffect(
                    effectId, maxDur, ts, attackerId)
        ...
        for effectKeyId in self.skillEffects.keys():
            if effectKeyId not in effectIdList:
                ...
                if buff:
                    effectId = buff[0]
                    attackerId = buff[3]
                    self.statusEffectsPanel.removeStatusEffect(
                        effectId, attackerId)
    ...
    def addDurationTask(self):
        if not self.durationTask:
            self.durationTask = taskMgr.add(
                self.updateDurationTask, self.taskName('updateStatusPanelTask'))
    ...
    def enableAnchorButton(self):
        if not self.anchorButton:
            self.anchorButton = AnchorButton.AnchorButton(
                parent=base.a2dBottomCenter,
                helpText=PLocalizer.AnchorButtonHelp,
                image_scale=0.18,
                pos=(0, 0, 0.34),
                scale=1.2,
                command=self.handleAnchorButton)
