...
class CollectionPage(InventoryPage.InventoryPage):
    ...
    def __init__(self):
        ...
        self.backButton = DirectButton(parent = self, relief = None, image_scale = 0.18, image = butImg, pos = (0.35, 0, 1.055), command = self.goMainPage, extraArgs = [])
        gui.removeNode()
        self.setPics = {}
        self.setFrames = {}
        self.setNum = {}
        ...
        for loopItr in range(setSize):
            ...
            if isLocked or howMany > 0:
                if setItem in self.setPics:
                    ...
                    self.setFrames[setItem] = DirectFrame(parent = self, relief = None, image = frameImg, image_scale = 0.4, image_pos = (0, 0, 0), pos = (0.28 + 0.18 * rowSpot, 0, localHeight - 0.36 - 0.18 * colSpot))
                    pic_name = CollectionMap.Assets[setItem]
                    if isLocked:
                        tex = gui.find('**/subscribers_lock')
                        use_scale = 0.25
                        self.setPics[setItem] = DirectButton(parent = self, relief = None, image = tex, image_scale = use_scale, image_pos = (0, 0, 0), pos = (0.28 + 0.18 * rowSpot, 0, localHeight - 0.36 - 0.18 * colSpot), text = PLocalizer.Collections[setItem], text_fg = PiratesGuiGlobals.TextFG2, text_align = TextNode.ACenter, text_wordwrap = 6, text_scale = 0.025, text_pos = (0, -0.085, 0), command = base.localAvatar.guiMgr.showNonPayer, extraArgs = [
                            'Restricted_Treasure_Selection',
                            7])
                    else:
                        tex = self.card.find('**/%s*' % pic_name)
                        use_scale = 0.39
                        self.setPics[setItem] = DirectFrame(parent = self, relief = None, image = tex, image_scale = use_scale, image_pos = (0, 0, 0), pos = (0.28 + 0.18 * rowSpot, 0, localHeight - 0.36 - 0.18 * colSpot), text = PLocalizer.Collections[setItem], text_fg = PiratesGuiGlobals.TextFG2, text_align = TextNode.ACenter, text_wordwrap = 6, text_scale = 0.025, text_pos = (0, -0.085, 0))
