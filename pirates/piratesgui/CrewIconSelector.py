...
class CrewIconSelector(GuiPanel.GuiPanel):
    ...
    def __init__(self, title, selectedIconKey=0):
        GuiPanel.GuiPanel.__init__(
            self, title, 0.8, 0.77, 0, '', pos=(0.52, 0, -0.82))
            ...
            for rowSpot in range(4):
                if colSpot > 0:
                    ...
                    blip.setPos(0.24 + 0.18 * rowSpot, 0,
                                self.localHeight - 0.4 - 0.18 * colSpot)

        crewIconParent.flattenStrong()
        self.iconFrame = DirectFrame(
            parent=self, relief=None, geom=crewIconParent)
        self.iconFrame.setPos(-0.1, 0, -0.12)
        self.bOk = CrewIconSelectorConfirmButton(
            text=PLocalizer.GenericConfirmOK, command=self.__handleOK)
        self.bCancel = CrewIconSelectorCancelButton(
            text=PLocalizer.DialogCancel, command=self.__handleCancel)
    ...
    def __handleOK(self):
        ...
        base.localAvatar.guiMgr.crewPage.enableClearCrewIconButton(
            self.selectedIconKey)
        self.destroy()
        ...
        for loopItr, iconFile in CREW_ICONS.iteritems():
            ...
            self.setPics[setKey] = DirectButton(
                ...
                pos=(0.24 + 0.18 * rowSpot, 0,
                     self.localHeight - 0.4 - 0.18 * colSpot - 0.18),
                command=self.__setIconKey,
                extraArgs=[setKey],
                pressEffect=1)
