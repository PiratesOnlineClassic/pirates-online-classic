...
class PiratesOffLineRequest(GuiPanel.GuiPanel):
    ...
    def __init__(self, title, message, tokenString=None, preExistPerm=0):
        ...
        if tokenString:
            GuiPanel.GuiPanel.__init__(
                self, title, 0.66, 0.79, 0, '', pos=(0.65, 0, -0.83))
        else:
        ...
        if tokenString != None:
            self.tokenMessage = DirectLabel(
                ...
                pos=(0.33, 0, 0.55),
                textMayChange=1)
        self.bOk = PiratesOffLineConfirmButton(
        ...
        if tokenString:
            self.cBox1 = self.makeCheckbox((0.215, 0, 0.46),
                                           PLocalizer.GuildInviteSingleButton,
                                           self.__handleSet, 1, [1])
            self.cBox1.reparentTo(self)
            self.cBox2 = self.makeCheckbox((0.215, 0, 0.365),
                                           PLocalizer.GuildInviteLimitedButton,
                                           self.__handleSet, 0, [2])
            self.cBox2.reparentTo(self)
            self.cBox3 = self.makeCheckbox(
                (0.215, 0, 0.175), PLocalizer.GuildInviteUnlimitedButton,
                self.__handleSet, 0, [3])
    ...
    def __handleOk(self):
        ...
        if self.currentCodeOption == 1:
            ...
            npTokenCount += 1
            base.localAvatar.guiMgr.guildPage.receiveNonPermTokenCount(
                npTokenCount)
            self.destroy()
            return
        else:
            if self.currentCodeOption == 2:
                base.cr.guildManager.updateTokenRValue(self.tokenString,
                                                       self.sliderOutputValue)
                npTokenCount = base.localAvatar.guiMgr.guildPage.getNonPermTokenCount(
                )
                npTokenCount += 1
                base.localAvatar.guiMgr.guildPage.receiveNonPermTokenCount(
