...
class WeaponButton(GuiButton):
    ...
    def __init__(self,
        ...
        buttonImages = (button_gui.find('**/box_base'),
                        button_gui.find('**/box_base_down'),
                        button_gui.find('**/box_base_over'))
        buttonColors = ((1, 1, 1, 1), (0.8, 0.8, 0.8, 1), (1.0, 1.0, 1.0, 1),
                        (0.6, 0.6, 0.6, 1))
        optiondefs = (('relief', None, None), ('state', 'normal', None),
                      ('frameSize', (0, 0.12, 0, 0.12),
                       None), ('image', buttonImages,
                               None), ('image_scale', 0.13,
                                       None), ('image_pos', (0.06, 0, 0.06),
                                               None), ('extraArgs', [0], None))
    ...
    def setWeaponId(self, weaponId):
        ...
        if not self.invReq:
            self.invReq = DistributedInventoryBase.getInventory(
                localAvatar.getInventoryId(), self.initInventory)

    def initInventory(self, inventory):
        self.invReq = None
        if not inventory:
            self.notify.warning(
                'Could not get inventory for setWeaponid: %s' % self.weaponId)
            return
        numItems = inventory.getStackQuantity(self.weaponId)
        if numItems <= 0:
            self.weaponId = 0
        ...
        if not Freebooter.getPaidStatus(base.localAvatar.getDoId()):
            if not Freebooter.allowedFreebooterWeapon(self.category):
                if EconomyGlobals.getItemCategory(
                        self.weaponId) == ItemType.WEAPON:
                    gui = loader.loadModel('models/gui/toplevel_gui')
        if gui:
    ...
    def updateRep(self, value):
        ...
        if self.lastExp:
            ...
            if expChange:
                localAvatar.guiMgr.gameGui.createExpAlert(
                    expChange, 4.0, Vec3(0.625, 0.0, -0.8), Vec3(
                        0.0, 0.0, 0.25))
        if self.weaponLeveledIval:
            self.weaponLeveledIval.finish()
            self.weaponLeveledIval = None
        ...
        if self.lastLevel != level:
            ...
            self.weaponLeveledIval = Sequence(
                Parallel(
                    LerpPosInterval(self, 0.2, movePos, startPos),
                    LerpColorScaleInterval(
                        glowObj,
                        0.2,
                        glowColor,
                        startColor,
                        blendType='easeInOut')), Wait(0.4),
                Parallel(
                    LerpColorScaleInterval(
                        glowObj, 3, startColor, blendType='easeInOut'),
                    Sequence(
                        LerpPosInterval(self, 0.085, startPos, movePos),
                        LerpPosInterval(self, 0.06,
                                        movePos - Point3(0.0, 0.0, 0.005),
                                        startPos),
                        LerpPosInterval(self, 0.035, startPos,
                                        movePos - Point3(0.0, 0.0, 0.005)))))
            ...
            if self.lastExp != value:
                ...
                self.expGainIval = Sequence(
                    Parallel(
                        LerpPosInterval(self, 0.1, movePos, startPos),
                        LerpColorScaleInterval(
                            glowObj,
                            0.1,
                            glowColor,
...
class TonicButton(SkillButton):
    ...
    def getBestTonic(self):
        ...
        if inv and inv.isReady():
            ...
            idealAmount = max(
                0,
                localAvatar.getMaxHp() * 0.8 - localAvatar.getHp())
    ...
    def initInventory(self, inventory):
        ...
        if not inventory:
            self.notify.warning(
                'Could not get inventory for setTonicid: %s' % self.skillId)
            return
...
class ShipRepairButton(SkillButton):
    ...
    def __init__(self):
        ...
        self.accept(
            'inventoryQuantity-%s-%s' % (localAvatar.getInventoryId(),
                                         self._skillId),
            self._inventoryQuantityChanged)
        self._updateAmount()
        return
    ...
    def _updateAmount(self):
        if not self.invReq:
            self.invReq = DistributedInventoryBase.getInventory(
                localAvatar.getInventoryId(), self._gotInventory)
    ...
    def callback(self, skillId):
        if localAvatar.guiMgr.combatTray.trySkill(InventoryType.UseItem,
                                                  skillId, 0):
            self._ignoredAmountUpdates += 1
...
class CombatTray(GuiTray):
    ...
    BASIC_ATTACKS = (InventoryType.CutlassHack, InventoryType.PistolShoot,
                     InventoryType.MusketShoot, InventoryType.MeleePunch,
                     InventoryType.DaggerCut, InventoryType.GrenadeThrow,
                     InventoryType.StaffBlast, InventoryType.DollAttune,
                     InventoryType.DollPoke)
    IGNORES_INPUT_LOCK = (InventoryType.UseItem, EnemySkills.PISTOL_RELOAD,
                          EnemySkills.GRENADE_RELOAD)
    NO_PRINT_RANGE = (InventoryType.UseItem, EnemySkills.PISTOL_RELOAD,
                      EnemySkills.PISTOL_CHARGE, EnemySkills.GRENADE_RELOAD,
                      EnemySkills.GRENADE_CHARGE, EnemySkills.STAFF_FIZZLE,
                      ...
                      EnemySkills.LEFT_BROADSIDE, EnemySkills.RIGHT_BROADSIDE,
                      EnemySkills.DOLL_UNATTUNE, InventoryType.CutlassHack,
    ...
    NO_VOLLEY_PROJECTILES = (EnemySkills.STAFF_WITHER_CHARGE,
                             EnemySkills.STAFF_SOULFLAY_CHARGE,
                             EnemySkills.STAFF_HELLFIRE_CHARGE,
                             EnemySkills.STAFF_PESTILENCE_CHARGE,
                             EnemySkills.STAFF_BANISH_CHARGE,
                             EnemySkills.STAFF_DESOLATION_CHARGE,
                             EnemySkills.PISTOL_CHARGE,
                             EnemySkills.PISTOL_RELOAD,
                             EnemySkills.GRENADE_CHARGE,
                             EnemySkills.GRENADE_RELOAD, InventoryType.UseItem)
    ...
    def __init__(self, parent, **kw):
        ...
        self.chargeMeter = DirectLabel(
            parent=base.a2dBottomCenter,
            relief=None,
            state=DGG.DISABLED,
            frameColor=(1, 1, 1, 1),
            pos=(0.45, 0, 0.2),
            image=self.chargeCard.find('**/distance_meter'),
            image_scale=(0.06, 0, 0.6),
            image_pos=(-0.012, 0, 0.3))
        self.chargeMeter.setTransparency(1)
        self.chargeMeterBar = DirectWaitBar(
            parent=self.chargeMeter,
            relief=DGG.FLAT,
            state=DGG.DISABLED,
            range=100,
            value=0,
            frameColor=(0, 0, 0, 1),
            barColor=(1, 0.2, 0.1, 1),
            pos=(-0.005, 0, 0.01),
            hpr=(0, 0, -90),
            frameSize=(0, 0.583, 0, 0.014))
        self.chargeMeter.hide()
        self.activeName = DirectLabel(
            parent=self.reloadFrame,
            relief=None,
            state=DGG.DISABLED,
            text=PLocalizer.UsingSkill,
        ...
        self.skillFrame = DirectFrame(
            parent=self.reloadFrame,
            relief=None,
            state=DGG.DISABLED,
            image=self.card.find('**/cutlass_sweep'),
            image_scale=0.12,
            sortOrder=20,
            image_pos=(0, 0, 0.02))
        ...
        if not Freebooter.getPaidStatus(base.localAvatar.getDoId()):
            ...
            if not Freebooter.allowedFreebooterWeapon(category):
                base.localAvatar.guiMgr.showNonPayer(
                    quest='Restricted_Weapon_Selection', focus=5)
                return
        state = localAvatar.getGameState()
        if state in ('Spawn', 'Stunned', 'Death', 'WaterRoam',
                     'WaterTreasureRoam', 'Cutscene', 'MakeAPirate',
                     'TeleportOut', 'TeleportIn', 'ParlorGame', 'NPCInteract',
                     'ShipBoarding', 'Ensnared', 'Thrown', 'Knockdown',
                     'Unconcious', 'ThrownInJail', 'DoorKicking', 'EnterTunnel',
                     'LeaveTunnel', 'PVPWait'):
            return
        if self.isMouseMode:
            return
        if not weaponId:
            localAvatar.guiMgr.createWarning(PLocalizer.NothingEquippedWarning,
    ...
    def setEquippedWeapons(self, weaponIds):
        if weaponIds == [0, 0, 0, 0, 0, 0]:
            weaponIds = [InventoryType.MeleeWeaponL1]
    ...
    def setCurrentWeapon(self, currentWeaponId, isWeaponDrawn):
        ...
        weaponButton = WeaponButton(
            parent=self,
            scale=1.1,
            command=self.toggleWeapon,
            showQuant=showQuant)
        weaponButton.setWeaponId(currentWeaponId)
        weaponButton['extraArgs'] = [currentWeaponId]
        ...
        if currentWeaponId and isWeaponDrawn:
            if not Freebooter.getPaidStatus(base.localAvatar.getDoId()):
                if Freebooter.allowedFreebooterWeapon(self.rep):
                    if EconomyGlobals.getItemCategory(
                            currentWeaponId) == ItemType.WEAPON:
                        self.skillTray.updateSkillTray(repId, self.weaponMode,
                                                       self.__skillTrayCallback)
            else:
                self.skillTray.updateSkillTray(repId, self.weaponMode,
                                               self.__skillTrayCallback)
    ...
    def initCombatTray(self, rep=None):
        ...
        self.accept('inventoryChanged-%s' % localAvatar.getInventoryId(),
                    self.refreshList)
        self.weaponMode = WeaponGlobals.getWeaponCategory(
        ...
        if len(rightSkills):
            ...
            lastAmmo = self.lastAmmoSkillId.get(localAvatar.currentWeaponId,
                                                rightSkills[0])
    ...
    def trySkill(self, skillId, ammoSkillId, combo=0, charge=0):
        ...
        if ammoSkillId in (InventoryType.Potion1, InventoryType.Potion2,
                           InventoryType.Potion3, InventoryType.Potion4,
                           InventoryType.Potion5):
            if localAvatar.getHp() == localAvatar.getMaxHp(
            ) and localAvatar.getMojo() == localAvatar.getMaxMojo():
                localAvatar.guiMgr.createWarning(PLocalizer.FullHealthWarning,
                                                 PiratesGuiGlobals.TextFG6)
                return 0

        if ammoSkillId in (InventoryType.ShipRepairKit,) and hasattr(
                localAvatar, 'ship') and localAvatar.ship:
            if localAvatar.ship.getHp() == localAvatar.ship.getMaxHp(
            ) and localAvatar.ship.getSp() == localAvatar.ship.getMaxSp():
                localAvatar.guiMgr.createWarning(
                    PLocalizer.FullShipHealthWarning, PiratesGuiGlobals.TextFG6)
                return 0

        if WeaponGlobals.getIsShipSkill(
                skillId
        ) and not WeaponGlobals.getIsShout(
                skillId
        ) and not skillId == InventoryType.SailBroadsideLeft and not skillId == InventoryType.SailBroadsideRight:
            if localAvatar.ship:
                if not localAvatar.ship.sailsReady:
                    localAvatar.guiMgr.createWarning(
                        PLocalizer.SailsNotReadyWarning,
                        PiratesGuiGlobals.TextFG6)
                    return 0
            else:
                return 0
        ...
        if WeaponGlobals.getIsShipSkill(skillId):
            if localAvatar.ship:
                if skillId == InventoryType.SailRammingSpeed and localAvatar.ship.queryGameState(
                ) in 'InBoardingPosition':
                    localAvatar.guiMgr.createWarning(
                        PLocalizer.RamWhileBoardingWarning,
                        PiratesGuiGlobals.TextFG6)
                    return 0
                if localAvatar.ship.isRamming(
                ) and skillId != InventoryType.SailBroadsideLeft and skillId != InventoryType.SailBroadsideRight:
                    return 0

        if skillId not in self.IGNORES_INPUT_LOCK and not WeaponGlobals.getIsShipSkill(
                skillId):
            ...
            weaponVolley = WeaponGlobals.getWeaponVolley(
                localAvatar.currentWeaponId)
            if weaponVolley > 0 and self.volley >= weaponVolley:
                return 0

            skillRepCategory = WeaponGlobals.getSkillReputationCategoryId(
                skillId)
            if self.rep != skillRepCategory and skillRepCategory > 0:
                return 0

            if ammoSkillId:
                ammoRepCategory = WeaponGlobals.getSkillReputationCategoryId(
                    ammoSkillId)
        ...
        if WeaponGlobals.C_STUN in skillEffects:
            localAvatar.guiMgr.createWarning(PLocalizer.StunWarning,
                                             PiratesGuiGlobals.TextFG6)
            return 0

        if not WeaponGlobals.getIsInstantSkill(skillId, ammoSkillId):
            if self.isUsingSkill:
                if (self.weaponMode == WeaponGlobals.COMBAT or
                        self.weaponMode == WeaponGlobals.THROWING
                   ) and skillId != EnemySkills.DOLL_UNATTUNE:
        ...
        if WeaponGlobals.getNeedSight(skillId, ammoSkillId):
            ...
            if self.aimAssistTarget:
                ...
                target = base.cr.targetMgr.takeAim(localAvatar, skillId,
                                                   ammoSkillId)
                if target:
                    inView = localAvatar.checkViewingArc(target)
            if not inView and (target or self.aimAssistTarget):
                localAvatar.guiMgr.createWarning(PLocalizer.OutOfSightWarning,
                                                 PiratesGuiGlobals.TextFG6)
                return 0
        ...
        if rechargingTime > 0:
            if skillId == InventoryType.UseItem:
                if ammoSkillId in InventoryType.Potions:
                    localAvatar.guiMgr.createWarning(
                        PLocalizer.TonicRechargingWarning,
                        PiratesGuiGlobals.TextFG6)
                elif ammoSkillId == InventoryType.ShipRepairKit:
                    localAvatar.guiMgr.createWarning(
                        PLocalizer.RepairKitRechargingWarning,
                        PiratesGuiGlobals.TextFG6)
            else:
                if skillId not in self.BASIC_ATTACKS:
                    localAvatar.guiMgr.createWarning(
                        PLocalizer.SkillRechargingWarning,
                        PiratesGuiGlobals.TextFG6)
            return 0
        if ammoSkillId:
            if not WeaponGlobals.isInfiniteAmmo(ammoSkillId):
                ...
                if amt < 1:
                    if EconomyGlobals.getItemCategory(
                            ammoSkillId) == ItemType.AMMO:
                        localAvatar.guiMgr.createWarning(
                            PLocalizer.OutOfAmmoWarning,
                            PiratesGuiGlobals.TextFG6)
                    else:
                        localAvatar.guiMgr.createWarning(
                            PLocalizer.OutOfItemWarning,
                            PiratesGuiGlobals.TextFG6)
                    return 0
        if skillId:
        ...
        if localAvatar.mojo < cost and cost:
            localAvatar.guiMgr.createWarning(PLocalizer.NoManaWarning,
                                             PiratesGuiGlobals.TextFG6)
            return 0
        if localAvatar.isAirborne():
            if not WeaponGlobals.getUsableInAir(skillId, ammoSkillId):
                localAvatar.guiMgr.createWarning(
                    PLocalizer.NotUsableInAirWarning, PiratesGuiGlobals.TextFG6)
                return 0
        if self.weaponMode == WeaponGlobals.STAFF and WeaponGlobals.getAttackMaxCharge(
                skillId):
            if self.chargeTime < self.maxCharge:
                localAvatar.guiMgr.createWarning(PLocalizer.SpellFailedWarning,
                                                 PiratesGuiGlobals.TextFG6)
                return 0
        if skillId == InventoryType.SailBroadsideLeft or skillId == InventoryType.SailBroadsideRight:
            if localAvatar.ship and not localAvatar.ship.broadside:
                localAvatar.guiMgr.createWarning(PLocalizer.NoBroadsidesWarning,
                                                 PiratesGuiGlobals.TextFG6)
                return 0
        ...
        if combo == -1:
            ...
            if prevSkillId in self.L1_COMBO_ATTACKS and skillId == prevSkillId and time <= self.BUTTON_MASH_WINDOW:
                ...
                self.activeName['text'] = PLocalizer.InventoryTypeNames[
                    visSkillId]
        if self.skillFrame:
            ...
            for target in allTargets:
                ...
                for buffId in target.skillEffects:
                    ...
                    if newPriority < priority and category == newCategory:
                        localAvatar.guiMgr.createWarning(
                            PLocalizer.BuffPriorityWarning,
                            PiratesGuiGlobals.TextFG6)
                        return 0
        ...
        ) and skillId != InventoryType.DollAttune:
        ...
        if target:
            ...
            attackRange = base.cr.battleMgr.getModifiedAttackRange(
                localAvatar, skillId, ammoSkillId)
            if skillId not in self.NO_PRINT_RANGE:
                if distance > attackRange * 1.333:
                    localAvatar.guiMgr.createWarning(
                        PLocalizer.OutOfRangeWarning, PiratesGuiGlobals.TextFG6)
            if target != localAvatar.currentTarget:
                if distance <= attackRange * 1.333:
                    target.requestInteraction(
                        localAvatar.doId,
                        interactType=PiratesGlobals.INTERACT_TYPE_HOSTILE)
            if skillId == InventoryType.DollAttune:
                if distance <= attackRange:
                    pass
                else:
                    localAvatar.guiMgr.createWarning(
                        PLocalizer.TooFarAttuneWarning,
                        PiratesGuiGlobals.TextFG6)
                    return 0
            backstab = 0
        ...
        if ammoSkillId in InventoryType.Potions:
            ...
            if ammoSkillId in localAvatar.guiMgr.weaponPage.tonicButtons:
                localAvatar.guiMgr.weaponPage.tonicButtons[
                    ammoSkillId].updateQuantity(amt - 1)
                for skillButton in localAvatar.guiMgr.weaponPage.tonicButtons:
                    if localAvatar.guiMgr.weaponPage.tonicButtons[skillButton].quantity > 0:
                        localAvatar.guiMgr.weaponPage.tonicButtons[
                            skillButton].startRecharge()
                    else:
                        localAvatar.guiMgr.weaponPage.tonicButtons[
                            skillButton].checkAmount()
        ...
        if skillId == InventoryType.UseItem:
            ...
            if WeaponGlobals.isProjectileSkill(skillId, ammoSkillId):
                ...
                if WeaponGlobals.getIsShipSkill(
                        skillId) and not WeaponGlobals.getIsShout(skillId):
                    messenger.send(WeaponGlobals.LocalAvatarUseShipSkill,
                                   [skillId, ammoSkillId])
                else:
                    if skillId in InventoryType.BackstabSkills:
                        messenger.send(
                            WeaponGlobals.LocalAvatarUseTargetedSkill,
        ...
        if skillId:
            if self.weaponMode in (WeaponGlobals.COMBAT, WeaponGlobals.MELEE,
                                   WeaponGlobals.VOODOO, WeaponGlobals.THROWING,
                                   WeaponGlobals.SAILING):
                skillTrack = WeaponGlobals.getSkillTrack(skillId)
                if skillTrack == WeaponGlobals.RADIAL_SKILL_INDEX:
                    rechargeT = base.cr.battleMgr.getModifiedRechargeTime(
                        localAvatar, skillId)
                    taskMgr.doMethodLater(
                        rechargeT,
                        self.skillRechargeAlert,
                        'skillRechargePopup-' + str(skillId),
                        extraArgs=[skillId])
        ...
        if skillId not in (EnemySkills.PISTOL_CHARGE,
                           EnemySkills.STAFF_WITHER_CHARGE,
                           EnemySkills.STAFF_SOULFLAY_CHARGE,
                           EnemySkills.STAFF_PESTILENCE_CHARGE,
                           EnemySkills.STAFF_HELLFIRE_CHARGE,
                           ...
                           EnemySkills.STAFF_DESOLATION_CHARGE,
                           EnemySkills.GRENADE_CHARGE
                          ) and skillId not in InventoryType.BackstabSkills:
            self.chargeTime = 0
    ...
    def __startSkillTimers(self):
        ...
        taskMgr.remove('comboReadyTimer')
        taskMgr.remove('skillFinishedTimer')
        taskMgr.doMethodLater(self.WINDOW_LENGTH * duration,
                              self.__comboReadyTimer, 'comboReadyTimer')
        taskMgr.doMethodLater(
            duration,
            self.__skillFinishedTimer,
            'skillFinishedTimer',
            priority=40)
    ...
    def __skillFinishedTimer(self, task):
        ...
        if not self.invReq:
            DistributedInventoryBase.getInventory(localAvatar.getInventoryId(),
                                                  invArrived)
        ...
        if skillId > 0:
            ...
            if self.weaponMode == WeaponGlobals.STAFF:
                if self.chargeTime >= self.CHARGE_MODE_TIME_THRESHOLD:
                    localAvatar.guiMgr.createWarning(
                        PLocalizer.AmmoChargingWarning,
                        PiratesGuiGlobals.TextFG6)
                    return None
                        ...
                        for i in range(len(self.skillTray.traySkillMap)):
                            if self.skillTray.traySkillMap[i] == skillId:
                                if self.skillTray.origMap[i][1]:
                                    self.skillTray.tray[i +
                                                        1].toggleButton(True)

                            if self.skillTray.origMap[i][1]:
                                self.skillTray.tray[i + 1].toggleButton(False)

            if self.weaponMode in (WeaponGlobals.FIREARM, WeaponGlobals.STAFF,
                                   WeaponGlobals.CANNON, WeaponGlobals.GRENADE):
                    ...
                    if not maxQuant == WeaponGlobals.INF_QUANT:
                        ...
                        if ammoAmt > 0:
                            ...
                            if self.skillTray.traySkillMap:
                                if skillId in self.skillTray.traySkillMap:
                                    for i in range(
                                            len(self.skillTray.traySkillMap)):
                                        if self.skillTray.traySkillMap[
                                                i] == skillId:
                                            if self.skillTray.origMap[i][1]:
                                                self.skillTray.tray[
                                                    i + 1].toggleButton(True)

                                        if self.skillTray.origMap[i][1]:
                                            self.skillTray.tray[
                                                i + 1].toggleButton(False)
                        else:
                            localAvatar.guiMgr.createWarning(
                                PLocalizer.OutOfAmmoWarning,
                                PiratesGuiGlobals.TextFG6)
                        ...
                        if self.skillTray.traySkillMap:
                            if skillId in self.skillTray.traySkillMap:
                                for i in range(
                                    ...
                                    if self.skillTray.origMap[i][1]:
                                        self.skillTray.tray[i + 1].toggleButton(
                                            False)
                else:
                    ammoChoice = skillId
                    self.setAmmoSkillId(ammoChoice)
            elif self.weaponMode in (WeaponGlobals.COMBAT,
                                     WeaponGlobals.SAILING,
                ...
                if localAvatar.hasStickyTargets():
                    ...
                    localAvatar.guiMgr.createWarning(
                        PLocalizer.NotAttunedWarning, PiratesGuiGlobals.TextFG6)
            else:
                attackSelected = None
            if attackSelected:
    ...
    def isPVP(self):
        if isinstance(base.cr.activeWorld,
                      DistributedPVPInstance.DistributedPVPInstance):
            return 1
        else:
            return 0
    ...
    def beginButtonCharge(self):
        ...
        if self.weaponMode == WeaponGlobals.FIREARM:
            self.maxCharge = WeaponGlobals.getAttackMaxCharge(
                InventoryType.PistolTakeAim, self.ammoSkillId)
        else:
            if self.weaponMode == WeaponGlobals.GRENADE:
                self.maxCharge = WeaponGlobals.getAttackMaxCharge(
                    InventoryType.GrenadeLongVolley, self.ammoSkillId)
            else:
                if self.weaponMode == WeaponGlobals.STAFF:
                    self.maxCharge = WeaponGlobals.getAttackMaxCharge(
                        self.ammoSkillId)
                    ...
                    amt = max(
                        0,
                        inv.getStackQuantity(InventoryType.StaffSpiritLore) -
                        1) * WeaponGlobals.getAttackMaxCharge(
    ...
    def buttonCharging(self, task):
            ...
            if self.isUsingSkill and self.weaponMode != WeaponGlobals.VOODOO:
                ...
                if self.weaponMode == WeaponGlobals.STAFF:
                    ...
                    if localAvatar.mojo < cost:
                        self.endButtonCharge()
                        localAvatar.guiMgr.createWarning(
                            PLocalizer.NoManaWarning, PiratesGuiGlobals.TextFG6)
                        return Task.done
                if not self.thresholdHit:
                    if self.weaponMode == WeaponGlobals.FIREARM:
                        ...
                        if self.weaponMode == WeaponGlobals.GRENADE:
                            ...
                            if self.weaponMode == WeaponGlobals.STAFF:
                                skillId = WeaponGlobals.getChargeSkill(
                                    self.ammoSkillId)
        ...
        if self.InstantCast:
            ...
            self.chargeTime = max(
                0,
                min(1 + self.maxCharge + self.CHARGE_MODE_TIME_THRESHOLD,
                    task.time -
                    self.offsetTime)) - self.CHARGE_MODE_TIME_THRESHOLD
        return Task.cont
        ...
        if not inventory:
            ...
            if button == 'action-up':
                ...
                if self.weaponMode == WeaponGlobals.VOODOO:
                    ...
                    if localAvatar.currentTarget:
                        if not base.cr.battleMgr.obeysPirateCode(
                                localAvatar, localAvatar.currentTarget):
                    ...
                    self.trySkill(
                        skillId, self.ammoSkillId, charge=self.chargeTime)
                    return
                elif self.weaponMode == WeaponGlobals.GRENADE:
                    self.trySkill(
                        skillId, self.ammoSkillId, charge=self.chargeTime)
                    ...
                    if self.weaponMode == WeaponGlobals.VOODOO or self.weaponMode == WeaponGlobals.STAFF:
                        ...
                        if self.weaponMode == WeaponGlobals.FIREARM:
                            if inventory.getStackQuantity(
                                    InventoryType.PistolTakeAim) > 1:
                                self.beginButtonCharge()
                            else:
                                if localAvatar.currentTarget:
                                    if not base.cr.battleMgr.obeysPirateCode(
                                            localAvatar,
                                            localAvatar.currentTarget):
                                        if localAvatar.guiMgr.codeShown:
                                            self.tryShoot += 1
                                            if self.tryShoot > 5:
                                                self.tryShoot = 0
                            ...
                            if self.weaponMode == WeaponGlobals.GRENADE:
                                if inventory.getStackQuantity(
                                        InventoryType.GrenadeLongVolley) > 1:
                                    self.beginButtonCharge()
                                else:
                                    skillId = self.skillMapping[button][0]
                                    self.trySkill(skillId, self.ammoSkillId)
                                ...
                                if self.weaponMode == WeaponGlobals.MELEE:
                                    self.trySkill(InventoryType.MeleePunch, 0,
                                                  0)
                                    return
    ...
    def outOfAmmo(self):
        ...
        localAvatar.guiMgr.createWarning(PLocalizer.OutOfAmmoWarning,
                                         PiratesGuiGlobals.TextFG6)

    def beginAimAssist(self, target):
        self.aimAssistTarget = target
        taskMgr.remove('aimAssistTask')
        taskMgr.doMethodLater(AIM_ASSIST_DURATION, self.endAimAssist,
                              'aimAssistTask')
