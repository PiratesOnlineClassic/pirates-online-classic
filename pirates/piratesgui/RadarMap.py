...
class RadarMap(DirectFrame):
    ...
    def startUpdateTask(self):
        self.stopUpdateTask()
        taskMgr.doMethodLater(0.10000000000000001, self.updateTask, 'radarBackgroundTask')
...
class RadarUtil(DirectFrame):
    ...
    def setupControls(self):
        ...
        self.fileNameEntry = DirectEntry(parent = self.ctrlPanel, initialText = 'models/jungles/jungle_a', scale = 0.050000000000000003, width = 25, pos = (-0.98199999999999998, 0, 0.90500000000000003))
        ...
        self.xEntry = DirectEntry(parent = self.ctrlPanel, initialText = `base.camera.getX()`, scale = 0.050000000000000003, width = 3, pos = (-1.1870000000000001, 0, 0.22), command = self.handleXEntryUpdated)
        ...
        self.yEntry = DirectEntry(parent = self.ctrlPanel, initialText = `base.camera.getY()`, scale = 0.050000000000000003, width = 3, pos = (-1.1870000000000001, 0, 0.10000000000000001), command = self.handleYEntryUpdated)
        self.gScaleLabel = DirectLabel(parent = self.resPanel, relief = None, scale = 0.050000000000000003, pos = (1.03, 0, -0.75), text = 'gScale = ' + `int(base.camLens.getFilmSize()[0])`, text_fg = Vec4(0, 0, 0, 1), text_bg = Vec4(1, 1, 1, 0), text_align = TextNode.ALeft, textMayChange = True)
...
if __name__ == '__main__':
    ...
    carrot.setY(0.033329999999999999 / 2)
    m = RadarMap(av, relief = None, pos = (4.0 / 3 - 0.5, 0, 0.5), scale = 1)
    m.setTransparency(1)
    m.setZoomScale(1000)
    center = DirectFrame(parent = m, relief = DGG.FLAT, frameSize = (-0.5, 0.5, -0.5, 0.5), frameColor = (1, 0, 0, 1), scale = 0.025000000000000001)
