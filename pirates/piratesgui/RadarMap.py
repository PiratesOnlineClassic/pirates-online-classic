...
class RadarMap(DirectFrame):
    ...
    def setMap(self):
        ...
        if mapInfo:
            validInfo = [
                info for info in mapInfo
                if info['scale'] / self.zoomScale >= 1.0
                if (pos - Point2(*info['pos'])).length() <= info['scale']
            ]
            if not validInfo:
                info = mapInfo[0]
            info = validInfo[0]
    ...
    def _updateMapPosScale(self, pos, rot, scl):
        ...
        final = uvOrigin.compose(rot).compose(uvOriginInv).compose(pos).compose(
            zScl).compose(uvOriginInv)
...
class RadarUtil(DirectFrame):
    ...
    def mouseDragTask(self, task):
        if base.mouseWatcherNode.hasMouse():
            if task.time == 0.0:
                ...
                task.startCamPos = Point2(base.camera.getX(),
                                          base.camera.getY())
    ...
    def setupControls(self):
        ...
        self.fileNameEntry = DirectEntry(
        ...
        self.xEntry = DirectEntry(
            parent=self.ctrlPanel,
            initialText=repr( (base.camera.getX())),
            scale=0.05,
            width=3,
            pos=(-1.187, 0, 0.22),
        ...
        self.yEntry = DirectEntry(
            parent=self.ctrlPanel,
            initialText=repr( (base.camera.getY())),
            scale=0.05,
            width=3,
            pos=(-1.187, 0, 0.1),
        ...
        self.gScaleLabel = DirectLabel(
            ...
            scale=0.05,
            pos=(1.03, 0, -0.75),
            text='gScale = ' + repr( (int(base.camLens.getFilmSize()[0]))),
...
if __name__ == '__main__':
    ...
    def updateMap(t):
        ...
        par = Sequence(
            Func(av.setPos, render, Vec3(-250, 80, 0)),
            Parallel(
                av.hprInterval(30, Vec3(360, 0, 0), Vec3(0, 0, 0)),
                LerpFunc(updateMap, 30)))
        par.loop()
