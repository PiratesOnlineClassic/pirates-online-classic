...
class ScoreboardItemGui(DirectFrame):
    ...
    def __init__(self, item, width, height, parent=None, **kw):
        self.width = width
        self.height = height
        optiondefs = (('state', DGG.NORMAL, None), ('frameColor', (0.1, 0.1, 1,
                                                                   0.08), None),
                      ('borderWidth', PiratesGuiGlobals.BorderWidth,
                       None), ('frameSize', (0.0, self.width, 0.0, self.height),
        ...
        self.icons = {
            ...
            ItemId.CARGO_SKCHEST: ('topgui_icon_ship_chest03',
                                   PLocalizer.SkChest),
            ItemId.WHEAT: ('icon_wheat',
                           PLocalizer.InventoryTypeNames.get(ItemId.WHEAT)),
            ItemId.COTTON: ('icon_cotton',
                            PLocalizer.InventoryTypeNames.get(ItemId.COTTON)),
            ItemId.RUM: ('icon_rum',
                         PLocalizer.InventoryTypeNames.get(ItemId.RUM)),
            ItemId.IRON_ORE: ('icon_ironore',
                              PLocalizer.InventoryTypeNames.get(
                                  ItemId.IRON_ORE)),
            ItemId.IVORY: ('icon_ivory',
                           PLocalizer.InventoryTypeNames.get(ItemId.IVORY)),
            ItemId.SILK: ('icon_silk',
                          PLocalizer.InventoryTypeNames.get(ItemId.SILK)),
            ItemId.SPICES: ('icon_spices',
                            PLocalizer.InventoryTypeNames.get(ItemId.SPICES)),
            ItemId.COPPER_BARS:
            ('icon_copperbars',
             PLocalizer.InventoryTypeNames.get(ItemId.COPPER_BARS)),
            ItemId.SILVER_BARS: ('icon_silverbars',
                                 PLocalizer.InventoryTypeNames.get(
                                     ItemId.SILVER_BARS)),
            ItemId.GOLD_BARS: ('icon_goldbars',
                               PLocalizer.InventoryTypeNames.get(
                                   ItemId.GOLD_BARS)),
            ItemId.EMERALDS: ('icon_emeralds',
                              PLocalizer.InventoryTypeNames.get(
                                  ItemId.EMERALDS)),
            ItemId.RUBIES: ('icon_rubies',
                            PLocalizer.InventoryTypeNames.get(ItemId.RUBIES)),
            ItemId.DIAMONDS: ('icon_diamonds',
                              PLocalizer.InventoryTypeNames.get(
                                  ItemId.DIAMONDS)),
            ItemId.CURSED_COIN: ('icon_cursedcoin',
                                 PLocalizer.InventoryTypeNames.get(
                                     ItemId.CURSED_COIN)),
            ItemId.ARTIFACT: ('icon_artifact',
                              PLocalizer.InventoryTypeNames.get(
                                  ItemId.ARTIFACT)),
            ItemId.RELIC: ('icon_relic',
                           PLocalizer.InventoryTypeNames.get(ItemId.RELIC)),
            ItemId.RARE_DIAMOND: ('icon_rarediamond',
                                  PLocalizer.InventoryTypeNames.get(
                                      ItemId.RARE_DIAMOND)),
            ItemId.CROWN_JEWELS: ('icon_crownjewels',
                                  PLocalizer.InventoryTypeNames.get(
                                      ItemId.CROWN_JEWELS)),
            ItemId.RAREITEM6: ('icon_rareitem6',
                               PLocalizer.InventoryTypeNames.get(
                                   ItemId.RAREITEM6))
        }
        self.item = item
    ...
    def _createIface(self):
        ...
        self.valueText = DirectLabel(
            ...
            text_shadow=PiratesGuiGlobals.TextShadow,
            textMayChange=1,
            pos=(self.width * 0.65, 0, self.height / 2),
            text_font=PiratesGlobals.getInterfaceOutlineFont())
        if self.item.get('Type') == 'Title':
            ...
            if 'Value2' in self.item:
                self.valueText2 = DirectLabel(
                    ...
                    text_shadow=PiratesGuiGlobals.TextShadow,
                    textMayChange=1,
                    pos=(self.width * 0.8, 0, self.height / 2),
                    text_font=PiratesGlobals.getInterfaceOutlineFont())
            if self.valueText2:
                ...
                if self.item.get('Type') == 'Space':
                    ...
                    if self.item.get('Type') == 'Button':
                        ...
                        if self.item.get('Type') == 'Cargo':
                            ...
                            self.descText[
                                'geom_scale'] = 0.09 * self.height * 10
                            ...
                            self.descText[
                                'text_scale'] = 0.05 * self.height * 10
                            ...
                            self.descText2 = DirectLabel(
                                ...
                                text_shadow=PiratesGuiGlobals.TextShadow,
                                textMayChange=1,
                                pos=(0.08, 0, self.height / 2),
                                geom=self.getIcon(icon[0]),
                                geom_scale=0.09 * self.height * 10,
                                geom_pos=(0.1, 0, 0.01),
                                text_pos=(0.1, 0, 0),
                                geom_color=Vec4(0, 0, 0, 1),
                                text_font=PiratesGlobals.
                                getInterfaceOutlineFont())
        return
    ...
    def showSelf(self, args=None):
        self.show()
        if self.item.get('Type') == 'Entry' and isinstance(
                self.item.get('Value1'), int):

            def refreshValue(val):
                self.valueText['text'] = str(int(val))

            self.counter = LerpFunctionInterval(
                refreshValue,
                fromData=0,
                toData=self.item.get('Value1'),
                duration=self.revealTime)
            self.counter.start()
    ...
    def setRevealTimer(self, time, unwrapMode=0):
        ...
        if unwrapMode:
            taskMgr.doMethodLater(
                time, self.reveal,
                'scoreboardItemGuiWait-' + str(self.revealTime))
        else:
            self.hide()
            taskMgr.doMethodLater(
                time, self.showSelf,
