...
class GuildPage(SocialPage.SocialPage):
    ...
    def __init__(self):
        ...
        self.membersList = PirateMemberList.PirateMemberList(5, self.membersFrame, 'FOOLIO HC', height = 0.5475, memberHeight = 0.1, memberWidth = 0.48, memberOffset = 0.055)
        self.membersList.setPos(0.001, 0.0, 0.123)
        self.mainChain = PirateButtonChain.PirateButtonChain(0.49, self.mainFrame, True)
        self.mainChain.setPos(-0.012, 0.0, 0.045)
        self.memberChain = PirateButtonChain.PirateButtonChain(0.49, self.membersFrame, True)
        self.memberChain.setPos(-0.012, 0.0, 0.045)
        self.membersFrame.hide()
        self.tokenChain = PirateButtonChain.PirateButtonChain(0.49, self.tokenFrame, True)
        self.tokenChain.setPos(-0.012, 0.0, 0.045)
        self.tokenFrame.hide()
        self.accept('Guild Status Updated', self.revertGui)
        self.headingLabel = DirectLabel(parent = self, relief = None, text = PLocalizer.GuildPageTitle, state = DGG.NORMAL, text_align = TextNode.ACenter, text_scale = PiratesGuiGlobals.TextScaleLarge, text_pos = (0.0, 0.0), text_fg = PiratesGuiGlobals.TextFG1, pos = (0.23, 0, 0.694))
    ...
    def setupButtons(self):
        if self.guildId:
            ...
            if not self.nameLabel:
                self.nameLabel = DirectLabel(parent = self.mainFrame, relief = None, text = self.guildName, text_fg = (0.95, 1, 1, 1), text_align = TextNode.ACenter, text_shadow = (0, 0, 0, 1), text_scale = 0.06)
            ...
            if not self.rankLabel:
                self.rankLabel = DirectLabel(parent = self.mainFrame, relief = None, text = ranktxt, text_fg = (1, 1, 1, 1), text_align = TextNode.ACenter, text_shadow = (0, 0.0, 0, 1), text_scale = 0.04)
    ...
    def b_clearPermToken(self):
        self.confirmBox = PiratesConfirm.PiratesConfirm(PLocalizer.GuildClearPerm, PLocalizer.GuildMessageClearPermInvite, self.executeClearPermToken)
    ...
    def b_clearLimitedUseToken(self):
        self.confirmBox = PiratesConfirm.PiratesConfirm(PLocalizer.GuildClearLimUse, PLocalizer.GuildMessageClearLimInvite, self.executeClearLimitedUseToken)
    ...
    def renameGuild(self):
        buttonColor = ((0.33, 0.3, 0.26, 1.0), (0.26, 0.24, 0.21, 1.0), (0.49, 0.45, 0.39, 1.0), (0.16, 0.15, 0.13, 1.0))
        if self.nameEntry:
            self.nameLabel.hide()
            self.nameEntry.show()
            ...
            self.nameLabel.hide()
            self.nameEntry = DirectEntry(parent = self.mainFrame, relief = DGG.RAISED, scale = 0.051, pos = (0.24, 0, 0.637), borderWidth = PiratesGuiGlobals.BorderWidthSmall, frameColor = (1, 1, 1, 0.1), text_align = TextNode.ACenter, width = 8, numLines = 1, focus = 1, cursorKeys = 1, text_fg = (1, 1, 1, 1), command = self._typedAName, suppressKeys = 1, suppressMouse = 1, autoCapitalize = 1)
    ...
    def inputTokenForGuild(self):
        ...
        if self.tokenEntry:
            ...
            self.tokenEntry = DirectEntry(parent = self.mainFrame, relief = DGG.RAISED, scale = 0.051, pos = (0.24, 0, 0.637), borderWidth = PiratesGuiGlobals.BorderWidthSmall, frameColor = (1, 1, 1, 0.1), text_align = TextNode.ACenter, width = 8, numLines = 1, focus = 1, cursorKeys = 1, text_fg = (1, 1, 1, 1), command = self._typedAToken, suppressKeys = 1, suppressMouse = 1)
    ...
    def displayTokenOptionsFrame(self):
        ...
        if not self.permTokenLabel:
            ...
            self.permTokenLabel = DirectLabel(parent = self.tokenFrame, relief = None, text = str(label), text_fg = (0.95, 1, 1, 1), text_align = TextNode.ACenter, text_shadow = (0, 0, 0, 1), text_scale = 0.06)
            self.permTokenLabel.setPos(0.23, 0, 0.57)
        
        if not self.permTokenLabelTitle:
            self.permTokenLabelTitle = DirectLabel(parent = self.tokenFrame, relief = None, text = PLocalizer.GuildPermCodeLabel, text_fg = (0.95, 1, 1, 1), text_align = TextNode.ACenter, text_shadow = (0, 0, 0, 1), text_scale = 0.04)
