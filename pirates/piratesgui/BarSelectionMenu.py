...
class BarSelectionMenu(GuiPanel.GuiPanel):
    ...
    def __init__(self, items, command = None):
        ...
        self.cursor = DirectFrame(parent = self, state = DGG.DISABLED, relief = None, frameSize = (0, 0.08, 0, 0.08), pos = (0.08, 0, 0.07), geom = texCard, geom_scale = 0.12)
    ...
    def loadWeaponButtons(self):
        ...
        self.repMeters = []
        self['frameSize'] = (0, self.ICON_WIDTH * len(self.items) + 0.04, 0, self.HEIGHT)
        ...
        for i in range(len(self.items)):
            if self.items[i]:
                ...
                icon = DirectFrame(parent = self, state = DGG.DISABLED, relief = None, frameSize = (0, 0.08, 0, 0.08), pos = (self.ICON_WIDTH * i + 0.08, 0, 0.082))
                icon.setTransparency(1)
                key = WeaponGlobals.getWeaponKey(self.items[i])
                hotkeyText = 'F%s' % key
                hotkey = DirectFrame(parent = icon, state = DGG.DISABLED, relief = None, text = hotkeyText, text_align = TextNode.ACenter, text_scale = 0.045, text_pos = (0, 0), text_fg = PiratesGuiGlobals.TextFG2, text_shadow = PiratesGuiGlobals.TextShadow, image = kbButton, image_scale = 0.06, image_pos = (0, 0, 0.01), image_color = (0.5, 0.5, 0.35, 1), pos = (0, 0, 0.08))
                ...
                if Freebooter.getPaidStatus(base.localAvatar.getDoId()) or Freebooter.allowedFreebooterWeapon(category):
                    ...
                    icon.resetFrameSize()
                    self.icons.append(icon)
                repMeter = DirectWaitBar(parent = icon, relief = DGG.SUNKEN, state = DGG.DISABLED, borderWidth = (0.002, 0.002), range = 0, value = 0, frameColor = (0.24, 0.24, 0.21, 1), barColor = (0.8, 0.8, 0.7, 1), pos = (-0.05, 0, -0.0525), hpr = (0, 0, 0), frameSize = (0.005, 0.095, 0, 0.0125))
