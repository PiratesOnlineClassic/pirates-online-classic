...
class ChatBar(DirectFrame, FSM):
    ...
    def setupGui(self, chatEntry, whiteListEntry):
        ...
        self.speedButton = DirectButton(parent = self, relief = None, frameSize = (-0.055, 0.045, -0.055, 0.045), geom = (skullgui.find('**/*skull'), skullgui.find('**/*skull'), skullgui.find('**/*skull_down')), geom_scale = 0.2, pos = (0.049, 0, 0.045), rolloverSound = None, command = self.chatMgr.activateSpeedChat)
        self.normalChatButton = DirectButton(parent = self, relief = None, frameSize = (-0.055, 0.045, -0.055, 0.045), geom = (icons.find('**/chat_bubble_icon'), icons.find('**/chat_bubble_icon'), icons.find('**/chat_bubble_icon_over')), geom_scale = 0.25, pos = (0.136, 0, 0.045), rolloverSound = None, command = self.chatMgr.toggleWhiteListChat)
        self.normalChatButton.hide()
        self.whiteListButton = DirectButton(parent = self, relief = None, frameSize = (-0.055, 0.045, -0.055, 0.045), geom = (icons.find('**/open_chat_enabled_icon'), icons.find('**/open_chat_enabled_icon'), icons.find('**/open_chat_enabled_icon_over')), geom_scale = 0.25, pos = (0.136, 0, 0.045), rolloverSound = None, command = self.chatMgr.toggleWhiteListChat)
        ...
        self.startChatButton = DirectButton(parent = self, relief = None, image = triangle, image_scale = 0.065, pos = (0.14, 0.0, 0.05), rolloverSound = None, command = self.chatMgr.activateChat)
        ...
        guildTab = self.chatTabs.addTab('Guild', label = PLocalizer.ChatTabGuild, command = self.chatMgr.activateChat, extraArgs = [
            'Guild'])
        shipPVPTab = self.chatTabs.addTab('ShipPVP', label = PLocalizer.ChatTabShipPVP, command = self.chatMgr.activateChat, frameSize = (0, 0.24, 0.0, 0.1), textMayChange = 1, extraArgs = [
            'ShipPVP'])
        self.chatTabs.stash()
        ...
        DirectLabel(parent = whisperNameTab, relief = None, state = DGG.DISABLED, text = PLocalizer.AvatarPanelWhisper + ':', text_scale = PiratesGuiGlobals.TextScaleLarge * 1.1, text_align = TextNode.ALeft, text_fg = PiratesGuiGlobals.TextFG1, text_shadow = PiratesGuiGlobals.TextShadow, text_pos = (0.033, 0.04, 0), text_font = PiratesGlobals.getInterfaceFont())
        DirectLabel(parent = whisperCancelTab, relief = None, state = DGG.DISABLED, text = 'X', text_scale = PiratesGuiGlobals.TextScaleLarge * 1.3, text_align = TextNode.ACenter, text_fg = PiratesGuiGlobals.TextFG1, text_shadow = PiratesGuiGlobals.TextShadow, text_pos = (0.053, 0.043, 0), text_font = PiratesGlobals.getInterfaceFont())
        self.whisperTabs.stash()
        self.request('Hidden')
    ...
    def enterWhisper(self, avatarName = 'John Sharkbait', whisperId = 0):
        ...
        self.whisperNameLabel = DirectLabel(parent = self.whisperTabs.getTab('Name'), relief = None, state = DGG.DISABLED, text = avatarName, text_scale = PiratesGuiGlobals.TextScaleLarge * 1.1, text_align = TextNode.ALeft, text_fg = PiratesGuiGlobals.TextFG2, text_shadow = PiratesGuiGlobals.TextShadow, text_pos = (0.21, 0.04, 0), text_font = PiratesGlobals.getInterfaceFont())
