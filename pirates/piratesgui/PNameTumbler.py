...
class PNameTumbler(NameTumbler.NameTumbler):
    ...
    def loadTumblerGUI(self):
        self.charGui = loader.loadModelCopy('models/gui/char_gui')
        self.triangleGui = loader.loadModelCopy('models/gui/triangle')
    ...
    def makeHighlight(self, pos):
        return DirectFrame(parent = self, relief = DGG.FLAT, frameColor = (1, 1, 1, 0.4), frameSize = (-1.4, 4.2, -2.2, -1.1), borderWidth = (1, 0.5), pos = pos, scale = 1.0)
    
    def makeCheckbox(self, pos, text, command):
        c = DirectCheckButton(parent = self, relief = None, scale = 0.1, boxBorder = 0.08, boxRelief = None, pos = pos, text = text, text_fg = (1, 1, 1, 1), text_scale = 0.8, text_pos = (0.4, 0), indicator_pos = (0, 0, 0), indicator_text_fg = (1, 1, 1, 1), command = command, text_align = TextNode.ALeft)
        return c
