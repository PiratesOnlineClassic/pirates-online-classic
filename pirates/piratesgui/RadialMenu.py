...
Tolerance = 20
FrameSize = 0.12
Center = [(0, 0), (-50, -50), (0, -75), (50, -50), (75, 0), (50, 50), (0, 75),
          (-50, 50), (-75, 0)]
                            ...
                            if skillId == InventoryType.GrenadeRep:
                                ...
                                if skillId == InventoryType.PistolRep:
                                    ...
                                    if skillId == InventoryType.DollRep:
                                        ...
                                        return WeaponGlobals.getSkillIcon(
                                            skillId)
                    ...
                    if repId == InventoryType.DollRep:
                        ...
                        while skillId < InventoryType.end_WeaponSkillDoll:
                            if WeaponGlobals.getSkillTrack(
                                ...
                                if not Freebooter.getPaidStatus(
                                        base.localAvatar.getDoId()):
                                    locked = not WeaponGlobals.canFreeUse(
                                        skillId)
                                if inv.getStackQuantity(skillId) >= minlvl:
                                    skill = (skillId, True, locked)
                                else:
                                            ...
                                            while skillId < InventoryType.end_SkillSailing:
                                                if WeaponGlobals.getSkillTrack(
                                                    ...
                                                    if not Freebooter.getPaidStatus(
                                                        ...
                                                        locked = not WeaponGlobals.canFreeUse(
                                                            skillId)
                                                    shouldBeVisible = inv.getStackQuantity(
                                                        skillId) >= minlvl
                                                    skill = (skillId,
                                                             shouldBeVisible,
                                                             locked)
...
class RadialMenu:
    ...
    def __init__(self, rep, weaponMode):
        ...
        if self.weaponMode in (WeaponGlobals.MELEE, WeaponGlobals.COMBAT,
        ...
        self.rstatus = {}
        self.rstatus[0] = 0
        self.rframe[0] = skillRing = SkillRing(
            Vec4(1, 0.8, 0.5, 1), Vec4(0, 0, 0, 1.0))
        ...
        self.radial[0] = DirectFrame(
            parent=aspect2d,
            relief=None,
            image=self.SkillIcons.find('**/%s' % asset),
            image_scale=ImageScale(self.rep),
        ...
        for i in range(self.numberOfItems):
            ...
            if self.weaponMode in (WeaponGlobals.FIREARM,
                                   WeaponGlobals.THROWING, WeaponGlobals.CANNON,
                ...
                if maxQuant == WeaponGlobals.INF_QUANT:
                    ...
                    amtstr = '%d' % ammoAmt
                self.ammoAmt[i + 1] = DirectLabel(
                    parent=aspect2d,
                    relief=None,
                    text=amtstr,
                    text_align=TextNode.ACenter,
                    text_scale=0.03,
                    text_fg=(0.7, 1.0, 1.0, 1),
        ...
        self.radialHelp = DirectLabel(
            parent=aspect2d,
            relief=None,
            text='',
            text_align=TextNode.ACenter,
            text_scale=0.04,
            text_fg=(0.95, 1.0, 1.0, 1),
        ...
        self.hiLiteItem = 0
        taskMgr.add(
            self.radialMenuHeartBeat, 'radialMenuHeartBeat', priority=40)
    ...
    def radialMenuHeartBeat(self, task):
        if base.mouseWatcherNode.hasMouse():
            ...
            curX = 0
            curY = 0
        hLItem = RingMatch(self.numberOfItems, curX - self.mouseBasePosX,
                           curY - self.mouseBasePosY)
        if hLItem != self.hiLiteItem:
            if hLItem < 1:
                self.radialHelp.hide()
            else:
                self.radialHelp['text'] = PLocalizer.InventoryTypeNames[
                    self.radialSkillMap[hLItem - 1]]
                basePos = self.radial[hLItem].getPos()
                self.radialHelp.setPos(basePos[0] + 0.06, basePos[1],
                                       basePos[2] - 0.04)
        ...
        for skillId in self.radialSkillMap:
            ...
            if self.rep == InventoryType.DollRep:
                ...
                if haveFriendly:
                    if not haveHostile:
                        if not WeaponGlobals.isFriendlyFire(
                                self.radialSkillMap[count - 1]):
                            greyOut = 1
                    elif not haveFriendly and haveHostile:
                        if WeaponGlobals.isFriendlyFire(
                                self.radialSkillMap[count - 1]):
                            greyOut = 1
                if self.rstatus[count] != greyOut:
                    ...
                    if greyOut == 2:
                        ...
                        self.rframe[count].meterFaceHalf1.setColorScale(
                            0.4, 0.4, 0.4, 1.0)
                        self.rframe[count].meterFaceHalf2.setColorScale(
                            0.4, 0.4, 0.4, 1.0)
                    elif greyOut == 3:
                        self.radial[count].setColorScale(0.4, 0.4, 0.4, 1.0)
                    else:
...
class SkillTray:
    ...
    def __init__(self):
        ...
        self.repMeter = None
        self.skillRechargedSound = base.loader.loadSfx(
            'audio/sword-swoosh2.mp3')
        self.skillTray = DirectFrame(
        ...
        self.showSkillTrayIval = Sequence(
            Func(self.show),
            LerpFunc(self.skillTray.setZ, 0.25, skillTrayZ,
                     moveUp + skillTrayZ))
        self.hideSkillTrayIval = Sequence(
            LerpFunc(self.skillTray.setZ, 0.25, moveUp + skillTrayZ,
                     skillTrayZ), Func(self.hide))
    ...
    def updateSkillTray(self, rep, weaponMode, callback=None, hideFirst=True):
        ...
        if self.skillTrayState and hideFirst:
            ...
            taskMgr.doMethodLater(
                ...
                'updateSkillTray',
                extraArgs=[rep, weaponMode, callback])
            return
            ...
            if self.origMap[i][1] == False:
                ...
                if locked:
                    image = (self.SkillIcons.find('**/base'),
                             self.SkillIcons.find('**/base_down'),
                             self.SkillIcons.find('**/base_over'))
                else:
                    image = self.SkillIcons.find('**/base')
                button = DirectButton(
                    parent=self.skillTray,
                    relief=None,
                    state=DGG.DISABLED,
                    image=image,
                    image_pos=(0.0, 0.0, 0.06),
                    image_scale=0.12,
                    image_color=(0.2, 0.2, 0.2, 0.55),
                    sortOrder=100,
                    pos=(x, 0, -0.04))
                ...
                self.tray[i + 1] = button
                if weaponMode in (WeaponGlobals.FIREARM, WeaponGlobals.GRENADE,
                                  WeaponGlobals.STAFF):
                    lastAmmo = localAvatar.guiMgr.combatTray.lastAmmoSkillId.get(
                    ...
                    if hasattr(self.tray[i + 1], 'skillButton'):
                        ...
                        if greyOut == 1:
                            ...
                            if self.tray[i + 1].showRing:
                                self.tray[
                                    i +
                                    1].skillRing.meterFaceHalf1.setColorScale(
                                        0.4, 0.4, 0.4, 1.0)
                                self.tray[
                                    i +
                                    1].skillRing.meterFaceHalf2.setColorScale(
                                        0.4, 0.4, 0.4, 1.0)
                        elif greyOut == 2:
                            self.tray[i + 1].setGeomColor(0.5, 0.5, 0.5, 1.0)
                        elif greyOut == 3:
                            ...
                            if self.tray[i + 1].showRing:
                                self.tray[
                                    i +
                                    1].skillRing.meterFaceHalf1.clearColorScale(
                                    )
                                self.tray[
                                    i +
                                    1].skillRing.meterFaceHalf2.clearColorScale(
                                    )
    ...
    def decrementSkillTrayAmount(self, skillId, amt=1):
        if not self.traySkillMap:
            return
        if self.weaponMode not in (WeaponGlobals.FIREARM,
                                   WeaponGlobals.THROWING, WeaponGlobals.CANNON,
                                   WeaponGlobals.GRENADE):
            return
        if not hasattr(base, 'localAvatar'):
    ...
    def updateSkillIval(self, skillId):
        for button in self.tray:
            if isinstance(self.tray[button], SkillButton):
                ...
                ) and self.tray[button].skillId == skillId:
                    self.tray[button].skillRingIval.isPlaying(
                    ) or self.tray[button].skillRingIval.start()

    def addPowerRechargeEffect(self):
        for button in self.tray:
            if isinstance(self.tray[button], SkillButton):
                if not self.tray[button].isEmpty() and (
                        WeaponGlobals.getIsShipSkill(self.tray[button].skillId)
                        or WeaponGlobals.getIsCannonSkill(
                            self.tray[button].skillId)
                ) and self.tray[button].skillId != InventoryType.SailPowerRecharge:
...
# okay decompiling .\pirates\piratesgui\RadialMenu.pyc
