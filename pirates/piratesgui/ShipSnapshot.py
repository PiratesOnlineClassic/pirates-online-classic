...
class ShipSnapshot(DirectFrame):
    ...
    PrivateerRepairOnLaunch = base.config.GetBool('privateer-repair-on-launch',
                                                  0)
    ...
    def __init__(self,
        ...
        optiondefs = (('relief', None, None), ('shipOV', shipOV,
                                               None), ('siegeTeam', siegeTeam,
                                                       None), ('shipName',
                                                               shipName, None),
                      ('shipClass', shipClass,
                       None), ('mastInfo', mastInfo,
                               None), ('hp', hp, None), ('maxHp', maxHp, None),
                      ('sp', sp, None), ('maxSp', maxSp, None), ('cargo', cargo,
                                                                 None),
                      ('maxCargo', maxCargo,
                       None), ('crew', crew,
                               None), ('crewNames', [],
                                       self.setCrewNames), ('maxCrew', maxCrew,
                                                            None), ('time',
                                                                    time, None))
    ...
    def loadGUI(self):
        ...
        self.cargoMeter = GuiButton.GuiButton(
            ...
            state=DGG.DISABLED,
            relief=None,
            helpText=PLocalizer.CargoIconHelp2,
            helpPos=(0.1, 0, 0.15),
            image=cargoTex,
            image_scale=0.32,
            text='%d/%d' % (cargo, maxCargo),
    ...
    def loadShipIcon(self):
        self.shipIcon = ShipMeter.ShipMeter(0, self['shipClass'],
                                            self['mastInfo'])
        ...
        if hasattr(self, 'crewMeter'):
            if self['crewNames']:
                ...
                self.crewMeter['helpText'] = '%s\n-\n%s:\n%s' % (
                    PLocalizer.CrewIconHelp, PLocalizer.KnownCrew,
                    ('\n').join(self['crewNames']))
            else:
