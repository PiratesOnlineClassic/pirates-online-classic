...
class StatusTray(GuiTray.GuiTray):
    ...
    def __init__(self, parent, showSkills=0, **kw):
        ...
        if showSkills:
            ...
            self.reloadFrame = DirectFrame(
                parent=self, state=DGG.DISABLED, relief=None)
    ...
    def show(self):
        if not self.doId:
            return
        if base.cr.doId2do[self.
                           doId].state != 'Spawn' and base.cr.doId2do[self.
                                                                      doId].state != 'Death' and base.cr.doId2do[self.
                                                                                                                 doId].state != 'Waiting':
    ...
    def updateName(self, name, level, doId):
        ...
        if color:
            text = '%s  \x01smallCaps\x01%s%s%s\x02\x02' % (
                name, color, PLocalizer.Lv, self.level)
        else:
            text = '%s' % self.name
    ...
    def updateHp(self, hp, maxHp, srcDoId=None):
        ...
        if self.prevValue > hp:
            ...
            self.hpMeterChange.setPos(frameX + float(self.meterChangeOffset[0]),
                                      0.0, float(self.meterChangeOffset[2]))
            self.hpMeterChange['frameSize'] = (0.0, frameRight, frameBottom,
                                               frameTop)
        ...
        if not self.hideValues:
            if not self.doId:
                ...
                if not vtLevel:
                    ...
                    self.voodooMeter[
                        'text'] = '%s\x01Bred\x01/%s\x02' % (voodoo, modVoodoo)
            else:
                self.voodooMeter['text'] = '%s/%s' % (voodoo, maxVoodoo)
        return
    ...
    def hideSkill(self, args=None):
        fadeOut = LerpFunctionInterval(
            self.reloadFrame.setAlphaScale,
