...
class OptionItemGui(DirectFrame):
    ...
    def createNewItem(self, item, parent, itemType = None, columnWidths = [], color = None):
        newItem = ButtonListItem(item, 0.08, 0.38, parent, parentList = self, txtColor = color, pressEffect = False, image = GuiButton.GuiButton.genericButton, frameColor = (0, 0, 0, 0), textScale = 0.05)
        newItem.setup()
        return newItem
    ...
    def createFrame(self):
        self.removeFrame()
        self.borderFrame = BorderFrame(parent = self, pos = (0.5, 0, 0.15), scale = (0.57, 1, 0.33))
