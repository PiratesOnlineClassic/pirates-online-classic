...
class MusicManager(AmbientManagerBase.AmbientManagerBase):
    ...
    def load(self, name, looping=True):
        if name in musicDict:
            path = musicDict[name]
            retval = AmbientManagerBase.AmbientManagerBase.load(
                self, name, path, isMusic=True, looping=looping)
        ...
        if self.current == self.playlist[0]:
            ...
            if self.current != None:
                if self.ambientDict[self.current.name].finalVolume > 0:
                    self.notify.debug(
                        'calling requestFadeOut on %s' % self.current.name)
                    self.requestFadeOut(
                        self.current.name, removeFromPlaylist=False)
        ...
        if self.wantMusic:
            ...
            if self.current.looping == False:
                if songLength is not None:
                    self.doMethodLater(songLength,
                                       self.handleCurrentTrackFinished,
                                       'currentTrackFinished')
                else:
                    self.notify.warning('non-looping song %s has no length!' %
                                        self.current.name)

            self.requestFadeIn(
                self.current.name, finalVolume=self.current.volume)

    def requestFadeOut(self,
                       name,
                       duration=3,
                       finalVolume=0.0,
                       priority=0,
    ...
    def requestChangeVolume(self,
                            name,
                            duration,
                            finalVolume,
                            priority=0,
                            removeFromPlayList=False):
        AmbientManagerBase.AmbientManagerBase.requestChangeVolume(
            self, name, duration, finalVolume, priority)
        if finalVolume == 0:
    ...
    def requestCurMusicFadeOut(self,
                               duration=3,
                               finalVolume=0.0,
                               removeFromPlaylist=False):
