...
class RangeSlider(DirectFrame):
    ...
    def setup(self, label, range, value, orientation, *args, **kwargs):
        ...
        self.min = DirectEntry(
            parent=self,
            initialText=repr( (float(self.slider['range'][0]))),
            scale=0.05,
            width=width,
            pos=pos)
        updateField(
            self.min, 'command',
            lambda x: updateField(self.slider, 'range', (float(x), self.slider['range'][1]))
        )
        if orientation == DGG.HORIZONTAL:
            pos = (0.275, 0, -0.02)
        else:
            pos = (-0.025 * width, 0, 0.3)
        self.max = DirectEntry(
            parent=self,
            initialText=repr( (float(self.slider['range'][1]))),
            scale=0.05,
            width=width,
            pos=pos)
        ...
        self.label = DirectLabel(
            ...
            text=label,
            text_scale=0.05,
            text_pos=(0.03 - 0.395, 0.35 - 0.24, 0),
...
class MapConfig(DirectFrame):
    ...
    def setup(self):
        ...
        self.worldPSlider = RangeSlider(
            ...
            pos=(0.395, 0, 0.24))
        self.worldDecorScaleSlider = RangeSlider(
            guiId='worldP',
        ...
        self.finalSlider = RangeSlider(
            ...
            parent=self.worldFrame,
            pos=(0.395, 0, -0.48))
        self.saveState0Button = DirectButton(
            guiId='save0Button',
            parent=self.mainFrame,
            scale=0.1,
            pos=(0.2, 0, -0.65),
            borderWidth=(0.1, 0.1),
            text='save pt0')
        self.saveState1Button = DirectButton(
            guiId='save1Button',
            parent=self.mainFrame,
            scale=0.1,
            pos=(0.6, 0, -0.65),
