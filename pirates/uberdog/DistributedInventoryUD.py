...
class DistributedInventoryUD(DistributedObjectUD):
    notify = DirectNotifyGlobal.directNotify.newCategory(
        'DistributedInventoryUD')

    def __init__(self, air):
        DistributedObjectUD.__init__(self, air)

    def generate(self):
        DistributedObjectUD.generate(self)

    def announceGenerate(self):
        DistributedObjectUD.announceGenerate(self)

        self.air.netMessenger.accept('getAccumulatorsResponse', self,
                                     self.proccessCallbackResponse)
        self.air.netMessenger.accept('getAccumulatorResponse', self,
                                     self.proccessCallbackResponse)
        self.air.netMessenger.accept('getStackLimitResponse', self,
                                     self.proccessCallbackResponse)
        self.air.netMessenger.accept('getStackResponse', self,
                                     self.proccessCallbackResponse)
        self.air.netMessenger.accept('getOwnerIdResponse', self,
                                     self.proccessCallbackResponse)
    ...
    def b_setAccumulator(self, accumulatorType, quantity):
        self.air.netMessenger.send('b_setAccumulator',
    ...
    def getAccumulator(self, accumulatorType, callback):
        self.air.netMessenger.send('getAccumulator',
                                   [accumulatorType, callback])
    ...
    def proccessCallbackResponse(self, callback, *args, **kwargs):
        if callback and callable(callback):
            ...
            return
        self.notify.warning(
            "No valid callback for a callback response! What was the purpose of that?"
        )
