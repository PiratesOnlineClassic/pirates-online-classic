...
class SeaSerpent(SeaMonster):
    ...
    sfx = {}
    AnimList = (('idle', 'idle'), ('swim', 'swim'), ('walk', 'swim'),
                ('submerge', 'submerge'), ('attack', 'attack'),
                ('emerge', 'emerge'), ('death', 'submerge'))
    ...
    def __init__(self):
        ...
        if not SeaSerpent.sfx:
            for name in SeaSerpent.SfxNames:
                SeaSerpent.sfx[name] = loader.loadSfx('audio/' +
                                                      SeaSerpent.SfxNames[name])
