...
class Seagull(Animal):
    ...
    SfxNames.update({})
    AnimList = (('idle', 'flying'), ('walk', 'flying'), ('run', 'flying'),
                ('flying', 'flying'), ('takeoff', 'takeoff'),
                ('landing', 'landing'), ('groom_idle', 'groom_idle'))

    class AnimationMixer(Animal.AnimationMixer):

        notify = DirectNotifyGlobal.directNotify.newCategory(
            'SeagullAnimationMixer')
    ...
    def setupAnimInfo(cls):
        cls.setupAnimInfoState('LandRoam',
                               (('idle', 1.0), ('walk', 1.0), ('run', 1.0),
                                ...
                                ('run', 1.0), ('groom_idle', 1.0), ('run', 1.0),
                                ('run', 1.0), ('fly', 1.0), ('fly', 1.0)))
    ...
    def __init__(self):
        ...
        if not Seagull.sfx:
            for name in Seagull.SfxNames:
                Seagull.sfx[name] = loader.loadSfx('audio/' +
                                                   Seagull.SfxNames[name])
