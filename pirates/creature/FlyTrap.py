...
class FlyTrap(Creature):
    ...
    sfx = {}
    AnimList = (('idle', 'idle'), ('attack_a', 'attack_a'), ('attack_jab',
                                                             'attack_jab'),
                ('attack_left_fake', 'attack_left_fake'), ('attack_right_fake',
                                                           'attack_right_fake'),
                ('intro', 'rise_from_ground'), ('shoot', 'spit'),
                ('pain', 'hit'), ('death', 'death'))

    class AnimationMixer(Creature.AnimationMixer):

        notify = DirectNotifyGlobal.directNotify.newCategory(
            'FlyTrapAnimationMixer')
    ...
    def __init__(self):
        ...
        if not FlyTrap.sfx:
            for name in FlyTrap.SfxNames:
                FlyTrap.sfx[name] = loader.loadSfx('audio/' +
                                                   FlyTrap.SfxNames[name])
