...
from pirates.shipparts import DistributedShippart, MastDNA, Sail, SailDNA


class DistributedSail(
    ...
    def createProp(self):
        ...
        if mast:
            ...
            if sail:
                if sail[0]:
                    self.notify.debug('SHIP-%d:Reattaching Sail-%d' %
                                      (self.ship.doId, self.doId))
                    self.prop = sail[0]
                    self.ship.sails[self.dna.mastPosIndex][self.dna.posIndex][
                        1] = self
                    return
        self.notify.debug(
            'SHIP-%d:Creating New Sail-%d' % (self.ship.doId, self.doId))
        ...
        if not self.ship.sails.get(self.dna.mastPosIndex, 0):
            ...
            projScreen = self.ship.modelGeom.attachNewNode(
                ProjectionScreen('sail-set-%d' % self.dna.mastPosIndex))
            ...
            self.prop.setProjScreen(
                self.ship.sailProjectors[self.dna.mastPosIndex])
        self.ship.sails[self.dna.mastPosIndex][self.dna.posIndex] = [
            self.prop, self
        ]
    ...
    def getHp(self):
        return [self.dna.mastPosIndex, self.dna.posIndex, self.Hp, self.maxHp]
    ...
    def addStats(self):
        ...
        if self.dna.sailType >= ShipGlobals.MAINSAILL1 and self.dna.sailType <= ShipGlobals.MAINSAILL3:
            ...
            if self.dna.sailType >= ShipGlobals.FORESAILL1 and self.dna.sailType <= ShipGlobals.FORESAILL3:
                ship.acceleration += self.acceleration
                ship.maxSpeed += self.maxSpeed
                ship.reverseAcceleration += self.reverseAcceleration
                ship.maxReverseSpeed += self.maxReverseSpeed
                ...
                if self.dna.sailType >= ShipGlobals.AFTSAILL1 and self.dna.sailType <= ShipGlobals.AFTSAILL3:
