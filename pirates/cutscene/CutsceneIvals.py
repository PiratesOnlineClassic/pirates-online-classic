...
def Cutscene1_2ivals(cutscene):
    ...
    subs, clearSubs = subtitleSequence(cutscene.cutsceneName)
    return (
     Parallel(hideInterceptorFunc, Sequence(Func(av.recordEvent, 'enableBoatBoarding')), subs),
...
def Cutscene2_1ivals(cutscene):
    ...
    subs, clearSubs = subtitleSequence(cutscene.cutsceneName)
    return (
     Parallel(Func(base.transitions.fadeIn), Func(willTurner.attachHandheld, cutlass, True), Func(av.adjustPos, 30, -8, -5.975), Sequence(WaitFrames(619), Func(willTurner.detachHandheld, cutlass)), Sequence(WaitFrames(619), Func(av.attachHandheld, cutlass, True)), Sequence(WaitFrames(750), Func(av.detachHandheld, cutlass, True)), Sequence(WaitFrames(860), Func(willTurner.recordTransAtCutscene, 860)), Sequence(WaitFrames(864), Func(base.transitions.fadeOut, 2.0)), Sequence(WaitFrames(912), Func(base.transitions.fadeIn, 2.0)), subs), clearSubs)
...
def Cutscene2_1_bivals(cutscene):
    ...
    subs, clearSubs = subtitleSequence(cutscene.cutsceneName)
    return (
     Parallel(Func(av.attachHandheld, cutlass, True), Sequence(WaitFrames(80), Func(av.detachHandheld, cutlass)), Sequence(WaitFrames(404), Func(cutcam.changeCameraParams, 0, 35)), Sequence(WaitFrames(528), Func(willTurner.attachHandheld, cutlass, True)), Sequence(WaitFrames(553), Func(base.transitions.fadeOut)), subs),
...
def Cutscene2_4_bivals(cutscene):
    ...
    subs, clearSubs = subtitleSequence(cutscene.cutsceneName)
    return (
     Parallel(Sequence(WaitFrames(1345 - 497), Func(av.syncZombieAnimation)), Sequence(WaitFrames(1663 - 497), Func(captBarbossa.attachHandheld, coin, True)), Sequence(WaitFrames(1708 - 497), Func(captBarbossa.detachHandheld, coin)), Sequence(WaitFrames(1709 - 497), Func(av.attachHandheld, coin, True)), Sequence(WaitFrames(1729 - 497), Func(av.detachHandheld, coin)), Sequence(WaitFrames(1729 - 497), Func(av.showZombie)), Sequence(WaitFrames(1729 - 497), Func(av.attachHandheld, coin, True, True)), Sequence(WaitFrames(1827 - 497), Func(av.detachHandheld, coin, True, True)), Sequence(WaitFrames(1827 - 497), Func(captBarbossa.attachHandheld, pistol, True)), Sequence(WaitFrames(2087 - 497), Func(captBarbossa.detachHandheld, pistol, True)), subs), clearSubs)
...
# okay decompiling .\pirates\cutscene\CutsceneIvals.pyc
