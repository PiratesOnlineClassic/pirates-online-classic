...
def SetTimeOfDay(tod, time=0):
    if 'localAvatar' in __builtins__:
        return Func(base.cr.timeOfDayManager.request,
                    base.cr.timeOfDayManager.getStateName(tod), 0)
    return Wait(0.001)
    ...
    def _forceInteractCallback(objDoId):
        ...
        if doorIndex != None:
            if len(objRef.links) <= doorIndex:
                print 'warning: could not find door index %s for object %s' % (
                    doorIndex, objDoId)
                return
...
def Cutscene1_1_1ivals(cutscene):
    ...
    return (Parallel(
        Sequence(Func(base.transitions.fadeIn, 7.0)),
...
def Cutscene1_1_2ivals(cutscene):
    ...
    subs, clearSubs = subtitleSequence(cutscene.cutsceneName)
    return (Parallel(
        Sequence(WaitFrames(KickFrame), Func(av.openJailDoor)),
        Sequence(
            WaitFrames(KickFrame), SoundInterval(cballHitTrack, volume=0.6)),
        Sequence(
            WaitFrames(CannonStartFrame),
            Func(messenger.send, 'startTutorialCannons')), subs),
...
def Cutscene1_1_5_aivals(cutscene):
    ...
    subs, clearSubs = subtitleSequence(cutscene.cutsceneName)
    return (Parallel(
        Sequence(Func(base.transitions.fadeIn, 4.0)),
        Sequence(Func(av.adjustPos, 9.561, 26.215, 5.255)),
        ...
        Sequence(
            WaitFrames(754),
            Func(reparentObject, seachestTable, render, VBase3(
                9.53, 33.22, 5.11), VBase3(180, 0, 90))), subs),
...
def Cutscene1_1_5_civals(cutscene):
    ...
    subs, clearSubs = subtitleSequence(cutscene.cutsceneName)
    return (Parallel(
        Sequence(WaitFrames(814), Func(base.transitions.fadeOut, 2.0)), subs),
            Parallel(
                Func(forceInteract, TutorialGlobals.TAVERN_INTERIOR, 1),
                clearSubs))


def Cutscene1_2ivals(cutscene):
    interceptor = cutscene.getActor(
        CutsceneActor.CutBoat.getActorKey(ShipGlobals.STUMPY_SHIP))
    av = cutscene.getActor(CutsceneActor.CutLocalPirate.getActorKey())
    if 'localAvatar' in __builtins__:
        interceptor.masts[0][0].rigging.hide()
        hideInterceptorFunc = setObjectVisibleByUid(
            TutorialGlobals.STUMPY_BOAT_UID)
        showInterceptorFunc = setObjectVisibleByUid(
            TutorialGlobals.STUMPY_BOAT_UID, False)
    else:
    ...
    subs, clearSubs = subtitleSequence(cutscene.cutsceneName)
    return (Parallel(hideInterceptorFunc,
                     Sequence(Func(av.recordEvent, 'enableBoatBoarding')),
                     subs), Parallel(Sequence(showInterceptorFunc), clearSubs))
...
def Cutscene1_3ivals(cutscene):
    ...
    interceptor = cutscene.getActor(
        CutsceneActor.CutBoat.getActorKey(ShipGlobals.STUMPY_SHIP))
    skelship = cutscene.getActor(
        CutsceneActor.CutBoat.getActorKey(ShipGlobals.SKEL_DEATH_OMEN))
    ...
    subs, clearSubs = subtitleSequence(cutscene.cutsceneName)
    return (Parallel(
        Func(messenger.send, 'toggleGlows'), Func(interceptor.hideCannons),
        Func(interceptor.hideMasts),
        Func(skelship.setColorScale, Vec4(0, 0, 0, 0)),
        Func(skelship.stormEffect.setColorScale, Vec4(0, 0, 0, 0)),
        Func(skelship.setTransparency, 1), Func(skelship.hideAllPanels),
        Func(skelship.showMasts),
        ...
        Sequence(
            Func(messenger.send, 'introduceJR'), WaitFrames(83),
            Func(interceptor.showMasts)),
        Sequence(WaitFrames(76), Func(skelship.fadeIn)),
        Sequence(WaitFrames(140), Func(skelship.hull[0].stopDarkFog)),
        Sequence(
            Func(messenger.send, 'JRAttackShip'), WaitFrames(274),
            Func(interceptor.hideMasts), Func(skelship.hull[0].stopDarkFog)),
        ...
        Sequence(
            WaitFrames(731), Func(captBeck.attachHandheld, moneybag, False)),
        Sequence(WaitFrames(1000), Func(jollyRoger.startAttuneEffect)),
        Sequence(WaitFrames(1120), Func(jollyRoger.startSoulSuckEffect)),
        Sequence(
            WaitFrames(1147), Func(captBeck.detachHandheld, moneybag, True)),
        Sequence(
            WaitFrames(1144), Func(captBeck.setBin, 'fixed', 140),
            Func(captBeckSkel.fadeIn, 1.0), Func(captBeck.fadeOut, 1.5)),
        ...
        Sequence(WaitFrames(1800), Func(interceptor.showRigging)),
        Sequence(
            WaitFrames(1910),
            Func(
                skelship.fireCannon,
                ...
                targetNode=interceptor.find('**/mast-3'),
                offset=Vec3(0, 0, 5),
                wantCollisions=1,
                flightTime=0.3,
                preciseHit=1)),
        ...
        Sequence(
            WaitFrames(1955),
            Func(
                interceptor.explosionVFX,
                node=interceptor.find('**/mast-3'),
                offset=Vec3(-15, 15, 45))),
        Sequence(
            WaitFrames(1968),
            Func(
        ...
        Sequence(
            WaitFrames(1960), Func(interceptor.masts[0][0].setBreakAnim, 1),
            Func(interceptor.masts[0][0].breakRigging),
            Func(interceptor.sails[0][1][0].setAnimState, 'break1A')),
        Sequence(
            WaitFrames(1975), Func(interceptor.masts[3][0].setBreakAnim, 0),
            Func(interceptor.sails[3][0][0].setAnimState, 'break0A')),
        Sequence(
            WaitFrames(2020), Func(skelship.hull[0].startDarkFog, -70),
            Func(interceptor.stopJRteleportFX)),
        Sequence(
            WaitFrames(2112), Func(skelship.hull[0].stopDarkFog),
            Func(skelship.fadeOut)), subs),
...
def Cutscene2_1ivals(cutscene):
    ...
    return (Parallel(
        ...
        Func(willTurner.attachHandheld, cutlass, True),
        ...
        Sequence(WaitFrames(912), Func(base.transitions.fadeIn, 2.0)), subs),
            clearSubs)
...
def Cutscene2_1_bivals(cutscene):
    ...
    subs, clearSubs = subtitleSequence(cutscene.cutsceneName)
    return (Parallel(
        Func(av.attachHandheld, cutlass, True),
        Sequence(WaitFrames(80), Func(av.detachHandheld, cutlass)),
        Sequence(WaitFrames(404), Func(cutcam.changeCameraParams, 0, 35)),
        Sequence(
            WaitFrames(528), Func(willTurner.attachHandheld, cutlass, True)),
        Sequence(WaitFrames(553), Func(base.transitions.fadeOut)), subs),
            Parallel(
                Func(willTurner.detachHandheld, cutlass, True),
                Func(forceInteract, TutorialGlobals.PIT_INTERIOR, 1),
                clearSubs))
...
def Cutscene2_2ivals(cutscene):
    ...
    lanternLight = None
    if 'localAvatar' not in __builtins__:
        lanternLightObject = base.pe.objectMgr.findObjectById(
            '1175893134.2dzlu')
    ...
    if lanternLight:
        ...
        reparentFunc1 = Func(reparentObject, lanternLight, wrtNode, Vec3(
            0, 0, 0), Vec3(0, 0, 0))
        reparentFunc2 = Func(reparentObject, lanternLight, render, oldPos,
                             oldHpr)
    else:
    ...
    cutlassDict = [bg1Cutlass, bg2Cutlass, bg3Cutlass]
    func1 = Sequence(
        Func(
    ...
    subs, clearSubs = subtitleSequence(cutscene.cutsceneName)
    return (Parallel(
        func1, Func(tiaDalma.hideVision, cutscene, cutlassDict, False),
        ...
        Sequence(
            WaitFrames(1295),
            Func(tiaDalma.hideVision, cutscene, cutlassDict, True)),
        Sequence(
            WaitFrames(1524), Func(tiaDalma.attachHandheld, compass, True)),
        Sequence(WaitFrames(1685), reparentFunc1),
        Sequence(
            WaitFrames(1690), Func(tiaDalma.detachHandheld, compass, True)),
        Sequence(WaitFrames(2192), Func(base.transitions.fadeOut, 1.0)), subs),
            Parallel(func2, reparentFunc2, Func(lantern.unloadEffects),
                     Func(base.transitions.fadeIn), clearSubs))


def Cutscene2_3ivals(cutscene):
    cutcam = cutscene.getActor(CutsceneActor.CutCam.getActorKey())
    elizabethSwan = cutscene.getActor(
        CutsceneActor.CutElizabethSwan.getActorKey())
    subs, clearSubs = subtitleSequence(cutscene.cutsceneName)
    return (Parallel(
        Func(base.transitions.fadeIn),
        Sequence(WaitFrames(144), Func(cutcam.changeCameraParams, 0, 45)),
        Sequence(WaitFrames(918), Func(cutcam.changeCameraParams, 0, 32)),
        Sequence(WaitFrames(1348), Func(base.transitions.fadeOut, 1.0)),
        Sequence(
            WaitFrames(1372),
            Func(forceInteract, TutorialGlobals.MANSION_INTERIOR, 0)), subs),
            clearSubs)


def Cutscene2_4ivals(cutscene):
    av = cutscene.getActor(CutsceneActor.CutLocalPirate.getActorKey())
    captBarbossa = cutscene.getActor(
        CutsceneActor.CutCaptBarbossa.getActorKey())
    pistol = loader.loadModel('models/handheld/pistol_high')
    pistol.flattenStrong()
    if 'localAvatar' in __builtins__:
        hideMunkieFunkie = setObjectVisibleByUid(
            TutorialGlobals.JACK_THE_MONKEY)
    else:
    ...
    subs, clearSubs = subtitleSequence(cutscene.cutsceneName)
    return (Parallel(
        hideMunkieFunkie, Func(av.adjustPos, -5.4, 11.6, 4.2, 230),
        Sequence(
            WaitFrames(230), Func(captBarbossa.attachHandheld, pistol, False)),
        ...
        Sequence(WaitFrames(496), Func(av.detachHandheld, pistol, True)), subs),
            clearSubs)
...
def Cutscene2_4_bivals(cutscene):
    ...
    subs, clearSubs = subtitleSequence(cutscene.cutsceneName)
    return (Parallel(
        Sequence(WaitFrames(1345 - 497), Func(av.syncZombieAnimation)),
        Sequence(
            WaitFrames(1663 - 497), Func(captBarbossa.attachHandheld, coin,
                                         True)),
        Sequence(
            WaitFrames(1708 - 497), Func(captBarbossa.detachHandheld, coin)),
        ...
        Sequence(
            WaitFrames(1729 - 497), Func(av.attachHandheld, coin, True, True)),
        Sequence(
            WaitFrames(1827 - 497), Func(av.detachHandheld, coin, True, True)),
        Sequence(
            WaitFrames(1827 - 497),
            Func(captBarbossa.attachHandheld, pistol, True)),
        Sequence(
            WaitFrames(2087 - 497),
            Func(captBarbossa.detachHandheld, pistol, True)), subs), clearSubs)
...
def Cutscene2_5ivals(cutscene):
    ...
    subs, clearSubs = subtitleSequence(cutscene.cutsceneName)
    return (Parallel(
        Sequence(WaitFrames(24), Func(HideAllNametags)),
        Func(forceLowLODOnAvatars), Func(base.transitions.fadeIn),
...
def Cutscene3_1ivals(cutscene):
    ...
    return (Parallel(
        Sequence(
            Func(base.transitions.fadeIn), WaitFrames(fadeOutFrame),
            base.transitions.getFadeOutIval(tFadeOut)), subs),
            Parallel(clearSubs))
...
def Cutscene3_2ivals(cutscene):
    ...
    return (Parallel(
        ...
        Sequence(WaitFrames(24), Func(HideAllNametags)),
        Sequence(
            WaitFrames(1387), Func(jackSparrow.attachHandheld, goldbag, True)),
        ...
        Sequence(WaitFrames(1445), Func(av.detachHandheld, goldbag, True)),
        hideJackFunc, subs),
...
def Cutscene6_1ivals(cutscene):
    ...
    subs, clearSubs = subtitleSequence(cutscene.cutsceneName)
    return (Parallel(
        Func(av.localAvatar.hide),
        Sequence(WaitFrames(280), Func(tiaDalma.attachHandheld, vdoll, True)),
        Sequence(WaitFrames(585), Func(tiaDalma.detachHandheld, vdoll, True)),
        subs), Parallel(Func(av.localAvatar.show), clearSubs))
