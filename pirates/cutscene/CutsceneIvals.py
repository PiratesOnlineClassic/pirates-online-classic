...
def Cutscene1_1_2ivals(cutscene):
    ...
    subs, clearSubs = subtitleSequence(cutscene.cutsceneName)
    return (Parallel(Sequence(WaitFrames(KickFrame), Func(av.openJailDoor)),
...
def Cutscene1_1_5_aivals(cutscene):
    ...
    subs, clearSubs = subtitleSequence(cutscene.cutsceneName)
    return (Parallel(Sequence(Func(base.transitions.fadeIn, 4.0)), Sequence(Func(av.adjustPos, 9.561, 26.215, 5.255)),
                     Sequence(Func(av.recordEvent, 'beginSeachest')),
                     Sequence(WaitFrames(125), Func(Nell.attachHandheld, mugRight, True)),
                     Sequence(WaitFrames(126), Func(Nell.attachHandheld, mugLeft, False)),
...
def Cutscene1_1_5_civals(cutscene):
    ...
    av = cutscene.getActor(CutsceneActor.CutLocalPirate.getActorKey())
    subs, clearSubs = subtitleSequence(cutscene.cutsceneName)
    return (Parallel(Sequence(WaitFrames(814), Func(base.transitions.fadeOut, 2.0)), subs),
...
def Cutscene1_2ivals(cutscene):
    ...
    subs, clearSubs = subtitleSequence(cutscene.cutsceneName)
    return (Parallel(hideInterceptorFunc, Sequence(Func(av.recordEvent, 'enableBoatBoarding')), subs),
...
def Cutscene1_3ivals(cutscene):
    ...
    subs, clearSubs = subtitleSequence(cutscene.cutsceneName)
    return (Parallel(Func(messenger.send, 'toggleGlows'), Func(interceptor.hideCannons), Func(interceptor.hideMasts),
                     Func(skelship.setColorScale, Vec4(0, 0, 0, 0)),
                     Func(skelship.stormEffect.setColorScale, Vec4(0, 0, 0, 0)), Func(skelship.setTransparency, 1),
                     Func(skelship.hideAllPanels), Func(skelship.showMasts),
                     Func(jollyRoger.setColorScale, Vec4(0, 0, 0, 0)), hideInterceptorFunc,
                     Sequence(WaitFrames(12), Func(skelship.hull[0].startDarkFog, -80)),
                     Sequence(Func(messenger.send, 'introduceJR'), WaitFrames(83), Func(interceptor.showMasts)),
                     Sequence(WaitFrames(76), Func(skelship.fadeIn)),
                     Sequence(WaitFrames(140), Func(skelship.hull[0].stopDarkFog)),
                     Sequence(Func(messenger.send, 'JRAttackShip'), WaitFrames(274), Func(interceptor.hideMasts),
                              Func(skelship.hull[0].stopDarkFog)),
                     Sequence(WaitFrames(295), Func(jollyRoger.startTeleportEffect, 1.25)),
                     Sequence(WaitFrames(310), Func(jollyRoger.fadeInBlack, 2.0)),
                     Sequence(WaitFrames(385), Func(jollyRoger.spawnSkeletons, cutscene)),
                     Sequence(WaitFrames(731), Func(captBeck.attachHandheld, moneybag, False)),
                     Sequence(WaitFrames(1000), Func(jollyRoger.startAttuneEffect)),
                     Sequence(WaitFrames(1120), Func(jollyRoger.startSoulSuckEffect)),
                     Sequence(WaitFrames(1147), Func(captBeck.detachHandheld, moneybag, True)),
                     Sequence(WaitFrames(1144), Func(captBeck.setBin, 'fixed', 140), Func(captBeckSkel.fadeIn, 1.0),
                              Func(captBeck.fadeOut, 1.5)),
                     Sequence(WaitFrames(1240), Func(jollyRoger.stopSoulSuckEffect)),
                     Sequence(WaitFrames(1547), Func(interceptor.showMasts)),
                     Sequence(WaitFrames(1548), Func(interceptor.hideRigging)),
                     Sequence(WaitFrames(1760), Func(jollyRoger.startTeleportEffect, -1.0)),
                     Sequence(WaitFrames(1765), Func(jollyRoger.fadeOutBlack, 1.5)),
                     Sequence(WaitFrames(1800), Func(interceptor.showRigging)), Sequence(WaitFrames(1910),
                                                                                         Func(skelship.fireCannon, 0,
                                                                                              ammo=InventoryType.CannonFirebrand,
                                                                                              targetNode=interceptor.find(
                                                                                                  '**/mast-3'),
                                                                                              offset=Vec3(0, 0, 5),
                                                                                              wantCollisions=1,
                                                                                              flightTime=0.3,
                                                                                              preciseHit=1)),
                     Sequence(WaitFrames(1928), Func(skelship.fireCannon, 3, ammo=InventoryType.CannonFirebrand,
                                                     targetNode=interceptor.find('**/mast-3'), offset=Vec3(0, 0, 5),
                                                     wantCollisions=1, flightTime=0.5, preciseHit=1)),
                     Sequence(WaitFrames(1948), Func(skelship.fireCannon, 1, ammo=InventoryType.CannonFirebrand,
                                                     targetNode=interceptor.find('**/mast-3'), offset=Vec3(-15, 15, 45),
                                                     wantCollisions=1, flightTime=0.5, preciseHit=1)),
                     Sequence(WaitFrames(1955), Func(interceptor.explosionVFX, node=interceptor.find('**/mast-3'),
                                                     offset=Vec3(-15, 15, 45))), Sequence(WaitFrames(1968),
                                                                                          Func(skelship.fireCannon, 4,
                                                                                               ammo=InventoryType.CannonFirebrand,
                                                                                               targetNode=
                                                                                               interceptor.hull[0],
                                                                                               offset=Vec3(0, 0, 10),
                                                                                               ...
                                                                                               flightTime=0.5,
                                                                                               preciseHit=1)),
                     Sequence(WaitFrames(1980),
                              Func(interceptor.explosionVFX, node=interceptor.hull[0], offset=Vec3(0, 0, 10))),
                     Sequence(WaitFrames(1980), Func(skelship.fireCannon, 0, ammo=InventoryType.CannonFirebrand,
                                                     targetNode=interceptor.hull[0], offset=Vec3(0, 0, 5),
                                                     wantCollisions=1, flightTime=0.5, preciseHit=1)),
                     Sequence(WaitFrames(2008), Func(skelship.fireCannon, 4, ammo=InventoryType.CannonFirebrand,
                                                     targetNode=interceptor.hull[0], offset=Vec3(0, 0, 10),
                                                     wantCollisions=1, flightTime=0.5, preciseHit=1)),
                     Sequence(WaitFrames(2020),
                              Func(interceptor.explosionVFX, node=interceptor.hull[0], offset=Vec3(0, 0, 10))),
                     Sequence(WaitFrames(1980), Func(interceptor.sinkingVFX)),
                     Sequence(WaitFrames(1960), Func(interceptor.masts[0][0].setBreakAnim, 1),
                              Func(interceptor.masts[0][0].breakRigging),
                              Func(interceptor.sails[0][1][0].setAnimState, 'break1A')),
                     Sequence(WaitFrames(1975), Func(interceptor.masts[3][0].setBreakAnim, 0),
                              Func(interceptor.sails[3][0][0].setAnimState, 'break0A')),
                     Sequence(WaitFrames(2020), Func(skelship.hull[0].startDarkFog, -70),
                              Func(interceptor.stopJRteleportFX)),
                     Sequence(WaitFrames(2112), Func(skelship.hull[0].stopDarkFog), Func(skelship.fadeOut)), subs),
...
def Cutscene2_1ivals(cutscene):
    ...
    if 'localAvatar' in __builtins__:
        base.localAvatar.noQuestArrow = True
    subs, clearSubs = subtitleSequence(cutscene.cutsceneName)
    return (Parallel(Func(base.transitions.fadeIn), Func(willTurner.attachHandheld, cutlass, True),
                     Func(av.adjustPos, 30, -8, -5.975),
                     Sequence(WaitFrames(619), Func(willTurner.detachHandheld, cutlass)),
                     Sequence(WaitFrames(619), Func(av.attachHandheld, cutlass, True)),
                     Sequence(WaitFrames(750), Func(av.detachHandheld, cutlass, True)),
                     Sequence(WaitFrames(860), Func(willTurner.recordTransAtCutscene, 860)),
                     Sequence(WaitFrames(864), Func(base.transitions.fadeOut, 2.0)),
                     Sequence(WaitFrames(912), Func(base.transitions.fadeIn, 2.0)), subs), clearSubs)
...
def Cutscene2_1_bivals(cutscene):
    ...
    if 'localAvatar' in __builtins__:
        base.localAvatar.noQuestArrow = False
    subs, clearSubs = subtitleSequence(cutscene.cutsceneName)
    return (Parallel(Func(av.attachHandheld, cutlass, True), Sequence(WaitFrames(80), Func(av.detachHandheld, cutlass)),
                     Sequence(WaitFrames(404), Func(cutcam.changeCameraParams, 0, 35)),
                     Sequence(WaitFrames(528), Func(willTurner.attachHandheld, cutlass, True)),
                     Sequence(WaitFrames(553), Func(base.transitions.fadeOut)), subs),
            Parallel(Func(willTurner.detachHandheld, cutlass, True),
                     Func(forceInteract, TutorialGlobals.PIT_INTERIOR, 1), clearSubs))
...
def Cutscene2_2ivals(cutscene):
    ...
    func2 = Sequence(
        Func(base.ambientMgr.requestFadeIn, 'jungle', 10, finalVolume=PiratesGlobals.DEFAULT_AMBIENT_VOLUME,
             priority=1))
    subs, clearSubs = subtitleSequence(cutscene.cutsceneName)
    return (Parallel(func1, Func(tiaDalma.hideVision, cutscene, cutlassDict, False),
                     Sequence(WaitFrames(784), Func(tiaDalma.showVisionBG, cutscene)),
                     Sequence(WaitFrames(990), Func(tiaDalma.showVisionJR, cutscene)),
                     Sequence(WaitFrames(1010), Func(tiaDalma.spawnSkeleton1, cutscene)),
                     Sequence(WaitFrames(1100), Func(tiaDalma.spawnSkeleton2, cutscene)),
                     Sequence(WaitFrames(1295), Func(tiaDalma.hideVision, cutscene, cutlassDict, True)),
                     Sequence(WaitFrames(1524), Func(tiaDalma.attachHandheld, compass, True)),
                     Sequence(WaitFrames(1685), reparentFunc1),
                     Sequence(WaitFrames(1690), Func(tiaDalma.detachHandheld, compass, True)),
                     Sequence(WaitFrames(2192), Func(base.transitions.fadeOut, 1.0)), subs),
...
def Cutscene2_3ivals(cutscene):
    ...
    subs, clearSubs = subtitleSequence(cutscene.cutsceneName)
    return (Parallel(Func(base.transitions.fadeIn), Sequence(WaitFrames(144), Func(cutcam.changeCameraParams, 0, 45)),
                     Sequence(WaitFrames(918), Func(cutcam.changeCameraParams, 0, 32)),
                     Sequence(WaitFrames(1348), Func(base.transitions.fadeOut, 1.0)),
                     Sequence(WaitFrames(1372), Func(forceInteract, TutorialGlobals.MANSION_INTERIOR, 0)), subs),
            clearSubs)
...
def Cutscene2_4ivals(cutscene):
    ...
    base.transitions.fadeIn(0.5)
    subs, clearSubs = subtitleSequence(cutscene.cutsceneName)
    return (Parallel(hideMunkieFunkie, Func(av.adjustPos, -5.4, 11.6, 4.2, 230),
                     Sequence(WaitFrames(230), Func(captBarbossa.attachHandheld, pistol, False)),
                     Sequence(WaitFrames(264), Func(captBarbossa.detachHandheld, pistol)),
                     Sequence(WaitFrames(265), Func(av.attachHandheld, pistol, True)),
                     Sequence(WaitFrames(496), Func(av.detachHandheld, pistol, True)), subs), clearSubs)
...
def Cutscene2_4_bivals(cutscene):
    ...
    if 'localAvatar' in __builtins__:
        localAvatar.getParentObj().envEffects.soundVolumeDown()
    subs, clearSubs = subtitleSequence(cutscene.cutsceneName)
    return (Parallel(Sequence(WaitFrames(1345 - 497), Func(av.syncZombieAnimation)),
                     Sequence(WaitFrames(1663 - 497), Func(captBarbossa.attachHandheld, coin, True)),
                     Sequence(WaitFrames(1708 - 497), Func(captBarbossa.detachHandheld, coin)),
                     Sequence(WaitFrames(1709 - 497), Func(av.attachHandheld, coin, True)),
                     Sequence(WaitFrames(1729 - 497), Func(av.detachHandheld, coin)),
                     Sequence(WaitFrames(1729 - 497), Func(av.showZombie)),
                     Sequence(WaitFrames(1729 - 497), Func(av.attachHandheld, coin, True, True)),
                     Sequence(WaitFrames(1827 - 497), Func(av.detachHandheld, coin, True, True)),
                     Sequence(WaitFrames(1827 - 497), Func(captBarbossa.attachHandheld, pistol, True)),
                     Sequence(WaitFrames(2087 - 497), Func(captBarbossa.detachHandheld, pistol, True)), subs),
            clearSubs)
...
def Cutscene2_5ivals(cutscene):
    ...
    subs, clearSubs = subtitleSequence(cutscene.cutsceneName)
    return (
    Parallel(Sequence(WaitFrames(24), Func(HideAllNametags)), Func(forceLowLODOnAvatars), Func(base.transitions.fadeIn),
             Func(av.adjustPos, -5, 23, 1.0, 130), hideJackFunc, subs),
...
def Cutscene3_1ivals(cutscene):
    ...
    subs, clearSubs = subtitleSequence(cutscene.cutsceneName)
    return (Parallel(
        Sequence(Func(base.transitions.fadeIn), WaitFrames(fadeOutFrame), base.transitions.getFadeOutIval(tFadeOut)),
        subs), Parallel(clearSubs))
...
def Cutscene3_2ivals(cutscene):
    ...
    return (Parallel(Func(base.transitions.fadeIn), Sequence(WaitFrames(24), Func(HideAllNametags)),
                     Sequence(WaitFrames(1387), Func(jackSparrow.attachHandheld, goldbag, True)),
                     Sequence(WaitFrames(1403), Func(jackSparrow.detachHandheld, goldbag)),
                     Sequence(WaitFrames(1438), Func(av.attachHandheld, goldbag, True)),
                     Sequence(WaitFrames(1445), Func(av.detachHandheld, goldbag, True)), hideJackFunc, subs),
...
def Cutscene6_1ivals(cutscene):
    ...
    vdoll.setP(180)
    subs, clearSubs = subtitleSequence(cutscene.cutsceneName)
    return (Parallel(Func(av.localAvatar.hide), Sequence(WaitFrames(280), Func(tiaDalma.attachHandheld, vdoll, True)),
                     Sequence(WaitFrames(585), Func(tiaDalma.detachHandheld, vdoll, True)), subs),
