...
def showFriendSecret(secretType=AccountSecret):
    ...
    if base.cr.isPaid(
    ) != OTPGlobals.AccessFull and base.cr.productName == 'DisneyOnline-US':
        ...
        if base.cr.productName in [
                'DisneyOnline-AP', 'DisneyOnline-UK', 'ES', 'Wanadoo',
                'T-Online', 'JP'
        ]:
            chatMgr = base.localAvatar.chatMgr
...
class FriendSecretNeedsParentLogin(StateData.StateData):
    ...
    def enter(self):
        ...
        if self.dialog == None:
            ...
            buttonImage = (charGui.find('**/chargui_text_block_large'),
                           charGui.find('**/chargui_text_block_large_down'),
                           charGui.find('**/chargui_text_block_large_over'))
            self.dialog = GuiPanel.GuiPanel('Secret Codes!!! Arg!!', 1.6, 1.2,
                                            False)
            offX = -0.75
            offZ = -0.45
...
class FriendSecret(GuiPanel.GuiPanel, StateData.StateData):
    ...
    def load(self):
        ...
        self.introText = DirectLabel(
            parent=self,
            relief=None,
            pos=(0 + offX, 0, 0.4 + offZ),
            scale=0.05,
            text=PLocalizer.FriendSecretIntro,
            text_fg=PiratesGuiGlobals.TextFG2,
            text_wordwrap=30)
        self.introText.hide()
        ...
        self.getSecret = DirectButton(
            parent=self,
            relief=None,
            pos=(0 + offX, 0, -0.11 + offZ),
            image=buttonImage,
            image_scale=(0.85, 1, 0.4),
    ...
    def __enterSecret(self, secret):
        ...
        self.accept(OTPGlobals.PlayerFriendRejectUseSecretEvent,
                    self.__secretResponseReject)
        taskMgr.doMethodLater(10.0, self.__secretTimeout,
                              'timeoutSecretResponse')
    ...
    def __secretResponseOkay(self, avId, info):
        ...
        self.ignore(OTPGlobals.PlayerFriendRejectUseSecretEvent)
        self.nextText[
            'text'] = OTPLocalizer.FriendSecretEnteredSecretSuccess % info.playerName
        self.nextText.show()
        self.cancel.hide()
        self.ok1.hide()
        self.ok2.show()
