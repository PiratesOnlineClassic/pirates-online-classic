...
class ReportAPlayer(GuiPanel.GuiPanel, FSM.FSM):
    ...
    def __init__(self, playerId, avId, avName):
        ...
        self.fieldText = DirectLabel(parent=self, relief=None, text='', text_align=TextNode.ALeft, text_scale=PiratesGuiGlobals.TextScaleSmall, 
                                     text_wordwrap=20, text_fg=PiratesGuiGlobals.TextFG2, pos=(0.05, 0, 0.75), textMayChange=1)
        self.buttons = []
        for i in range(4):
            button = GuiButton.GuiButton(parent=self, text='', textMayChange=1, text_scale=PiratesGuiGlobals.TextScaleMed, text_pos=(0, -0.01), 
                                         text_wordwrap=16, image_scale=(0.56, 0.25, 0.25))
            button.hide()
            button.setPos(self.width * 0.5, 0, 0.5 + i * -0.1)
            self.buttons.append(button)

        self.cancelButton = GuiButton.GuiButton(parent=self, text=PLocalizer.ReportPlayerCancel, textMayChange=1, text_scale=PiratesGuiGlobals.TextScaleMed, 
                                                text_pos=(0.035, -0.01), image_scale=(0.3, 0.22, 0.22), geom=(geomX,) * 4, geom_pos=(-0.06, 0, 0), geom_scale=0.5, 
                                                geom0_color=PiratesGuiGlobals.ButtonColor3[0], geom1_color=PiratesGuiGlobals.ButtonColor3[1], geom2_color=PiratesGuiGlobals.ButtonColor3[2], 
                                                geom3_color=PiratesGuiGlobals.ButtonColor3[3], image3_color=(0.8, 0.8, 0.8, 1), pos=(self.width * 0.5, 0, 0.075), command=self.destroy)
