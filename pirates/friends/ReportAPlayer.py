...
class ReportAPlayer(GuiPanel.GuiPanel, FSM.FSM):
    ...
    def enterAlreadyReported(self):
        self.fieldText[
            'text'] = PLocalizer.ReportPlayerAlreadyReported % self.avName
    ...
    def enterTopMenu(self):
        ...
        self.buttons[3].configure(
            text=PLocalizer.ReportPlayerReport,
            command=self.request,
            extraArgs=['ChooseCategory'])
        self.buttons[3].show()
    ...
    def enterChooseCategory(self):
        self.fieldText[
            'text'] = PLocalizer.ReportPlayerChooseCategory % self.avName
        self.buttons[0].configure(
            text=PLocalizer.ReportPlayerFoulLanguage,
            command=self.chooseCategory,
            extraArgs=[CentralLogger.ReportFoulLanguage])
        self.buttons[0].show()
        self.buttons[1].configure(
            text=PLocalizer.ReportPlayerPersonalInfo,
            command=self.chooseCategory,
            extraArgs=[CentralLogger.ReportPersonalInfo])
        self.buttons[1].show()
        self.buttons[2].configure(
            text=PLocalizer.ReportPlayerRudeBehavior,
            command=self.chooseCategory,
            extraArgs=[CentralLogger.ReportRudeBehavior])
        self.buttons[2].show()
        self.buttons[3].configure(
            text=PLocalizer.ReportPlayerBadName,
            command=self.chooseCategory,
            extraArgs=[CentralLogger.ReportBadName])
        self.buttons[3].show()
    ...
    def sendReport(self):
        return base.cr.centralLogger.reportPlayer(self.category, self.playerId,
                                                  self.avId)
