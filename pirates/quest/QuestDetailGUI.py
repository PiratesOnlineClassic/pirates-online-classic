...
class QuestDetailBase(DirectFrame):
    ...
    def __init__(self, parent=aspect2d, pos=(0, 0, 0), *args, **kw):
        ...
        topGui.removeNode()
        optiondefs = (
         ('relief', None, None), ('pos', pos, None), ('image', questScroll, None), ('image_scale', 0.47, None), ('text', '', None), ('text_align', TextNode.ALeft, None), ('text_fg', PiratesGuiGlobals.TextFG2, None), ('text_scale', PiratesGuiGlobals.TextScaleLarge, None), ('text_pos', (-0.42, 0.21), None), ('text_shadow', (0, 0, 0, 1), None), ('text_wordwrap', 22, None))
...
class QuestDetailGUI(QuestDetailBase):
    ...
    def setQuestInfoFromOffer(self, offer):
        ...
        if len(reward) > 1:
            text = PLocalizer.QuestItemGuiIncompleteFormat % {'title': title, 'status': status, 'desc': desc, 'reward': reward}
        else:
            text = PLocalizer.QuestItemGuiIncompleteFormatNoReward % {'title': title, 'status': status, 'desc': desc}
        self['text'] = text
