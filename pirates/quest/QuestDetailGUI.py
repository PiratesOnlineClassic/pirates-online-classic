...
class QuestDetailBase(DirectFrame):
    ...
    def __init__(self, parent=aspect2d, pos=(0, 0, 0), *args, **kw):
        ...
        topGui.removeNode()
        optiondefs = (('relief', None, None), ('pos', pos, None),
                      ('image', questScroll, None), ('image_scale', 0.47, None),
                      ('text', '', None), ('text_align', TextNode.ALeft, None),
                      ('text_fg', PiratesGuiGlobals.TextFG2,
                       None), ('text_scale', PiratesGuiGlobals.TextScaleLarge,
                               None), ('text_pos', (-0.42, 0.21),
                                       None), ('text_shadow', (0, 0, 0, 1),
                                               None), ('text_wordwrap', 22,
                                                       None))
...
class QuestDetailGUI(QuestDetailBase):
    ...
    def __init__(self, offer, callback, quest=None):
        self.width = 1
        QuestDetailBase.__init__(
            self, parent=base.a2dBottomRight, pos=(0.6, 0, 0.52))
    ...
    def buildIvals(self):
        self.showIval = LerpPosInterval(
            self, 0.3, pos=Point3(-0.47, 0, 0.52), blendType='easeOut')
        self.hideIval = LerpPosInterval(
            self, 0.3, pos=Point3(0.6, 0, 0.52), blendType='easeIn')
    ...
    def setQuestInfoFromOffer(self, offer):
        ...
        if offer.isLadder():
            ...
            status = QuestDB.QuestDict[questId].getDescriptionText(
                offer.initialTaskStates)
    ...
    def setQuestInfoFromQuest(self, quest):
        ...
        if not taskStates:
            taskStates = QuestDB.QuestDict[questId].getInitialTaskStates(
                localAvatar)
        ...
        if hasRewards:
            ...
            base.cr.centralLogger.writeClientEvent(
                'Debug_QuestDetailGUI_GetRewardCrash: Error would have occured for localAvatar=%s; with questId=%s'
                % (localAvatar.getDoId(), questId))
            if hasattr(quest, 'doId'):
                base.cr.centralLogger.writeClientEvent(
                    'Debug_QuestDetailGUI_GetRewardCrash: Error would have occured for localAvatar=%s; with questDoId=%s'
                    % (localAvatar.getDoId(), quest.getDoId()))
        text = PLocalizer.QuestItemGuiIncompleteFormat % {
            'title': title,
            'status': status,
            'desc': desc,
...
# okay decompiling .\pirates\quest\QuestDetailGUI.pyc
