...
# Embedded file name: pirates.ship.DistributedShipOV
...
class DistributedShipOV(DistributedObjectOV.DistributedObjectOV):
    ...
    def setWishName(self, name):
        self.wishName = name
        messenger.send('setWishName-%s' % self.getDoId(), [self.wishName])
    ...
    def setCrewId(self, crewId):
        self.crewId = crewId
        messenger.send('setCrewId-%s' % self.getDoId(), [self.crewId])
...
# okay decompiling .\pirates\ship\DistributedShipOV.pyc
