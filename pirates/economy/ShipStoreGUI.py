...
class ShipStoreGUI(GuiPanel.GuiPanel):
    ...
    def __init__(self, inventory, name):
        GuiPanel.GuiPanel.__init__(
            self, name, self.width, self.height, showClose=False)
        ...
        self.namePanel = None
        self.updateStats()
        self.storeInventory = ShipItemList.ShipItemList(
            inventory, self.height - 0.1, buy=PiratesGuiGlobals.InventoryAdd)
        self.storeInventory.reparentTo(self)
        self.storeInventory.setPos(0.02, 0, 0.02)
        self.purchaseTitle = DirectFrame(
            parent=self,
            relief=None,
            text=PLocalizer.InventoryTypeNames[self.purchaseInventory[0][0]],
            ...
            textMayChange=1)
        self.purchaseTitle.setPos(self.storeInventory.width, 0,
                                  0.707 + (self.height - 0.065) / 2.5)
        self.card = loader.loadModel('models/textureCards/shipRenders')
        self.shipImage = DirectFrame(
            parent=self,
            relief=DGG.FLAT,
            image_scale=0.2,
            ...
            pad=(0.01, 0.01),
            frameSize=(-0.26, 0.26, -0.185, 0.185),
            textMayChange=1,
            pos=(self.width * 0.72, 0, 1))
        self.shipImage.setTransparency(1)
        barChartWidth = self.width - (self.storeInventory.width + 0.04)
        self.shipStats = BarChart.BarChart(self.statData, 0.25, barChartWidth,
                                           PLocalizer.ShipProfile,
                                           PiratesGuiGlobals.TextFG1)
        self.shipStats.reparentTo(self)
        self.shipStats.setPos(self.storeInventory.width + 0.03, 0, 0.12)
        self.descText = DirectFrame(
            parent=self,
            relief=None,
            text=PLocalizer.ShipDescriptions[self.purchaseInventory[0][0]],
            text_align=TextNode.ALeft,
            ...
            text_shadow=PiratesGuiGlobals.TextShadow,
            frameColor=(0, 0, 0, 0),
            frameSize=(0.02, self.width - self.storeInventory.width - 0.02, 0,
                       0.05),
            textMayChange=1,
            text_wordwrap=18,
            pos=(self.storeInventory.width, 0, 0.75))
        self.balanceTitle = DirectFrame(
            parent=self,
            relief=None,
            text=PLocalizer.Cost,
            text_align=TextNode.ALeft,
            text_scale=PiratesGuiGlobals.TextScaleLarge,
            text_pos=(0.31, 0.411),
            ...
            text_shadow=PiratesGuiGlobals.TextShadow,
            textMayChange=0,
            pos=(self.columnWidth, 0, -0.01))
        self.goldTitle = DirectFrame(
            parent=self,
            relief=None,
            text='%s:' % PLocalizer.MoneyName,
            text_align=TextNode.ALeft,
            text_scale=PiratesGuiGlobals.TextScaleLarge,
            text_pos=(0.31, 0.411),
            text_fg=PiratesGuiGlobals.TextFG1,
            text_shadow=PiratesGuiGlobals.TextShadow,
            textMayChange=0,
            pos=(self.columnWidth, 0, 0.04))
        coinImage = loader.loadModel('models/gui/toplevel_gui').find(
            '**/treasure_w_coin*')
        ...
        self.commitButton = DialogButton.DialogButton(
            ...
            text_scale=PiratesGuiGlobals.TextScaleLarge,
            textMayChange=0,
            pos=(self.width - 0.39, 0, 0.07),
            sortOrder=0)
        lockImage = loader.loadModel('models/gui/toplevel_gui').find(
            '**/subscribers_lock')
        self.lock = DirectFrame(
            parent=self.commitButton,
            relief=None,
            image=lockImage,
            image_scale=0.15,
            pos=(-0.06, 0, 0))
    ...
    def updateStats(self):
        ...
        self.statData.append(
            [PLocalizer.Hull, stats['maxHp'], maxStats['maxHp']])
        self.statData.append(
            [PLocalizer.Sail, stats['maxSp'], maxStats['maxSp']])
        self.statData.append(
            [PLocalizer.Cannon, stats['maxCannons'], maxStats['maxCannons']])
        self.statData.append([
            PLocalizer.Broadsides, stats['maxBroadsides'],
            maxStats['maxBroadsides']
        ])
        self.statData.append(
            [PLocalizer.Cargo, stats['maxCargo'], maxStats['maxCargo']])
        self.statData.append(
            [PLocalizer.Crew, stats['maxCrew'], maxStats['maxCrew']])
    ...
    def handleCommitPurchase(self):
        ...
        if inventory:
            ...
            if len(inventory.getShipDoIdList()) >= inventory.getCategoryLimit(
                ...
                base.localAvatar.guiMgr.createWarning(
                    PLocalizer.CannotHoldShipWarning, PiratesGuiGlobals.TextFG6)
                return
        nameData = [
    ...
    def handleCommitPurchasePart2(self, shipNames):
        ...
        ShipStoreGUI.notify.debug('Make Purchase - Buying: %s, Selling: %s' %
                                  (self.purchaseInventory, self.sellInventory))
        messenger.send(
            'makeSale',
            [self.purchaseInventory, self.sellInventory, nameIndices])
    ...
    def updateBalance(self, extraArgs=None):
        ...
        if inventory:
            ...
            elif len(inventory.getShipDoIdList()) >= inventory.getCategoryLimit(
                    InventoryCategory.SHIPS):
                self.commitButton['frameColor'] = PiratesGuiGlobals.ButtonColor3
            else:
