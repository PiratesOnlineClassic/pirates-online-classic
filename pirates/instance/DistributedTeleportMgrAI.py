...
class DistributedTeleportMgrAI(DistributedObjectAI):
    ...
    def initiateTeleport(self, instanceType, fromInstanceType, shardId, locationUid, instanceDoId, instanceName, gameType, friendDoId, friendAreaDoId):
        if simbase.config.GetBool('want-force-portroyal', True):
        ...
        if not spawnPt:
            self.notify.warning('No available spawn points for island, locationUid=%s!' % \
                locationUid)

            spawnPt = (0, 0, 0, 0, 0, 0)

        self.d_teleportHasBegun(avatar.doId, instanceType, fromInstanceType,
