...
class DistributedTeleportMgr(DistributedObject.DistributedObject):
    ...
    def generate(self):
        ...
        self.accept('localAvTeleportFinishedRequest',
                    self.localAvTeleportFinishedRequest)

    def requestLocalTeleport(self, locationName=None):
        self.requestData = ((), {'locationName': locationName})
        localAvatar.confirmTeleport(
            self.localTeleportConfirmation, feedback=True)
    ...
    def localTeleportConfirmation(self, confirmed):
        if confirmed:
            ...
            base.richPresence.setLocation(locationUid)
        self.requestData = None
        return
    ...
    def localTeleportEffect(self, teleportPosHpr, parent=None, smooth=False):
        if localAvatar.testTeleportFlag(PiratesGlobals.TFInWater):
            ...
            taskMgr.doMethodLater(
                5,
                self.localTeleportPos,
                self.uniqueName('localTeleportPos'),
                extraArgs=[teleportPosHpr, parent, smooth])
    ...
    @report(
        types=['deltaStamp', 'args'],
        prefix='------',
        dConfigParam=['want-teleport-report', 'want-shipboardreport'])
    def localTeleportToId(self,
        ...
        if locationId in base.cr.doId2do and base.cr.doId2do[locationId].dclass.getName(
        ) == 'DistributedOceanGrid':
            logBlock(
    ...
    def _localTeleportToIdInterestComplete(self):
        ...
        if isinstance(teleportToObj, DistributedShip.DistributedShip):
            if teleportToObj.gameFSM.getCurrentOrNextState() in ('PutAway',
                                                                 'Sunk',
                                                                 'Sinking',
                                                                 'Off'):
                self.failTeleport(0, 0,
                                  PLocalizer.TeleportToGoneShipFailMessage)
                return
            else:
                if teleportToObj.gameFSM.getCurrentOrNextState() in (
                        'InBoardingPosition', 'OtherShipBoarded'):
                    self.failTeleport(
                        0, 0, PLocalizer.TeleportToBoardingShipFailMessage)
                    return

            teleportToObj.setZoneLevel(3)
            teleportToObj.registerMainBuiltFunction(localAvatar.placeOnShip,
                                                    [teleportToObj])
            teleportToObj.registerBuildCompleteFunction(
                teleportToObj.enableOnDeckInteractions)
            self.acceptOnce(
                teleportToObj.getParentObj().uniqueName('visibility'),
                self._localTeleportToIdDone)
            base.setLocationCode('Ship')
            return

        self.notify.debug('teleporting obj position is %s' %
                          self.localTeleportingObj.getPos())
        ...
        self.localTeleportingObj.setPosHpr(
            self.localTeleportDestPos[0], self.localTeleportDestPos[1],
    ...
    def _localTeleportToIdDone(self):
        self.cr.loadingScreen.scheduleHide(
            base.cr.getAllInterestsCompleteEvent())
        self.cr.clearTaggedInterestNamed(
            'localTeleportToIdInterestRemoveComplete', ['localTeleportToId'])
    ...
    def requestTeleportToIsland(self, islandUid):
        ...
        if not base.config.GetBool('teleport-all', False):
            localAvatar.setTeleportFlag(PiratesGlobals.TFNoIslandToken,
                                        localAvatar.confirmIslandTeleport,
                                        [islandUid])
    ...
    def islandTeleportConfirmation(self, confirmed, islandUid):
        if confirmed:
            ...
            base.richPresence.setLocation(islandUid)
    ...
    def queryAvatarForTeleport(self, avId):
        ...
        @report(
            types=['deltaStamp', 'args'], dConfigParam='want-teleport-report')
        def teleportConfirmation(confirmed, avId=avId):
            if confirmed:
                handle = self.cr.identifyAvatar(avId)
                if handle:
                    shardId = self.cr.distributedDistrict.doId
                    handle.sendTeleportQuery(avId, shardId)

        localAvatar.confirmTeleport(teleportConfirmation, feedback=True)

    @report(types=['deltaStamp', 'args'], dConfigParam='want-teleport-report')
    def handleAvatarTeleportQuery(self, requesterId, requesterShardId):
        handle = self.cr.identifyAvatar(requesterId)
        if not handle:
            return

        if self.cr.identifyFriend(requesterId) and (
                requesterId in localAvatar.ignoreList or
                self.cr.avatarFriendsManager.checkIgnored(requesterId)):
            handle.sendTeleportResponse(
                PiratesGlobals.encodeTeleportFlag(PiratesGlobals.TFIgnore),
                0,
                0,
                0,
            ...
            if canTeleportTo:
                if self.cr.getActiveWorld(
                    ...
                    localAvatar.setSystemMessage(
                        requesterId, OTPLocalizer.WhisperComingToVisit % avName)
                    localAvatar.guiMgr.messageStack.addTextMessage(
                        OTPLocalizer.WhisperComingToVisit % avName,
                        icon=('friends', None))
                    handle.sendTeleportResponse(
        ...
        localAvatar.confirmTeleportTo(confirmed, requesterId)
    ...
    def initiateTeleport(self,
        ...
        if self.amInTeleport():
            if queue:
                self.queueInitiateTeleport(
                    instanceType, instanceName, shardId, locationUid,
                    instanceDoId, doneCallback, startedCallback, gameType,
                    friendDoId, friendAreaDoId, doEffect)
                return
    ...
    def queueInitiateTeleport(self,
        ...
        teleInfo = [
            ...
            doneCallback, startedCallback, gameType, friendDoId, friendAreaDoId,
            doEffect
        ]
    ...
    def forceTeleportStart(self, instanceName, tzDoId, thDoId, worldGridDoId,
        ...
        taskMgr.removeTasksMatching(self.uniqueName('fadeDone'))
        taskMgr.doMethodLater(
            1, fadeDone, self.uniqueName('fadeDone'), extraArgs=[])

    def teleportAddInterestTZ(self, instanceName, tzDoId, thDoId, worldGridDoId,
                              tzParent, tzZone):
    ...
    def teleportAddInterestCompleteTZ(self, tzDoId, thDoId, worldGridDoId):
        s = MiniLogSentry(self.miniLog, 'teleportAddInterestCompleteTZ', tzDoId,
                          thDoId, worldGridDoId)
