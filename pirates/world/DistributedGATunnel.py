...
class DistributedGATunnel(DistributedGAConnector.DistributedGAConnector,
    ...
    def setupCollisions(self):
        ...
        for i in range(len(self.floorNames)):
            ...
            self.accept(
                'enterFloor' + uniqueFloorName,
                self.__handleOnFloor,
                extraArgs=[i])

    @report(
        types=['frameCount', 'printInterests'],
        dConfigParam='want-connector-report')
    ...
    def fadeOutAmbient(self, index):
        if self.ambientNames[index]:
            base.ambientMgr.requestFadeOut(
                self.ambientNames[index], duration=0.01)
        if self.ambientNames[1 - index]:
            base.ambientMgr.requestChangeVolume(
                self.ambientNames[1 - index], duration=0, finalVolume=0)
    ...
    def __handleOnFloor(self, areaIndex, collEntry):
        if not collEntry or areaIndex in (
                0, 1) and not localAvatar.testTeleportFlag(
                    PiratesGlobals.TFInTunnel):
    ...
    def calcAmbientNames(self):
        ...
        if not self.ambientNames[0] and not self.ambientNames[1]:
            self.ambientNames[1] = base.ambientMgr.calcAmbientNameFromStr(
                self.modelPath)
            self.notify.debug(
                'Assuming self.ambientNames[1] = %s' % self.ambientNames[1])
