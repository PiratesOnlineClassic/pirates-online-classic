...
class ClientArea(DirectObject):
    ...
    def __init__(self):
        ...
        self.uid2obj = {}
        self.isGridLod = base.config.GetBool('make-grid-lod', True)
        self.dynamicLights = []
        self.namedAreas = {}
    ...
    def createPropAvatar(self, objType, object, parent, uid):
            ...
            if anim == 'Track 1':
                ...
                ival = Sequence(
                    ActorInterval(
                        ...
                        propAv, 'sword_slash'), ActorInterval(
                        propAv, 'sword_idle', duration=randWait))
                ival.loop()
                ivals.append(ival)
                ...
                ival = Sequence(
                    ActorInterval(
                        ...
                        propAv, 'sword_thrust', duration=1), ActorInterval(
                        propAv, 'sword_idle'), ActorInterval(
                        propAv, 'boxing_kick'), ActorInterval(
                        propAv, 'sword_idle'))
                ival.loop()
                ivals.append(ival)
                propAv.swordIvals = ivals
        ...
        if objType == 'Animated Avatar - Skeleton':
            ...
            if 'Effect Type' in object and object['Effect Type'] is not None and object[
                    'Effect Type'] in ObjectEffects.OBJECT_EFFECTS:
        ...
        if propAv:
            ...
            if object['Animation Track'] == 'walk' or object['Animation Track'] == 'run':
                self.createPropAvatarMovement(
                    uid, propAv, object['Animation Track'])
            ...
            if self.uniqueId in currData['Objects']:
                for currLink in currData['Node Links']:
                    ...
                    if amNode1 or amNode2:
                        ...
                        moveIval = Sequence(
                            LerpPosInterval(
                                propAv, moveTime, dstPos), LerpHprInterval(
                                propAv, 0, Vec3(h1, p, r)), LerpPosInterval(
                                propAv, moveTime, srcPos), LerpHprInterval(
                                propAv, 0, Vec3(h0, p, r)))
    ...
    def addChildObj(
        ...
        loadableType = objectCat == 'PROP_OBJ' or objectCat == 'BUILDING_OBJ' or objectType == 'Cell Portal Area' or \
                ...
                if 'Visual' in objData and 'Model' in objData['Visual']:
                    if 'SubObjs' in objData:
                        ...
                        if isinstance(objData['Visual']['Model'], types.ListType):
                            ...
                            self.notify.warning("Attempting to load object of type '%s', will load a big red smiley instead" % \
                                objectType)
                        else:
        ...
        if (objModel is None or objModel.isEmpty()) and objNode is None and 'Visual' in objData and 'Model' in objData['Visual']:
            if not objStolen:
                self.notify.warning(
                    'ClientArea: No model named %s' %
        ...
        if 'Objects' in objData:
            self.cr.distributedDistrict.worldCreator.registerPostLoadCall(Functor(
    ...
    def loadAnimatedTree(self, obj, modelName, animName, partName):
        ...
        if loader.loadModelCopy(newModelName) is not None:
            if bLODLoaded:
                obj.addLOD(1, 200, 0)
                ...
                if 'Scale' in currSubObj['Visual']:
                    if bAnimatedTree:
                        transform = TransformState.makeMat(
                            Mat4(obj.getJointTransform('modelRoot', attachInfo[1], '1')))
    ...
    def setupCannonballBldgColl(self, collNode, mask):
        if collNode is None or collNode.isEmpty():
            return
                ...
                while len(toLoad) > 0:
                    ...
                    for currToLoad in toLoad:
                        ...
                        if globalClock.getRealTime() - \
                                startTime > 0.05 and base.config.GetBool('object-load-delay', False):
