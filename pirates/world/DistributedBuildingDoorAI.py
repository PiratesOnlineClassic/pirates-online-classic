...
class DistributedBuildingDoorAI(DistributedDoorBaseAI):
    ...
    def d_setInteriorId(self, interiorDoId, interiorUid, interiorWorldParentId,
                        interiorWorldZoneId):
        self.sendUpdate('setInteriorId', [
            interiorDoId, interiorUid, interiorWorldParentId,
            interiorWorldZoneId
        ])

    def b_setInteriorId(self, interiorDoId, interiorUid, interiorWorldParentId,
                        interiorWorldZoneId):
        self.setInteriorId(interiorDoId, interiorUid, interiorWorldParentId,
                           interiorWorldZoneId)
        self.d_setInteriorId(interiorDoId, interiorUid, interiorWorldParentId,
                             interiorWorldZoneId)

    def getInteriorId(self):
        return [
            self.interiorDoId, self.interiorUid, self.interiorWorldParentId,
            self.interiorWorldZoneId
        ]
    ...
    def requestPrivateInteriorInstance(self):
        ...
        if not interior:
            self.notify.warning(
                'Cannot set interior instance %d interact for door %d, interior %d not found!'
        ...
        if not interiorDoor:
            self.notify.warning(
                'Cannot set interior instance %d interact for door %d, interior door not found!'
                % (avatar.doId, self.doId))

            return

        self.sendUpdateToAvatarId(
            avatar.doId, 'setPrivateInteriorInstance',
            [interior.parentId, interior.zoneId, interior.doId, True])
