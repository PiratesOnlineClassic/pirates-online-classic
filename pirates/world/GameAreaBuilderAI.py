...
class GameAreaBuilderAI(ClientAreaBuilderAI):
    ...
    def __init__(self, air, parent):
        ...
        self.wantBuildingExteriors = config.GetBool('want-building-exteriors',
                                                    True)
    ...
    def createObject(self,
        ...
        if objType == 'Player Spawn Node':
            newObj = self.__createPlayerSpawnNode(parent, parentUid, objKey,
                                                  objectData)
        elif objType == 'Building Exterior' and self.wantBuildingExteriors:
            newObj = self.__createBuildingExterior(parent, parentUid, objKey,
                                                   objectData)
            ...
            newObj = self.__createSearchableContainer(parent, parentUid, objKey,
                                                      objectData)
        elif objType in [
                'Animal', 'Townsperson', 'Spawn Node', 'Dormant NPC Spawn Node',
                'Skeleton', 'NavySailor', 'Creature'
        ]:
            newObj = self.air.enemySpawner.createObject(
                objType, objectData, parent, parentUid, objKey, dynamic)
        elif objType == 'Object Spawn Node' and self.wantSpawnNodes:
            newObj = self.__createObjectSpawnNode(parent, parentUid, objKey,
                                                  objectData)
        elif objType == 'Interactive Prop' and self.wantInteractives:
            newObj = self.__createInteractiveProp(parent, parentUid, objKey,
                                                  objectData)
    ...
    def __createPlayerSpawnNode(self, parent, parentUid, objKey, objectData):
        ...
        if not parent or not isinstance(parent, DistributedInstanceBaseAI):
            self.notify.warning(
                'Cannot setup player spawn point for %r!' % parent)
            return None
        ...
        (x, y, z), objectParent = self.getObjectTruePosAndParent(
            objKey, parentUid, objectData)
    ...
    def __createBuildingExterior(self, parent, parentUid, objKey, objectData):
        ...
        if not parent:
            self.notify.warning(
                'Cannot create building exterior %s, current parent %s, has no parent object!'
                % (objKey, parentUid))
        ...
        if not exteriorUid:
            self.notify.warning(
                'Cannot create building exterior %s, no exterior uid found!' %
                (objKey))
        ...
        if not interiorFile:
            self.notify.debug(
                'Cannot create building exterior %s, no interior file found!' %
                (objKey))
        ...
        if not modelPath:
            self.notify.warning(
                'Cannot create building exterior %s, no model path found for file %s!'
                % (objKey, interiorFile))
        ...
        if not objectList:
            exteriorLocatorNode = self.__createDoorLocatorNode(
                self.parent,
                objKey,
                exteriorUid,
                objectData,
                wantTruePosHpr=False)

            interiorLocatorNode = interior.builder._InteriorAreaBuilderAI__createDoorLocatorNode(
                self.parent, objKey, objKey, objectData)
        else:
            self.air.worldCreator.loadObjectDict(objectList, self.parent,
                                                 objKey, True)

        self.air.worldCreator.loadObjectsFromFile(interiorFile + '.py',
                                                  interior)

        return interior

    def __createDoorLocatorNode(self,
                                parent,
                                parentUid,
                                objKey,
                                objectData,
        ...
        if not parent:
            self.notify.warning(
                'Cannot create door locator node %s, current parent %s, has no parent object!'
                % (objKey, parentUid))
        ...
        if not interior or parentUid == self.parent.getUniqueId():
            self.notify.debug(
                'Cannot create door locator node %s, interior not found!' %
                (objKey))
        ...
        if wantTruePosHpr:
            self.setObjectTruePosHpr(doorLocatorNode, objKey, parentUid,
                                     objectData)
    ...
    def __createSearchableContainer(self, parent, parentUid, objKey,
                                    objectData):
        ...
        if 'Visual' in objectData and 'Color' in objectData['Visual']:
            container.setContainerColor(objectData['Visual'].get(
