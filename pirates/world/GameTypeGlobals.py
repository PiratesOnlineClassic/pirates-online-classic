...
GameTypeStrings = {
    ...
    'descriptionStyle': {
        ...
        PLocalizer.GameStyleBattleDesc,
        PiratesGlobals.GAME_STYLE_TEAM_BATTLE:
        PLocalizer.GameStyleTeamBattleDesc,
        PiratesGlobals.GAME_STYLE_SHIP_BATTLE:
        PLocalizer.GameStyleShipBattleDesc,
        PiratesGlobals.GAME_STYLE_CTF:
        PLocalizer.GameStyleCTFDesc,
        PiratesGlobals.GAME_STYLE_CTL:
        PLocalizer.GameStyleCTLDesc,
        PiratesGlobals.GAME_STYLE_PIRATEER:
        PLocalizer.GameStylePirateer,
        PiratesGlobals.GAME_STYLE_POKER:
        PLocalizer.GameStylePoker,
        PiratesGlobals.GAME_STYLE_BLACKJACK:
        PLocalizer.GameStyleBlackjack,
        PiratesGlobals.CREW_STYLE_FIND_A_CREW:
        PLocalizer.CrewStyleFindACrewDesc,
        PiratesGlobals.CREW_STYLE_FIND_A_PVP_CREW:
        PLocalizer.CrewStyleFindAPVPCrewDesc,
        PiratesGlobals.CREW_STYLE_RECRUIT_MEMBERS:
        PLocalizer.CrewStyleRecruitMembersDesc
    },
    'icon': {
    ...
    if gameType == PiratesGlobals.GAME_TYPE_TM:
        ...
        if callback:
            if game.process == 'client':
                invRequest = DistributedInventoryBase.DistributedInventoryBase.getInventory(
                    localAvatar.inventoryId, gatherTMInfo)
            else:
                tmsAvailable = {}
                if gameStyle != None:
                    numPlayers = PiratesGlobals.DYNAMIC_GAME_STYLE_PROPS[
                        PiratesGlobals.GAME_TYPE_TM][gameStyle].get(
                            'NumPlayers')
    ...
    PiratesGlobals.GAME_TYPE_PVP: {
        'style': {
            ...
            PiratesGlobals.GAME_STYLE_TEAM_BATTLE: {
                'options': {
                    GAME_OPTION_MIN_PLAYERS:
                    [PiratesGuiGlobals.UIItemType_ListItem, ['4', '6']],
                    GAME_OPTION_MAX_PLAYERS:
                    [PiratesGuiGlobals.UIItemType_Hidden, '8']
                }
            },
            PiratesGlobals.GAME_STYLE_BATTLE: {
                'options': {
    ...
    PiratesGlobals.GAME_TYPE_PG: {
        'style': {
            PiratesGlobals.GAME_STYLE_BLACKJACK: {
                'options': {
                    GAME_OPTION_MAX_PLAYERS:
                    [PiratesGuiGlobals.UIItemType_Hidden, '6']
                }
            },
            PiratesGlobals.GAME_STYLE_POKER: {
                'options': {
...
if pvpMode < 1:
    del GameTypes[PiratesGlobals.GAME_TYPE_PVP]['style'][
        PiratesGlobals.GAME_STYLE_CTL]
if pvpMode < 2:
    del GameTypes[PiratesGlobals.GAME_TYPE_PVP]['style'][
        PiratesGlobals.GAME_STYLE_PIRATEER]
...
def _styleInfoIsDynamic(styleInfo):
    return type(styleInfo) == types.MethodType or type(
        styleInfo) == types.FunctionType
            ...
            if _styleInfoIsDynamic(styleInfo):
                ...
                def extractOptions(tmsOwned):
                    if callback:
                        foundOptions = {}
                        if tmsOwned:
                            foundOptions = tmsOwned[gameStyle].get(
                                'options', {})
                        callback(foundOptions)

                gameOptions = styleInfo(gameType, gameStyle,
                                        extractOptions).get('options', {})
