...
class DaggerProjectile(PooledEffect, EffectController):
    ...
    def createTrack(self, time, targetPos, motion_color, rate=1):
        ...
        spinTrack = LerpHprInterval(
            self.rotDummy,
            time,
            Vec3(hOffset, -3080 * time + offset, 90 + rOffset),
            startHpr=Vec3(hOffset, offset, 90 + rOffset))
        movement = Sequence(
            Func(self.motion_trail.beginTrail),
            Func(self.motion_trail2.beginTrail), throwTrack,
            Func(self.motion_trail.endTrail), Func(self.motion_trail2.endTrail))
        self.track = Sequence(
            Parallel(spinTrack, movement), Func(self.cleanUpEffect))
