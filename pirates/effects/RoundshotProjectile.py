...
class RoundshotProjectile(EffectController, NodePath):
    ...
    def __init__(self):
        ...
        self.motion_trail = PolyTrail.PolyTrail(None, self.vertex_list,
                                                self.motion_color)
    ...
    def createTrack(self, time, targetPos, motion_color, rate=1):
        ...
        fader = LerpColorScaleInterval(self.shot, time, Vec4(
            1.0, 1.0, 1.0, 0.0))
        movement = Sequence(
            Func(self.motion_trail.beginTrail), throwTrack,
            Func(self.motion_trail.endTrail))
        self.track = Sequence(
            Func(self.shot.setColorScale, Vec4(1, 1, 1, 1)),
            Parallel(movement, fader), Func(self.cleanUpEffect),
            Func(self.destroy))
