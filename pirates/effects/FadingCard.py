...
class FadingCard(EffectController, NodePath):
    ...
    def createTrack(self):
        if not self.card:
            self.track = Sequence()
            return
        fadeIn = self.card.colorScaleInterval(
            self.fadeTime, self.fadeColor, startColorScale=Vec4(0, 0, 0, 0))
        fadeOut = self.card.colorScaleInterval(
            self.waitTime, Vec4(0, 0, 0, 0), startColorScale=self.fadeColor)
        scaleBlast = self.card.scaleInterval(
            self.fadeTime / 3.0 + self.waitTime,
            self.endScale,
            startScale=self.startScale,
            blendType='easeOut')
        self.track = Sequence(
            Wait(2 * self.fadeTime), Func(self.flashDummy.show),
            Parallel(
                Sequence(fadeIn, Wait(self.fadeTime), fadeOut), scaleBlast),
            Func(self.flashDummy.hide), Func(self.cleanUpEffect))
