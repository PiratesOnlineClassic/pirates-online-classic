...
def getSkillSfx():
    ...
    if not skillSfxs:
        skillSfxs = {
            ...
            loader.loadSfx('audio/sfx_grenade_impact.mp3'),
            InventoryType.GrenadeShockBomb:
            loader.loadSfx('audio/sfx_grenade_impact_stink_pot.mp3'),
            InventoryType.GrenadeFireBomb:
            loader.loadSfx('audio/sfx_grenade_impact_firebomb_explo.mp3'),
            InventoryType.GrenadeSmokeCloud:
            loader.loadSfx('audio/sfx_grenade_impact_smoke.mp3'),
                ...
                if objType == PiratesGlobals.COLL_DESTRUCTIBLE:
                    ...
                    if objType == PiratesGlobals.COLL_SHIPPART:
                        ...
                        if objType == PiratesGlobals.COLL_SEA:
                            self.waterHitEffect(hitObject, pos, skillId,
                                                ammoSkillId)
                        else:
                            if objType == PiratesGlobals.COLL_LAND:
                                self.groundHitEffect(hitObject, pos, skillId,
                                                     ammoSkillId)
                            else:
                                if objType == PiratesGlobals.COLL_BLOCKER:
                                    self.blockerHitEffect(
                                        hitObject, pos, skillId, ammoSkillId)
                                else:
                                    if objType == PiratesGlobals.COLL_BLDG:
                                        self.buildingHitEffect(
                                            hitObject, pos, skillId,
                                            ammoSkillId)
                                    else:
                                        if objType == PiratesGlobals.COLL_GRAPPLE_TARGET:
                                            self.grappleHitEffect(
                                                hitObject, pos, skillId,
                                                ammoSkillId)
                                        else:
                                            if objType == PiratesGlobals.COLL_CANNON:
                                                self.cannonHitEffect(
                                                    hitObject, pos, skillId,
                                                    ammoSkillId)
                                            else:
        ...
        if ammoSkillId == InventoryType.CannonRoundShot or ammoSkillId == InventoryType.CannonChainShot or ammoSkillId == InventoryType.CannonBullet or ammoSkillId == InventoryType.CannonSkull or ammoSkillId == InventoryType.CannonBarShot or ammoSkillId == InventoryType.CannonFury:
            ...
            if base.options.getSpecialEffectsSetting(
                ...
                if shockwaveRingEffect:
                    ...
                    if self.normal:
                        shockwaveRingEffect.lookAt(
                            shockwaveRingEffect.getPos() + self.normal)
                ...
                if ammoSkillId == InventoryType.CannonExplosive:
                    ...
                    if ammoSkillId == InventoryType.CannonThunderbolt:
                        if base.options.getSpecialEffectsSetting(
                            ...
                            shockwaveRingEffect = ShockwaveRing.getEffect()
                            if shockwaveRingEffect:
                                shockwaveRingEffect.wrtReparentTo(
                                    base.effectsRoot)
                                shockwaveRingEffect.setPos(hitObject, pos)
                                shockwaveRingEffect.size = aoeRadius * 4
                                shockwaveRingEffect.play()
                        ...
                        if ammoSkillId == InventoryType.GrenadeExplosion:
                            if base.options.getSpecialEffectsSetting(
                                ...
                                if base.launcher.getPhaseComplete(3):
                                    for i in range(random.randint(2, 4)):
                                        ...
                                        if debrisEffect:
                                            debrisEffect.reparentTo(
                                                base.effectsRoot)
                                            debrisEffect.setPos(hitObject, pos)
                                            debrisEffect.offsetEndPlaneZFrom(
                                                hitObject.getZ())
                            ...
                            if base.options.getSpecialEffectsSetting(
                                ...
                                shockwaveRingEffect = ShockwaveRing.getEffect()
                                if shockwaveRingEffect:
                                    shockwaveRingEffect.reparentTo(
                                        base.effectsRoot)
                                    shockwaveRingEffect.setPos(hitObject, pos)
                                    shockwaveRingEffect.size = aoeRadius * 4
                                    shockwaveRingEffect.play()
                            ...
                            if ammoSkillId == InventoryType.GrenadeShockBomb:
                                if base.options.getSpecialEffectsSetting(
                                    ...
                                    explosionEffect = ExplosionFlip.getEffect()
                                    if explosionEffect:
                                        explosionEffect.reparentTo(
                                            base.effectsRoot)
                                        explosionEffect.setPos(hitObject, pos)
                                        explosionEffect.setScale(1.0)
                                        explosionEffect.play()
                                        self.playSfx(ammoSkillId,
                                                     explosionEffect)
                                    flashEffect = MuzzleFlash.getEffect()
                                    if flashEffect:
                                        flashEffect.reparentTo(base.effectsRoot)
                                ...
                                if ammoSkillId == InventoryType.GrenadeSiege:
                                    if base.options.getSpecialEffectsSetting(
                                        ...
                                        if smokePillarEffect:
                                            smokePillarEffect.reparentTo(
                                                hitObject)
                                            smokePillarEffect.setPos(
                                                hitObject, pos)
                                    ...
                                    if base.options.getSpecialEffectsSetting(
                                        ...
                                        if shipSplintersAEffect:
                                            shipSplintersAEffect.reparentTo(
                                                hitObject)
                                            shipSplintersAEffect.setPos(
                                                hitObject, pos)
                                            shipSplintersAEffect.play()
                                        explosionEffect = ExplosionFlip.getEffect(
                                        )
                                        if explosionEffect:
                                            explosionEffect.reparentTo(
                                                base.effectsRoot)
                                            explosionEffect.setPos(
                                                hitObject, pos)
                                            explosionEffect.setScale(2.0)
                                            explosionEffect.play()
                                            self.playSfx(
                                                ammoSkillId, explosionEffect)
                                    ...
                                    if base.options.getSpecialEffectsSetting(
                                        ...
                                        if dustRingEffect:
                                            dustRingEffect.reparentTo(hitObject)
                                            dustRingEffect.setPos(
                                                hitObject, pos)
                                            dustRingEffect.play()
                                        cameraShakerEffect = CameraShaker()
                                        cameraShakerEffect.wrtReparentTo(
                                            hitObject)
                                        cameraShakerEffect.setPos(
                                            hitObject, pos)
                                        cameraShakerEffect.shakeSpeed = 0.06
                                        cameraShakerEffect.shakePower = 4.0
                                        cameraShakerEffect.numShakes = 2
                                    ...
                                    if ammoSkillId == InventoryType.GrenadeFireBomb:
                                        ...
                                        if base.options.getSpecialEffectsSetting(
                                            ...
                                            if blackSmokeEffect:
                                                blackSmokeEffect.wrtReparentTo(
                                                    base.effectsRoot)
                                                blackSmokeEffect.setPos(
                                                    hitObject, pos)
                                        ...
                                        if ammoSkillId == InventoryType.GrenadeSmokeCloud:
                                            if base.options.getSpecialEffectsSetting(
                                                ...
                                                if smokeCloudEffect:
                                                    smokeCloudEffect.reparentTo(
                                                        hitObject)
                                                    smokeCloudEffect.setPos(
                                                        hitObject, pos)
                                                    smokeCloudEffect.radius = aoeRadius / 1.5
                                                    smokeCloudEffect.play()
                                                    self.playSfx(
