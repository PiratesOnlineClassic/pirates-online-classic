...
class JRSpiritEffect(PooledEffect, EffectController):
    ...
    def createTrack(self):
        ...
        duration = 3.5
        fadeIn = LerpColorInterval(
            self,
            0.75,
            Vec4(1.0, 1.0, 1.0, 1.0),
            startColor=Vec4(1.0, 1.0, 1.0, 0.0))
        fadeOut = LerpColorInterval(
            self,
            1.5,
        ...
        uvScrollB = LerpFunctionInterval(
            ...
            fromData=-2.6,
            extraArgs=[self.effectModel, textureStage])
        self.startEffect = Parallel(fadeIn, Sequence(Wait(0.5), fadeOut),
                                    posIval, scaleIval,
                                    Sequence(uvScrollA, uvScrollB))
