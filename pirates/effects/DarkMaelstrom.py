...
class DarkMaelstrom(DirectObject, NodePath):
    ...
    def __init__(self, newParent=render):
        ...
        self.glow = loader.loadModelCopy('models/effects/GhostShipFX')
        ...
        rotate = Parallel(
            stormTops[0].hprInterval(
                duration, Point3(360, 0, 0), startHpr=Point3(0, 0, 0)),
            stormTops[1].hprInterval(
                duration, Point3(0, 0, 0), startHpr=Point3(360, 0, 0)))
        uvScrollTop = Parallel(
            LerpFunctionInterval(
                self.setNewUVs,
                duration,
                toData=0,
                fromData=1,
                extraArgs=[stormTops[0], stormTopTs[0]]),
            LerpFunctionInterval(
                self.setNewUVs,
                duration,
                fromData=1,
                toData=0,
                extraArgs=[stormTops[1], stormTopTs[1]]))
        self.track = Parallel(rotate, uvScrollTop)
        texture = top.findAllTextures()[0]
        texture.setWrapU(Texture.WMRepeat)
        texture.setWrapV(Texture.WMRepeat)
        bolt1In = self.bolt1.colorInterval(
            0.1, Vec4(1, 1, 1, 1), startColor=Vec4(0, 0, 0, 0))
        bolt1Out = self.bolt1.colorInterval(
            0.1, Vec4(0, 0, 0, 0), startColor=Vec4(1, 1, 1, 1))
        bolt2In = self.bolt2.colorInterval(
            0.1, Vec4(1, 1, 1, 1), startColor=Vec4(0, 0, 0, 0))
        bolt2Out = self.bolt2.colorInterval(
            0.1, Vec4(0, 0, 0, 0), startColor=Vec4(1, 1, 1, 1))
        bolt3In = self.bolt3.colorInterval(
            0.1, Vec4(1, 1, 1, 1), startColor=Vec4(0, 0, 0, 0))
        bolt3Out = self.bolt3.colorInterval(
            0.1, Vec4(0, 0, 0, 0), startColor=Vec4(1, 1, 1, 1))
        bolt4In = self.bolt4.colorInterval(
            0.1, Vec4(1, 1, 1, 1), startColor=Vec4(0, 0, 0, 0))
        bolt4Out = self.bolt4.colorInterval(
            0.1, Vec4(0, 0, 0, 0), startColor=Vec4(1, 1, 1, 1))
        ...
        self.lightningTrack = Sequence(
            Wait(1.0), playBolt1, Wait(0.5), playBolt2, Wait(2.0), playBolt3,
            Wait(1.5), playBolt4, Wait(0.0))
        self.fadeTrack = None
    ...
    def fadeOutAndStop(self):
        ...
        self.fadeTrack = Sequence(
            LerpColorScaleInterval(self, 1.0, Vec4(1.0, 1.0, 1.0, 0.0)),
            Func(self.stop))
