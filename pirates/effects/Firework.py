...
class Firework(NodePath):
    ...
    def generateFireworkIval(self):
        if not self.fireworkIval:
            ...
            if self.typeId == FireworkType.BasicBlast:
                firework = FireworkEffect(FireworkBurstType.BasicShell, FireworkTrailType.Glow, self.velocity, self.scale, self.primaryColor, self.secondaryColor, self.burstDelay)
                firework.reparentTo(self)
                self.fireworkEffects.append(firework)
                self.fireworkIval.append(firework.getFireworkMainIval())
            else:
                if self.typeId == FireworkType.LongBlast:
                    firework = FireworkEffect(FireworkBurstType.LongShell, FireworkTrailType.Default, self.velocity, self.scale, self.primaryColor, self.secondaryColor, self.burstDelay)
                    firework.reparentTo(self)
                    self.fireworkEffects.append(firework)
                    self.fireworkIval.append(firework.getFireworkMainIval())
                else:
                    if self.typeId == FireworkType.TrailBurst:
                        ...
                        if self.typeId == FireworkType.RingBlast:
                            firework = FireworkEffect(FireworkBurstType.RingBlast, FireworkTrailType.GlowSparkle, self.velocity, self.scale, self.primaryColor, self.secondaryColor, self.burstDelay)
                            firework.reparentTo(self)
                            self.fireworkEffects.append(firework)
                            self.fireworkIval.append(firework.getFireworkMainIval())
                        else:
                            if self.typeId == FireworkType.NoiseBall:
                                firework = FireworkEffect(FireworkBurstType.NoiseBall, FireworkTrailType.GlowSparkle, self.velocity, self.scale, self.primaryColor, self.secondaryColor, self.burstDelay)
                                firework.reparentTo(self)
                                self.fireworkEffects.append(firework)
                                self.fireworkIval.append(firework.getFireworkMainIval())
                            else:
                                if self.typeId == FireworkType.GlowFlare:
                                    ...
                                    if self.typeId == FireworkType.PalmTree:
                                        ...
                                        if self.typeId == FireworkType.Mickey:
                                            head = FireworkEffect(FireworkBurstType.BasicShell, FireworkTrailType.Glow, velocity=Vec3(0, 0, 425 * self.scale), scale=self.scale / 1.2, primaryColor=self.primaryColor, secondaryColor=self.secondaryColor, burstDelay=1.5)
                                            leftEar = FireworkEffect(FireworkBurstType.BasicShell, FireworkTrailType.Glow, velocity=Vec3(-175 * self.scale, 0, 650 * self.scale), scale=self.scale / 2.0, primaryColor=self.primaryColor, secondaryColor=self.secondaryColor, burstDelay=1.7)
                                            rightEar = FireworkEffect(FireworkBurstType.BasicShell, FireworkTrailType.Glow, velocity=Vec3(175 * self.scale, 0, 650 * self.scale), scale=self.scale / 2.0, primaryColor=self.primaryColor, secondaryColor=self.secondaryColor, burstDelay=1.7)
                                            ...
                                            if self.typeId == FireworkType.PirateSkull:
                                                ...
                                                if self.typeId == FireworkType.AmericanFlag:
                                                    ...
                                                    firework = FireworkEffect(FireworkBurstType.Sparkles, FireworkTrailType.Default, velocity=Vec3(-150, 0, 400), scale=self.scale * 1.5)
                                                    firework.reparentTo(self)
                                                    self.fireworkEffects.append(firework)
    ...
    def cleanup(self):
        ...
        self.fireworkEffects = []
        return
# okay decompiling .\pirates\effects\Firework.pyc
