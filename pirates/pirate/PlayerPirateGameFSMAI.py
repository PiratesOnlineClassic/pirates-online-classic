...
class PlayerPirateGameFSMAI(BattleAvatarGameFSMAI):
    ...
    def enterDeath(self):
        ...
        if not area:
            self.notify.warning(
                'Cannot teleport avatar %d to jail, no parent object found!' %
                (self.avatar.doId))
        ...
        if not instance:
            self.notify.warning(
                'Cannot teleport avatar %d to jail, parent object has no instance!'
                % (self.avatar.doId))
        ...
        if not interior:
            self.notify.warning(
                'Cannot teleport avatar %d to jail, unknown interior object!' %
                (self.avatar.doId))
        ...
        if not cellDoor:
            self.notify.warning(
                'Cannot teleport avatar %d to jail, no cell doors found!' %
                (self.avatar.doId))
        ...
        def teleportToJail(task):
            ...
            if not inventory:
                self.notify.warning(
                    'Cannot set groggy state for avatar %d, no inventory was found!'
                    % self.avatar.doId)
        ...
        self.jailTeleportingTask = taskMgr.doMethodLater(
            5.0, teleportToJail, self.avatar.uniqueName('teleport-to-jail'))
    ...
    def enterThrownInJail(self):
        ...
        if not area:
            self.notify.warning(
                'Cannot finish teleporting to jail, Avatar %d has no parent object!'
                % (self.avatar.doId))
        ...
        if not instance:
            self.notify.warning(
                'Cannot finish teleporting to jail, avatar %d parent object has no instance!'
                % (self.avatar.doId))
        ...
        def teleportToJailFinish(task):
            ...
            (x, y, z, h) = instance.getSpawnPt(area.getUniqueId(),
                                               self.avatar.getJailCellIndex())
            instance.d_setSpawnInfo(self.avatar.doId, x, y, z, h, 0,
                                    [area.doId, area.parentId, area.zoneId])
        ...
        # eliminate race conditions...
        self.jailTeleportingFinishTask = taskMgr.doMethodLater(
            2.0, teleportToJailFinish,
