# Embedded file name: pirates.leveleditor.worldData.port_royal_building_int_6
from pandac.PandaModules import Point3, VBase3, Vec4
objectStruct = {
    'AmbientColors': {},
    'DirectionalColors': {},
    'FogColors': {},
    'FogRanges': {},
    'Objects': {
        '1155773612.45fxlara0': {
            'Type': 'Building Interior',
            'Name': '',
            'AdditionalData': ['interior_spanish_store_jeweler'],
            'Instanced': True,
            'Objects': {
                '1169151219.8mike': {
                    'Type': 'Townsperson',
                    'Category': 'Commoner',
                    'AnimSet': 'idleC',
                    'CustomModel': 'None',
                    'Hpr': VBase3(70.1, 0.0, 0.0),
                    'Patrol Radius': '12.0000',
                    'Pos': Point3(7.66, -3.967, 0.0),
                    'PoseAnim': '',
                    'PoseFrame': '',
                    'Private Status': 'All',
                    'Respawns': True,
                    'Scale': VBase3(1.0, 1.0, 1.0),
                    'ShopID': 'PORT_ROYAL_DEFAULTS',
                    'Start State': 'Idle',
                    'Team': 'Villager'
                },
                '1201033298.05dxschafe': {
                    'Type': 'Door Locator Node',
                    'Name': 'door_locator',
                    'Hpr': VBase3(178.916, 0.0, 0.0),
                    'Pos': Point3(-13.404, 47.298, 5.038),
                    'Scale': VBase3(1.0, 1.0, 1.0),
                    'TargetUIDs': []
                },
                '1201109074.66dxschafe': {
                    'Type': 'Townsperson',
                    'Category': 'Jeweler',
                    'AnimSet': 'default',
                    'CustomModel': 'None',
                    'Hpr': VBase3(99.807, 0.0, 0.0),
                    'Patrol Radius': '1.0000',
                    'Pos': Point3(9.299, 40.271, 5.265),
                    'PoseAnim': '',
                    'PoseFrame': '',
                    'Private Status': 'All',
                    'Respawns': True,
                    'Scale': VBase3(1.0, 1.0, 1.0),
                    'ShopID': 'PORT_ROYAL_DEFAULTS',
                    'Start State': 'Idle',
                    'Team': 'Villager'
                },
                '1201110056.77dxschafe': {
                    'Type': 'Furniture',
                    'DisableCollision': False,
                    'Hpr': VBase3(91.39, 0.0, 0.0),
                    'Objects': {},
                    'Pos': Point3(-9.222, -5.315, 0.0),
                    'Scale': VBase3(1.223, 1.223, 1.223),
                    'Visual': {
                        'Model': 'models/props/bench_bank'
                    }
                },
                '1201110107.2dxschafe': {
                    'Type': 'Furniture',
                    'DisableCollision': False,
                    'Hpr': VBase3(56.855, 0.0, 0.0),
                    'Objects': {},
                    'Pos': Point3(-6.115, 0.149, 0.0),
                    'Scale': VBase3(1.0, 1.0, 1.0),
                    'Visual': {
                        'Model': 'models/props/chair_bank'
                    }
                },
                '1201110157.89dxschafe': {
                    'Type': 'Furniture',
                    'DisableCollision': False,
                    'Hpr': VBase3(-133.701, 0.0, 0.0),
                    'Pos': Point3(-2.948, -6.777, 0.0),
                    'Scale': VBase3(1.0, 1.0, 1.0),
                    'Visual': {
                        'Model': 'models/props/chair_bank'
                    }
                },
                '1201110790.16dxschafe': {
                    'Type': 'Interior_furnishings',
                    'DisableCollision': False,
                    'Hpr': VBase3(-42.262, 0.0, 0.0),
                    'Pos': Point3(-6.001, -2.612, 2.781),
                    'Scale': VBase3(2.214, 2.214, 2.214),
                    'Visual': {
                        'Color': (1.0, 0.800000011920929, 0.6000000238418579,
                                  1.0),
                        'Model': 'models/props/shop_bsmith_bottleA'
                    }
                },
                '1201111177.05dxschafe': {
                    'Type': 'Furniture',
                    'DisableCollision': False,
                    'Hpr': Point3(0.0, 0.0, 0.0),
                    'Pos': Point3(-5.527, -4.942, 0.0),
                    'Scale': VBase3(0.958, 1.925, 0.958),
                    'Visual': {
                        'Color': (0.9, 0.8, 0.9, 1.0),
                        'Model': 'models/props/table_bar_square'
                    }
                },
                '1201111368.09dxschafe': {
                    'Type': 'Jugs_and_Jars',
                    'Hpr': VBase3(14.229, 0.0, 0.0),
                    'Pos': Point3(-4.65, -7.075, 2.803),
                    'Scale': VBase3(1.0, 1.0, 1.0),
                    'Visual': {
                        'Model': 'models/props/winebottle_B'
                    }
                },
                '1201113000.53dxschafe': {
                    'Type': 'Light - Dynamic',
                    'Attenuation': '0.005',
                    'ConeAngle': '60.0000',
                    'DropOff': '63.9759',
                    'FlickRate': '0.5000',
                    'Hpr': VBase3(-69.019, 38.174, -10.695),
                    'Intensity': '0.7349',
                    'LightType': 'SPOT',
                    'Pos': Point3(-9.55, 33.271, -5.151),
                    'Scale': VBase3(1.0, 1.0, 1.0),
                    'Visual': {
                        'Color': (1, 1, 1, 1),
                        'Model': 'models/props/light_tool_bulb'
                    }
                }
            },
            'Visual': {
                'Model': 'models/buildings/interior_spanish_store'
            }
        }
    },
    'Node Links': [],
    'Layers': {
        'Collisions': [
            '1184008208.59kmuller', '1184016064.62kmuller',
            '1184013852.84kmuller', '1185822696.06kmuller',
            '1184006140.32kmuller', '1184002350.98kmuller',
            '1184007573.29kmuller', '1184021176.59kmuller',
            '1184005963.59kmuller', '1188324241.31akelts',
            '1184006537.34kmuller', '1184006605.81kmuller',
            '1187139568.33kmuller', '1188324186.98akelts',
            '1184006730.66kmuller', '1184007538.51kmuller',
            '1184006188.41kmuller', '1184021084.27kmuller',
            '1185824396.94kmuller', '1185824250.16kmuller',
            '1185823630.52kmuller', '1185823760.23kmuller',
            '1185824497.83kmuller', '1185824751.45kmuller',
            '1187739103.34akelts', '1188323993.34akelts',
            '1184016538.29kmuller', '1185822200.97kmuller',
            '1184016225.99kmuller', '1195241421.34akelts',
            '1195242796.08akelts', '1184020642.13kmuller',
            '1195237994.63akelts', '1184020756.88kmuller',
            '1184020833.4kmuller', '1185820992.97kmuller',
            '1185821053.83kmuller', '1184015068.54kmuller',
            '1184014935.82kmuller', '1185821432.88kmuller',
            '1185821701.86kmuller', '1195240137.55akelts',
            '1195241539.38akelts', '1195238422.3akelts', '1195238473.22akelts',
            '1185821453.17kmuller', '1184021269.96kmuller',
            '1185821310.89kmuller', '1185821165.59kmuller',
            '1185821199.36kmuller', '1185822035.98kmuller',
            '1184015806.59kmuller', '1185822059.48kmuller',
            '1185920461.76kmuller', '1194984449.66akelts',
            '1185824206.22kmuller', '1184003446.23kmuller',
            '1184003254.85kmuller', '1184003218.74kmuller',
            '1184002700.44kmuller', '1186705073.11kmuller',
            '1187658531.86akelts', '1186705214.3kmuller',
            '1185824927.28kmuller', '1184014204.54kmuller',
            '1184014152.84kmuller'
        ]
    },
    'ObjectIds': {
        '1155773612.45fxlara0':
        '["Objects"]["1155773612.45fxlara0"]',
        '1169151219.8mike':
        '["Objects"]["1155773612.45fxlara0"]["Objects"]["1169151219.8mike"]',
        '1201033298.05dxschafe':
        '["Objects"]["1155773612.45fxlara0"]["Objects"]["1201033298.05dxschafe"]',
        '1201109074.66dxschafe':
        '["Objects"]["1155773612.45fxlara0"]["Objects"]["1201109074.66dxschafe"]',
        '1201110056.77dxschafe':
        '["Objects"]["1155773612.45fxlara0"]["Objects"]["1201110056.77dxschafe"]',
        '1201110107.2dxschafe':
        '["Objects"]["1155773612.45fxlara0"]["Objects"]["1201110107.2dxschafe"]',
        '1201110157.89dxschafe':
        '["Objects"]["1155773612.45fxlara0"]["Objects"]["1201110157.89dxschafe"]',
        '1201110790.16dxschafe':
        '["Objects"]["1155773612.45fxlara0"]["Objects"]["1201110790.16dxschafe"]',
        '1201111177.05dxschafe':
        '["Objects"]["1155773612.45fxlara0"]["Objects"]["1201111177.05dxschafe"]',
        '1201111368.09dxschafe':
        '["Objects"]["1155773612.45fxlara0"]["Objects"]["1201111368.09dxschafe"]',
        '1201113000.53dxschafe':
        '["Objects"]["1155773612.45fxlara0"]["Objects"]["1201113000.53dxschafe"]'
    }
}
extraInfo = {
    'camPos': Point3(-64.3119, 83.8757, 62.4736),
    'camHpr': VBase3(-121.812, -31.91, 0),
    'focalLength': 1.39999997616
}
