# Embedded file name: pirates.leveleditor.worldData.tortuga_building_int_3
from pandac.PandaModules import Point3, VBase3, Vec4
objectStruct = {
    'AmbientColors': {},
    'DirectionalColors': {},
    'FogColors': {},
    'FogRanges': {},
    'Objects': {
        '1156270917.73dzlu0': {
            'Type': 'Building Interior',
            'Name': '',
            'AdditionalData': ['interior_shanty_store_barber'],
            'Instanced': True,
            'Objects': {
                '1172091897.53kmuller': {
                    'Type': 'Interior_furnishings',
                    'DisableCollision': False,
                    'Hpr': VBase3(90.558, 0.0, 0.0),
                    'Pos': Point3(8.129, 13.078, 0.0),
                    'Scale': VBase3(1.0, 1.0, 1.0),
                    'Visual': {
                        'Model': 'models/props/interior_wall_shanty'
                    }
                },
                '1196911744.0dxschafe0': {
                    'Type': 'Townsperson',
                    'Category': 'Barber',
                    'AnimSet': 'default',
                    'CustomModel': 'None',
                    'Hpr': VBase3(122.968, 0.0, 0.0),
                    'Patrol Radius': '12.0000',
                    'Pos': Point3(8.779, -14.866, 0.0),
                    'PoseAnim': '',
                    'PoseFrame': '',
                    'Private Status': 'All',
                    'Respawns': True,
                    'Scale': VBase3(1.0, 1.0, 1.0),
                    'ShopID': 'TORTUGA_DEFAULTS',
                    'Start State': 'Idle',
                    'Team': 'Villager'
                },
                '1201041390.45dxschafe': {
                    'Type': 'Door Locator Node',
                    'Name': 'door_locator',
                    'Hpr': VBase3(0.0, 0.0, 0.0),
                    'Pos': Point3(0.047, -29.861, 0.067),
                    'Scale': VBase3(1.0, 1.0, 1.0)
                },
                '1201118516.17dxschafe': {
                    'Type': 'Light - Dynamic',
                    'Attenuation': '0.005',
                    'ConeAngle': '60.0000',
                    'DropOff': '73.7349',
                    'FlickRate': '0.5000',
                    'Hpr': VBase3(100.319, 2.978, 3.46),
                    'Intensity': '0.2410',
                    'LightType': 'POINT',
                    'Pos': Point3(6.066, -16.351, 3.873),
                    'Scale': VBase3(1.0, 1.0, 1.0),
                    'Visual': {
                        'Color': (1, 1, 1, 1),
                        'Model': 'models/props/light_tool_bulb'
                    }
                },
                '1201125071.14dxschafe': {
                    'Type': 'Light - Dynamic',
                    'Attenuation': '0.005',
                    'ConeAngle': '60.0000',
                    'DropOff': '0.0000',
                    'FlickRate': '0.5000',
                    'Hpr': VBase3(-57.191, 50.59, 3.327),
                    'Intensity': '0.3735',
                    'LightType': 'SPOT',
                    'Pos': Point3(6.227, -16.477, 1.507),
                    'Scale': VBase3(1.0, 1.0, 1.0),
                    'Visual': {
                        'Color': (1, 1, 1, 1),
                        'Model': 'models/props/light_tool_bulb'
                    }
                },
                '1201125244.02dxschafe': {
                    'Type': 'Light - Dynamic',
                    'Attenuation': '0.005',
                    'ConeAngle': '60.0000',
                    'DropOff': '0.0000',
                    'FlickRate': '0.5000',
                    'Hpr': VBase3(68.504, -4.572, -6.851),
                    'Intensity': '1.0241',
                    'LightType': 'SPOT',
                    'Pos': Point3(17.642, -20.817, 7.308),
                    'Scale': VBase3(1.0, 1.0, 1.0),
                    'Visual': {
                        'Color': (1, 1, 1, 1),
                        'Model': 'models/props/light_tool_bulb'
                    }
                }
            },
            'Visual': {
                'Model': 'models/buildings/interior_shanty_npc_house'
            }
        }
    },
    'Node Links': [],
    'Layers': {
        'Collisions': [
            '1184008208.59kmuller', '1184016064.62kmuller',
            '1184013852.84kmuller', '1185822696.06kmuller',
            '1184006140.32kmuller', '1184002350.98kmuller',
            '1184007573.29kmuller', '1184021176.59kmuller',
            '1184005963.59kmuller', '1188324241.31akelts',
            '1184006537.34kmuller', '1184006605.81kmuller',
            '1187139568.33kmuller', '1188324186.98akelts',
            '1184006730.66kmuller', '1184007538.51kmuller',
            '1184006188.41kmuller', '1184021084.27kmuller',
            '1185824396.94kmuller', '1185824250.16kmuller',
            '1185823630.52kmuller', '1185823760.23kmuller',
            '1185824497.83kmuller', '1185824751.45kmuller',
            '1187739103.34akelts', '1188323993.34akelts',
            '1184016538.29kmuller', '1185822200.97kmuller',
            '1184016225.99kmuller', '1195241421.34akelts',
            '1195242796.08akelts', '1184020642.13kmuller',
            '1195237994.63akelts', '1184020756.88kmuller',
            '1184020833.4kmuller', '1185820992.97kmuller',
            '1185821053.83kmuller', '1184015068.54kmuller',
            '1184014935.82kmuller', '1185821432.88kmuller',
            '1185821701.86kmuller', '1195240137.55akelts',
            '1195241539.38akelts', '1195238422.3akelts', '1195238473.22akelts',
            '1185821453.17kmuller', '1184021269.96kmuller',
            '1185821310.89kmuller', '1185821165.59kmuller',
            '1185821199.36kmuller', '1185822035.98kmuller',
            '1184015806.59kmuller', '1185822059.48kmuller',
            '1185920461.76kmuller', '1194984449.66akelts',
            '1185824206.22kmuller', '1184003446.23kmuller',
            '1184003254.85kmuller', '1184003218.74kmuller',
            '1184002700.44kmuller', '1186705073.11kmuller',
            '1187658531.86akelts', '1186705214.3kmuller',
            '1185824927.28kmuller', '1184014204.54kmuller',
            '1184014152.84kmuller'
        ]
    },
    'ObjectIds': {
        '1156270917.73dzlu0':
        '["Objects"]["1156270917.73dzlu0"]',
        '1172091897.53kmuller':
        '["Objects"]["1156270917.73dzlu0"]["Objects"]["1172091897.53kmuller"]',
        '1196911744.0dxschafe0':
        '["Objects"]["1156270917.73dzlu0"]["Objects"]["1196911744.0dxschafe0"]',
        '1201041390.45dxschafe':
        '["Objects"]["1156270917.73dzlu0"]["Objects"]["1201041390.45dxschafe"]',
        '1201118516.17dxschafe':
        '["Objects"]["1156270917.73dzlu0"]["Objects"]["1201118516.17dxschafe"]',
        '1201125071.14dxschafe':
        '["Objects"]["1156270917.73dzlu0"]["Objects"]["1201125071.14dxschafe"]',
        '1201125244.02dxschafe':
        '["Objects"]["1156270917.73dzlu0"]["Objects"]["1201125244.02dxschafe"]'
    }
}
extraInfo = {
    'camPos': Point3(-33.8949, -67.1637, 42.2035),
    'camHpr': VBase3(-25.373, -20.6714, 0),
    'focalLength': 1.39999997616
}
