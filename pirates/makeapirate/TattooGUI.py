...
class TattooGUI(DirectFrame, StateData.StateData):
    ...
    def loadZone1GUI(self):
        self.zone1FrameTitle = DirectFrame(
            parent=self._parent,
            relief=DGG.FLAT,
            frameColor=(1, 1, 0, 1),
            text='Chest Tattoo',
        ...
        self.zone2FrameTitle = DirectFrame(
            parent=self._parent,
            relief=DGG.FLAT,
            frameColor=(1, 1, 0, 1),
            text='LeftArm Tattoo',
        ...
        self.zone3FrameTitle = DirectFrame(
            parent=self._parent,
            relief=DGG.FLAT,
            frameColor=(1, 1, 0, 1),
            text='RightArm Tattoo',
            text_scale=0.18,
            text_pos=(0, 0),
            pos=(0.0, 0, 0.7),
            scale=(0.66, 0.4, 0.53))
        ...
        self.zone4FrameTitle = DirectFrame(
            parent=self._parent,
            relief=DGG.FLAT,
            frameColor=(1, 1, 0, 1),
            text='Face Tattoo',
        ...
        self.zone5FrameTitle = DirectFrame(
            parent=self._parent,
            relief=DGG.FLAT,
            frameColor=(1, 1, 0, 1),
            text='Zone5 Tattoo',
        ...
        self.zone6FrameTitle = DirectFrame(
            parent=self._parent,
            relief=DGG.FLAT,
            frameColor=(1, 1, 0, 1),
            text='Zone6 Tattoo',
        ...
        self.zone7FrameTitle = DirectFrame(
            parent=self._parent,
            relief=DGG.FLAT,
            frameColor=(1, 1, 0, 1),
            text='Zone7 Tattoo',
        ...
        self.zone8FrameTitle = DirectFrame(
            parent=self._parent,
            relief=DGG.FLAT,
            frameColor=(1, 1, 0, 1),
            text='Zone8 Tattoo',
        ...
        self.oXSlider = CharGuiSlider(
            self.main,
            parent=self.zone1FrameTitle,
            text='OffsetX',
            command=self.handleOxSlider,
            range=(0.0, 1.0))
        self.oXSlider['extraArgs'] = [self.oXSlider]
        self.oXSlider.setPos(-0.5, 0, -2.0)
        self.oYSlider = CharGuiSlider(
            self.main,
            parent=self.zone1FrameTitle,
            text='OffsetY',
            command=self.handleOySlider,
            range=(0.0, 1.0))
        self.oYSlider['extraArgs'] = [self.oYSlider]
        self.oYSlider.setPos(-0.5, 0, -2.3)
        self.scaleSlider = CharGuiSlider(
            self.main,
            parent=self.zone1FrameTitle,
            text='Scale',
            command=self.handleScaleSlider,
            range=(0.01, 1))
        self.scaleSlider['extraArgs'] = [self.scaleSlider]
        self.scaleSlider.setPos(-0.5, 0, -2.6)
        self.rotateSlider = CharGuiSlider(
            self.main,
            parent=self.zone1FrameTitle,
            text='Rotate',
        ...
        self.editZone5.hide()
        self.editButtons = [
            self.editZone1, self.editZone2, self.editZone3, self.editZone4,
            self.editZone5, self.editZone5, self.editZone5, self.editZone5
        ]
        ...
        if mode == Zone1_Tab:
            ...
            if mode == Zone2_Tab:
                ...
                if mode == Zone3_Tab:
                    ...
                    self.zone8Tab['relief'] = DGG.RAISED
                    self.hideZone8TattooCollections()
                    self.reparentCommonGui(self.zone3Frame,
                                           self.zone3FrameTitle)
                else:
                    if mode == Zone4_Tab:
                        self.zone1Tab['state'] = DGG.NORMAL
                        self.zone1Tab['relief'] = DGG.RAISED
                        ...
                        self.zone8Tab['relief'] = DGG.RAISED
                        self.hideZone8TattooCollections()
                        self.reparentCommonGui(self.zone4Frame,
                                               self.zone4FrameTitle)
                    else:
                        if mode == Zone5_Tab:
                            self.zone1Tab['state'] = DGG.NORMAL
                            self.zone1Tab['relief'] = DGG.RAISED
                            ...
                            self.zone8Tab['relief'] = DGG.RAISED
                            self.hideZone8TattooCollections()
                            self.reparentCommonGui(self.zone5Frame,
                                                   self.zone5FrameTitle)
                        else:
                            if mode == Zone6_Tab:
                                self.zone1Tab['state'] = DGG.NORMAL
                                self.zone1Tab['relief'] = DGG.RAISED
                                ...
                                self.zone8Tab['relief'] = DGG.RAISED
                                self.hideZone8TattooCollections()
                                self.reparentCommonGui(self.zone6Frame,
                                                       self.zone6FrameTitle)
                            else:
                                if mode == Zone7_Tab:
                                    self.zone1Tab['state'] = DGG.NORMAL
                                    self.zone1Tab['relief'] = DGG.RAISED
                                    ...
                                    self.zone8Tab['relief'] = DGG.RAISED
                                    self.hideZone8TattooCollections()
                                    self.reparentCommonGui(
                                        self.zone7Frame, self.zone7FrameTitle)
                                else:
                                    if mode == Zone8_Tab:
                                        self.zone1Tab['state'] = DGG.NORMAL
                                        self.zone1Tab['relief'] = DGG.RAISED
                ...
                if mode == Zone3_Tab:
                    ...
                    if mode == Zone4_Tab:
                        ...
                        if mode == Zone5_Tab:
                            ...
                            if mode == Zone6_Tab:
                                idx = self.avatar.pirate.style.getTattooZone6()[
                                    0]
                            else:
                                if mode == Zone7_Tab:
                                    idx = self.avatar.pirate.style.getTattooZone7(
                                    )[0]
                                else:
                                    if mode == Zone8_Tab:
                                        idx = self.avatar.pirate.style.getTattooZone8(
                                        )[0]
    ...
    def setupButtons(self):
        self.guiNextTattooButton = DirectButton(
            parent=self.zone1Frame,
            relief=DGG.RAISED,
            pos=(0.3, 0.0, -0.5),
            scale=(2.0, 2.0, 2.0),
            command=self.handleNextTattoo,
            frameSize=(-0.09, 0.09, -0.05, 0.05),
            borderWidth=(0.008, 0.008),
            text='>>',
            text_pos=(0, -0.015),
            text_scale=0.1)
        self.guiLastTattooButton = DirectButton(
            parent=self.zone1Frame,
            relief=DGG.RAISED,
            pos=(-0.3, 0.0, -0.5),
            scale=(2.0, 2.0, 2.0),
            command=self.handleLastTattoo,
            frameSize=(-0.09, 0.09, -0.05, 0.05),
            borderWidth=(0.008, 0.008),
            text='<<',
            text_pos=(0, -0.015),
    ...
    def reset(self):
        ...
        self.avatar.pirate.setTattooChest(val[TYPE], val[OFFSETX], val[OFFSETY],
                                          val[SCALE], val[ROTATE], val[COLOR])
        val = self.avatar.tattoos[ZONE2]
        self.avatar.pirate.setTattooZone2(val[TYPE], val[OFFSETX], val[OFFSETY],
                                          val[SCALE], val[ROTATE], val[COLOR])
        val = self.avatar.tattoos[ZONE3]
        self.avatar.pirate.setTattooZone3(val[TYPE], val[OFFSETX], val[OFFSETY],
                                          val[SCALE], val[ROTATE], val[COLOR])
        val = self.avatar.tattoos[ZONE4]
        self.avatar.pirate.setTattooZone4(val[TYPE], val[OFFSETX], val[OFFSETY],
                                          val[SCALE], val[ROTATE], val[COLOR])
        val = self.avatar.tattoos[ZONE5]
        self.avatar.pirate.setTattooZone5(val[TYPE], val[OFFSETX], val[OFFSETY],
                                          val[SCALE], val[ROTATE], val[COLOR])
        val = self.avatar.tattoos[ZONE6]
        self.avatar.pirate.setTattooZone6(val[TYPE], val[OFFSETX], val[OFFSETY],
                                          val[SCALE], val[ROTATE], val[COLOR])
        val = self.avatar.tattoos[ZONE7]
        self.avatar.pirate.setTattooZone7(val[TYPE], val[OFFSETX], val[OFFSETY],
                                          val[SCALE], val[ROTATE], val[COLOR])
        val = self.avatar.tattoos[ZONE8]
        self.avatar.pirate.setTattooZone8(val[TYPE], val[OFFSETX], val[OFFSETY],
                                          val[SCALE], val[ROTATE], val[COLOR])
    ...
    def saveDNA(self):
        ...
        self.avatar.pirate.setTattooChest(tattoo[TYPE], tattoo[OFFSETX],
                                          tattoo[OFFSETY], tattoo[SCALE],
                                          tattoo[ROTATE], tattoo[COLOR])
        tattoo = self.avatar.tattoos[ZONE2]
        self.avatar.pirate.setTattooZone2(tattoo[TYPE], tattoo[OFFSETX],
                                          tattoo[OFFSETY], tattoo[SCALE],
                                          tattoo[ROTATE], tattoo[COLOR])
        tattoo = self.avatar.tattoos[ZONE3]
        self.avatar.pirate.setTattooZone3(tattoo[TYPE], tattoo[OFFSETX],
                                          tattoo[OFFSETY], tattoo[SCALE],
                                          tattoo[ROTATE], tattoo[COLOR])
        tattoo = self.avatar.tattoos[ZONE4]
        self.avatar.pirate.setTattooZone4(tattoo[TYPE], tattoo[OFFSETX],
                                          tattoo[OFFSETY], tattoo[SCALE],
                                          tattoo[ROTATE], tattoo[COLOR])
        tattoo = self.avatar.tattoos[ZONE5]
        self.avatar.pirate.setTattooZone5(tattoo[TYPE], tattoo[OFFSETX],
                                          tattoo[OFFSETY], tattoo[SCALE],
                                          tattoo[ROTATE], tattoo[COLOR])
        tattoo = self.avatar.tattoos[ZONE6]
        self.avatar.pirate.setTattooZone6(tattoo[TYPE], tattoo[OFFSETX],
                                          tattoo[OFFSETY], tattoo[SCALE],
                                          tattoo[ROTATE], tattoo[COLOR])
        tattoo = self.avatar.tattoos[ZONE7]
        self.avatar.pirate.setTattooZone7(tattoo[TYPE], tattoo[OFFSETX],
                                          tattoo[OFFSETY], tattoo[SCALE],
                                          tattoo[ROTATE], tattoo[COLOR])
        tattoo = self.avatar.tattoos[ZONE8]
        self.avatar.pirate.setTattooZone8(tattoo[TYPE], tattoo[OFFSETX],
                                          tattoo[OFFSETY], tattoo[SCALE],
                                          tattoo[ROTATE], tattoo[COLOR])
    ...
    def handleNextTattoo(self):
        ...
        self.mapTexture(self.currTattoo, tattoo[OFFSETX], tattoo[OFFSETY],
                        tattoo[SCALE], tattoo[ROTATE])
        self.updateCard()
        self.avatar.updateTattoo(self.currTattoo)
        self.notify.info(
            'Current Tattoo: %s' % TattooGlobals.
            tattooNames[self.avatar.tattoos[self.currTattoo][TYPE]])
    ...
    def updateCard(self):
        tattooFrame = [
            self.zone1Frame, self.zone2Frame, self.zone3Frame, self.zone4Frame,
            self.zone5Frame, self.zone6Frame, self.zone7Frame, self.zone8Frame
        ]
        tattoo = self.avatar.tattoos[self.currTattoo]
        tex = TattooGlobals.getTattooImage(tattoo[0])
        t = TransformState.makePosRotateScale2d(
            Vec2(tattoo[OFFSETX], tattoo[OFFSETY]), tattoo[ROTATE],
            Vec2(tattoo[SCALE], tattoo[SCALE]))
        self.map.setTexTransform(self.cStage, t)
        self.map.setTexture(self.cStage, tex)
        tattooFrame[self.currTattoo]['text'] = TattooGlobals.tattooNames[
            self.avatar.tattoos[self.currTattoo][TYPE]]
    ...
    def mapTexture(self, index, offsetx, offsety, scale, rotate):
        ...
        self.vTattoos[index] = [
            self.vTattoos[index][TYPE], offsetx, offsety, scale, rotate,
    ...
    def handleOxSlider(self, pgs):
        ...
        value = pgs['value']
        tattoo = self.vTattoos[self.currTattoo]
        self.mapTexture(self.currTattoo, value, tattoo[OFFSETY], tattoo[SCALE],
                        tattoo[ROTATE])
        self.updateCard()
        self.avatar.updateTattoo(self.currTattoo)
    ...
    def handleOySlider(self, pgs):
        ...
        value = pgs['value']
        tattoo = self.vTattoos[self.currTattoo]
        self.mapTexture(self.currTattoo, tattoo[OFFSETX], value, tattoo[SCALE],
                        tattoo[ROTATE])
        self.updateCard()
        self.avatar.updateTattoo(self.currTattoo)
