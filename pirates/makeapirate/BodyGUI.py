...
class BodyGUI(DirectFrame, StateData.StateData):
    ...
    def loadShapeGUI(self):
        ...
        self.shapeFrameTitle = DirectFrame(
            parent=self._parent,
            relief=None,
            text=PLocalizer.BodyShapeFrameTitle,
            text_fg=(1, 1, 1, 1),
            text_scale=0.18,
            text_pos=(0, 0),
            pos=(0, 0, -0.1),
    ...
    def loadHeightGUI(self):
        self.heightSlider = CharGuiSlider(
            self.main,
            parent=self._parent,
            text=PLocalizer.BodyHeightFrameTitle,
            command=self.updateHeightSlider,
            range=(-0.1, 0.1))
    ...
    def loadColorGUI(self):
        idx = 0
        self.maleColorFrameTitle = DirectFrame(
            parent=self._parent,
            relief=None,
        ...
        for i in range(0, len(HumanDNA.skinColors)):
            ...
            skinTone = (skinColor[0] * whiteSkinTone[0],
                        skinColor[1] * whiteSkinTone[1],
                        skinColor[2] * whiteSkinTone[2], 1.0)
            self.maleColorButtons.append(
                DirectButton(
                    parent=self.maleColorFrameTitle,
                    relief=DGG.RAISED,
        ...
        idx = 1
        self.femaleColorFrameTitle = DirectFrame(
            parent=self._parent,
            relief=None,
            image=self.main.charGui.find('**/chargui_frame05'),
            image_pos=(0, 0, -0.6),
        ...
        for i in range(0, len(HumanDNA.skinColors)):
            ...
            skinTone = (skinColor[0] * whiteSkinTone[0],
                        skinColor[1] * whiteSkinTone[1],
                        skinColor[2] * whiteSkinTone[2], 1.0)
            self.femaleColorButtons.append(
                DirectButton(
                    parent=self.femaleColorFrameTitle,
                    relief=DGG.RAISED,
                    pos=(xOffset, 0, yOffset),
                    frameSize=(-0.1, 0.1, -0.1, 0.1),
                    borderWidth=(0.008, 0.008),
    ...
    def showShapeCollections(self):
        if self.entered:
            ...
            self.shapeButtons[self.main.pirate.style.body.shape].setColor(
                1, 1, 0.5, 1)
    ...
    def showColorCollections(self):
        if self.entered:
            if self.main.pirate.style.gender == 'f':
                ...
                self.femaleColorButtons[self.main.pirate.style.body.color][
                    'relief'] = DGG.SUNKEN
            else:
                self.maleColorFrameTitle.show()
                self.maleColorButtons[self.main.pirate.style.body.color][
                    'relief'] = DGG.SUNKEN
    ...
    def setupButtons(self):
        self.texturePicker = CharGuiPicker(
            self.main,
            parent=self._parent,
            text=PLocalizer.BodyHairFrameTitle,
        ...
        shapeButtonTexts = [
            PLocalizer.BodyShortFat, PLocalizer.BodyMediumSkinny,
            PLocalizer.BodyMediumIdeal, PLocalizer.BodyTallPear,
        ...
        for i in range(0, len(self.maleShapeButtonIcons)):
            self.shapeButtons.append(
                DirectButton(
                    parent=self.shapeFrameTitle,
                    relief=None,
                    geom=(self.maleShapeButtonIcons[i],
                          self.maleShapeButtonIconsOver[i],
                          self.maleShapeButtonIconsOver[i]),
                    image_scale=(0.7, 1, 1),
                    pos=(xOffset, 0, yOffset),
    ...
    def handleNextTexture(self):
        self.avatar.bodyTextureIdx = (
            self.avatar.bodyTextureIdx + 1) % self.avatar.numBodys
        ...
        if gender == 'f':
            ...
            for i in range(0, len(self.femaleShapeButtonIcons)):
                self.shapeButtons[i]['geom'] = (
                    self.femaleShapeButtonIcons[i],
                    self.femaleShapeButtonIconsOver[i],
                    self.femaleShapeButtonIconsOver[i])

        self.notify.debug('gender is male')
        for i in range(0, len(self.maleShapeButtonIcons)):
            self.shapeButtons[i]['geom'] = (self.maleShapeButtonIcons[i],
                                            self.maleShapeButtonIconsOver[i],
                                            self.maleShapeButtonIconsOver[i])
