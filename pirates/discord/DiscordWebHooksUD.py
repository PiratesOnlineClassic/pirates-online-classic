...
class WebhookBase(object):
    ...
    notify = DirectNotifyGlobal.directNotify.newCategory('WebhookBase')

    def __init__(self, air, url, extension=None):
        self.air = air
        self.url = url
    ...
    def send(self):
        ...
        def processResults(results):
            if results != '' and result != 'ok':
                self.notify.warning('Unexpected error occured while sending post; %s' % str(result))
            else:
...
class GenericWebhook(WebhookBase):
    ...
    notify = DirectNotifyGlobal.directNotify.newCategory('GenericWebhook')
...
class GithubWebhook(WebhookBase):
    ...
    notify = DirectNotifyGlobal.directNotify.newCategory('GithubWebhook')
...
class SlackWebhook(WebhookBase):
    ...
    notify = DirectNotifyGlobal.directNotify.newCategory('SlackWebhook')
    ...
    def formatMessage(self):
        ...
        for attachment in self.attachments:
            ...
            att['thumb_url'] = attachment.thumb_url
...
class SlackAttachment(object):
    ...
    def __init__(self, author_name='', author_icon='', color='', pretext='', title='', title_link='', image_url='', thumb_url='', footer='', footer_icon='', ts=0, fields=[]):
