...
class Pistol(Weapon.Weapon):
    modelTypes = {
        InventoryType.PistolWeaponL1: ('models/handheld/pistol_high',
                                       Vec4(1, 1, 1, 1)),
        InventoryType.PistolWeaponL2:
        ('models/handheld/double_barrel_pistol_high', Vec4(1, 1, 1, 1)),
        InventoryType.PistolWeaponL3:
        ('models/handheld/triple_barrel_pistol_high', Vec4(1, 1, 1, 1)),
        InventoryType.PistolWeaponL4:
        ('models/handheld/triple_barrel_pistol_b_high', Vec4(1, 1, 1, 1)),
        InventoryType.PistolWeaponL5:
        ('models/handheld/triple_barrel_pistol_c_high', Vec4(1, 1, 1, 1)),
        InventoryType.PistolWeaponL6:
        ('models/handheld/quad_barrel_pistol_high', Vec4(1, 1, 1, 1))
    }
    walkAnim = 'walk'
    ...
    def getDrawIval(self, av, ammoSkillId=0, blendInT=0.1, blendOutT=0):
        track = Parallel(
            Func(base.playSfx, self.drawSfx, node=av, cutoff=60),
            av.actorInterval(
                'gun_draw',
                playRate=1.5,
                endFrame=35,
    ...
    def getReturnIval(self, av, blendInT=0, blendOutT=0.1):
        track = Parallel(
            Func(base.playSfx, self.returnSfx, node=av, cutoff=60),
            av.actorInterval(
                'gun_putaway',
                playRate=2,
                endFrame=37,
    ...
    def setupSounds(cls):
        ...
        Pistol.skillSfxs = {
            ...
            loader.loadSfx('audio/sfx_pistol_shoot.mp3'),
