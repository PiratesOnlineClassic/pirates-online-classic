...
class WeaponBaseAI(WeaponBaseBase):
    ...
    def requestTargetedSkill(self, skillId, ammoSkillId, clientResult, targetId,
        ...
        self.__useTargetedSkill(avatar, target, skillId, ammoSkillId,
        ...
        for targetId in areaIdList:
            ...
            if not target:
                self.notify.debug(
                    'Cannot request targeted skill, unknown areaId target; avatarId=%d skillId=%d!'
                    % (avatar.doId, skillId))
    ...
    def requestProjectileSkill(self, skillId, ammoSkillId, posHpr, power,
                               timestamp):
        avatar = self.air.doId2do.get(self.air.getAvatarIdFromSender())
        ...
        if self.air.targetMgr.hasProjectile(avatar.doId, skillId, ammoSkillId):
            self.notify.debug(
                'Avatar %d tried to request projectile skill for already used skill; skillId=%d ammoSkillId=%d!'
        ...
        self.air.targetMgr.addProjectile(avatar.doId, skillId, ammoSkillId,
                                         timestamp)

        # send the projectile skill update so everyone in the same zone
        # will see the projectile...
        self.sendUpdate('useProjectileSkill',
                        [skillId, ammoSkillId, posHpr, timestamp, power])

    def suggestProjectileSkillResult(self, skillId, ammoSkillId, result,
                                     targetId, areaIdList, pos, normal, codes,
                                     timestamp):
        avatar = self.air.doId2do.get(self.air.getAvatarIdFromSender())
        ...
        if not self.air.targetMgr.hasProjectile(avatar.doId, skillId,
                                                ammoSkillId):
            self.notify.debug(
                'Avatar %d tried to request projectile skill for skill never used; skillId=%d ammoSkillId=%d!'
                % (avatar.doId, skillId, ammoSkillId))
        ...
        for targetId in areaIdList:
            ...
            if not target:
                self.notify.debug(
                    'Cannot request projectil skill, unknown areaId target; avatarId=%d skillId=%d!'
                    % (avatar.doId, skillId))

                continue

            self.__useProjectileSkill(avatar, target, skillId, ammoSkillId,
                                      result, targetId, areaIdList, pos, normal,
                                      codes, timestamp)
    ...
    def __useTargetedSkill(self, avatar, target, skillId, ammoSkillId,
                           clientResult, areaIdList, timestamp, pos, charge):
        targetResult = self.air.battleMgr.getTargetedSkillResult(
            avatar, target, skillId, ammoSkillId, clientResult, areaIdList,
            timestamp, pos, charge)

        if not targetResult:
            self.notify.debug(
                'Cannot get targeted skill, no valid result was given; avatarId=%d, skillId=%d!'
                % (avatar.doId, skillId))
    ...
    def __useProjectileSkill(self, avatar, target, skillId, ammoSkillId, result,
                             targetId, areaIdList, pos, normal, codes,
                             timestamp):
        targetResult = self.air.battleMgr.getTargetedSkillResult(
            avatar, target, skillId, ammoSkillId, result, areaIdList, timestamp,
            pos)

        if not targetResult:
            self.notify.debug(
                'Cannot get projectile targeted skill, no valid result was given; avatarId=%d, skillId=%d!'
                % (avatar.doId, skillId))
        ...
        self.sendUpdate('setProjectileSkillResult', [
            skillId, ammoSkillId, skillResult, targetId if not target else
            target.doId, areaIdList, attackerEffects, targetEffects,
            areaIdEffects, pos, normal, codes, avatar.doId, timestamp
        ])
