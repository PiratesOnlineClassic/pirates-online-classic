...
class Grenade(Weapon.Weapon):
    modelTypes = {
        InventoryType.GrenadeWeaponL1: ('models/ammunition/grenade',
                                        Vec4(1, 1, 1, 1)),
        InventoryType.GrenadeWeaponL2: ('models/ammunition/grenade',
                                        Vec4(1, 1, 1, 1)),
        InventoryType.GrenadeWeaponL3: ('models/ammunition/grenade',
                                        Vec4(1, 1, 1, 1)),
        InventoryType.GrenadeWeaponL4: ('models/ammunition/grenade',
                                        Vec4(1, 1, 1, 1)),
        InventoryType.GrenadeWeaponL5: ('models/ammunition/grenade',
                                        Vec4(1, 1, 1, 1)),
        InventoryType.GrenadeWeaponL6: ('models/ammunition/grenade',
                                        Vec4(1, 1, 1, 1))
    }
    walkAnim = 'walk'
    ...
    def changeStance(self, av):
        ...
        if not self.ammoSkillId:
            ...
            if av.isLocal():
                av.controlManager.setSpeeds(
                    *PiratesGlobals.PirateSpeeds[av.speedIndex])
    ...
    def getDrawIval(self, av, ammoSkillId=0, blendInT=0.1, blendOutT=0):
        if not ammoSkillId:
            track = Sequence(
                Func(self.setAmmoSkillId, 0), Func(self.changeStance, av),
                Func(self.detachFrom, av))
        else:
            if ammoSkillId == InventoryType.GrenadeSiege:
                track = Parallel(
                    ...
                    Sequence(
                        Wait(0.343), Func(self.attachTo, av),
                        Func(self.setAmmoSkillId, ammoSkillId),
                        Func(self.changeStance, av)))
            else:
                track = Parallel(
                    av.actorInterval(
                        'bomb_draw',
                    ...
                    Func(base.playSfx, self.drawSfx, node=av),
                    Sequence(
                        Wait(0.125), Func(self.attachTo, av),
                        Func(self.setAmmoSkillId, ammoSkillId),
                        Func(self.changeStance, av)))
        return track
    ...
    def getReturnIval(self, av, ammoSkillId=0, blendInT=0, blendOutT=0.1):
        ...
        if ammoSkillId == InventoryType.GrenadeSiege:
            track = Parallel(
                ...
                Sequence(
                    Wait(0.5), Func(self.detachFrom, av),
                    Func(self.setAmmoSkillId, 0), Func(self.changeStance, av)))
        else:
            track = Parallel(
                av.actorInterval(
                ...
                Func(base.playSfx, self.returnSfx, node=av),
                Sequence(
                    Wait(0.468), Func(self.detachFrom, av),
                    Func(self.setAmmoSkillId, 0), Func(self.changeStance, av)))
        return track
        ...
        if self.ammoSkillId == InventoryType.GrenadeSiege:
            track = Sequence(
                ...
                av.actorInterval(
                    ...
                    endFrame=11,
                    blendInT=0.25,
                    blendOutT=0), Func(self.detachFrom, av))
        else:
            track = Sequence(
                Func(self.lockInput, av),
                av.actorInterval(
    ...
    def setupSounds(cls):
        ...
        Grenade.skillSfxs = {
            ...
            loader.loadSfx('audio/sfx_grenade_throw.mp3'),
            InventoryType.GrenadeSiege:
            loader.loadSfx('audio/sfx_grenade_bigbomb_throw.mp3'),
            InventoryType.GrenadeLongVolley:
            loader.loadSfx('audio/sfx_grenade_long_volley_throw.mp3')
        }
        Grenade.chargingSfx = loader.loadSfx(
            'audio/sfx_grenade_long_volley_charging.mp3')
        Grenade.drawSfx = loader.loadSfx('audio/sfx_grenade_bigbomb_draw.mp3')
        Grenade.returnSfx = loader.loadSfx(
            'audio/sfx_grenade_bigbomb_put_away.mp3')
