...
class DistributedTreasureMap(DistributedInventory, QuestHolder):
    ...
    def requestTreasureMapGo(self, quick = True):
        if quick:
            ...
            localAvatar.guiMgr.lookoutPage.displayLookout(gameStyle=self.mapId, inviteOptions=[PiratesGlobals.LOOKOUT_INVITE_CREW], additionalOptions=[[
                str(GameTypeGlobals.GAME_OPTION_TM_ID), str(self.getDoId())]])
